...
def run(config_path: str | Path, *, stage: str) -> dict[str, Any]:
    ...
    if resolved_stage in {"all", "extract"}:
        ...
        cache = extract_shooting_embedding_cache(
            ...
            horizons_ps=[float(value) for value in required(cfg, "data.horizons_ps")],
            ...
            spatial_context_center_count=int(
                required(cfg, "data.legacy_context_center_count")
            ),
    ...
    fixed_kernel = OmegaConf.select(cfg, "target.fixed_path_kernel", default=None)
    if fixed_kernel is None:
        target = prepare_joint_path_target_data(
            ...
            horizon_weights=[
                float(value) for value in required(cfg, "target.horizon_weights")
            ],
            ...
            selection_source_velocity_seeds=[
                int(value)
                for value in required(cfg, "split.selection_source_velocity_seeds")
            ],
        ...
        target = prepare_joint_path_target_data_from_kernel(
            cache,
            kernel_path=resolve_path(fixed_kernel),
            selection_source_velocity_seeds=[
                int(value)
                for value in required(cfg, "split.selection_source_velocity_seeds")
            ],
    ...
    pretrained = _load_pretrained_backbones(
        resolve_path(required(cfg, "initialization.pretrained_backbones"))
    )
    marginal_target = prepare_distributional_target_data(
        cache,
        horizons_ps=[
            float(value) for value in required(cfg, "marginal_baseline.horizons_ps")
        ],
        change_pca_dim=int(required(cfg, "marginal_baseline.change_pca_dim")),
        rff_features_per_bandwidth=int(
            required(cfg, "marginal_baseline.rff_features_per_bandwidth")
        ),
        bandwidth_multipliers=[
            float(value)
            for value in required(cfg, "marginal_baseline.bandwidth_multipliers")
        ],
        selection_source_velocity_seeds=[
            int(value) for value in required(cfg, "split.selection_source_velocity_seeds")
        ],
        seed=int(required(cfg, "marginal_baseline.seed")),
    )
    marginal_fitted = fit_spatial_context_transformer(
        ...
        representation_dim=int(required(cfg, "marginal_baseline.representation_dim")),
    ...
    static_name = f"local_pca_{int(required(cfg, 'evaluation.static_pca_dim'))}d"
    ...
    rng = np.random.default_rng(int(required(cfg, "evaluation.seed")))
    validation_rows = target.split_rows["validation"]
    pullback_rows = np.sort(
        rng.choice(
            validation_rows,
            size=min(
                int(required(cfg, "evaluation.pullback.maximum_states")),
    ...
    pullback_eigenvalues, pullback_rank = compute_pullback_spectrum(
        ...
        rows=pullback_rows,
        device=device,
        batch_size=int(required(cfg, "evaluation.pullback.batch_size")),
        relative_eigenvalue_cutoff=float(
            required(cfg, "evaluation.pullback.relative_eigenvalue_cutoff")
        ),
    )
    ...
    metrics = {
        "scientific_contract": {
            ...
            "target": (
                ...
                "representation path across independent shooting branches"
            ),
            "claim_scope": (
