...
def run(config_path: str | Path, *, stage: str) -> dict[str, Any]:
    ...
    if resolved_stage in {"all", "extract"}:
        ...
        history_cache = extract_shooting_history_embedding_cache(
            ...
            context_cache,
            encoder=encoder,
            source_trajectory_root=resolve_path(
                required(cfg, "history.source_trajectory_root")
            ),
            cache_path=history_path,
            lag_frames=[int(value) for value in required(cfg, "history.lag_frames")],
            lag_times_ps=[
                float(value) for value in required(cfg, "history.lag_times_ps")
            ],
    ...
    direct_gain = _bootstrap_gain(
        ...
        seed=int(required(cfg, "evaluation.seed")) + 101,
    )
