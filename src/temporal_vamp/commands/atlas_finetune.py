...
from src.temporal_vamp.predictive_atlas_finetune import fit_predictive_atlas_last_geoframe_block
...
def run(config_path: str | Path, *, stage: str) -> dict[str, Any]:
    ...
    snapshot = load_predictive_shooting_snapshot(
        ...
        temperatures_K=[
            float(value) for value in required(cfg, "data.temperatures_K")
        ],
        minimum_complete_branches_per_parent=int(
            required(cfg, "data.minimum_complete_branches_per_parent")
        ),
        basin_roles=OmegaConf.select(cfg, "data.basin_roles", default=None),
    )
    expected = (
        int(required(cfg, "data.expected_parent_count")),
    ...
    base_run = resolve_path(required(cfg, "initialization.position_atlas_run"))
    ...
    history_cache = ShootingHistoryEmbeddingCache.load(
        resolve_path(required(cfg, "initialization.history_embedding_cache"))
    )
    ...
    target = prepare_joint_path_target_data_from_kernel(
        cache,
        kernel_path=resolve_path(required(cfg, "initialization.fixed_path_kernel")),
        selection_source_velocity_seeds=[
            int(value) for value in required(cfg, "split.selection_source_velocity_seeds")
        ],
        rff_device=device,
        rff_batch_size=int(required(cfg, "target.rff_batch_size")),
    )
    ...
    initial_atlas, _ = _load_temporal_atlas(
        resolve_path(required(cfg, "initialization.temporal_atlas_checkpoint"))
    )
    frozen_fitted = fit_predictive_atlas(
        ...
        latent_dim=int(required(cfg, "model.latent_dim")),
        decoder_hidden_dim=int(required(cfg, "model.decoder_hidden_dim")),
        ...
        variance_weight=float(required(cfg, "training.variance_weight")),
        covariance_weight=float(required(cfg, "training.covariance_weight")),
    ...
    fine_tuned = fit_predictive_atlas_last_geoframe_block(
        ...
        frozen_fitted,
        device=device,
        atlas_learning_rate=float(required(cfg, "fine_tuning.atlas_learning_rate")),
        encoder_learning_rate=float(
            required(cfg, "fine_tuning.encoder_learning_rate")
        ),
        weight_decay=float(required(cfg, "fine_tuning.weight_decay")),
        variance_weight=float(required(cfg, "fine_tuning.variance_weight")),
        covariance_weight=float(required(cfg, "fine_tuning.covariance_weight")),
        batch_size=int(required(cfg, "fine_tuning.batch_size")),
        maximum_epochs=int(required(cfg, "fine_tuning.maximum_epochs")),
        patience=int(required(cfg, "fine_tuning.patience")),
        gradient_clip_norm=float(required(cfg, "fine_tuning.gradient_clip_norm")),
        mixed_precision=bool(required(cfg, "fine_tuning.mixed_precision")),
    ...
    spaces, vamp = build_atlas_baseline_spaces(
        ...
        vamp_horizon_ps=float(required(cfg, "evaluation.vamp.horizon_ps")),
        vamp_dimension=int(required(cfg, "evaluation.vamp.dimension")),
        vamp_regularization=float(required(cfg, "evaluation.vamp.regularization")),
        vamp_eigenvalue_cutoff=float(
            required(cfg, "evaluation.vamp.eigenvalue_cutoff")
        ),
    )
    ...
    evaluation, evaluation_arrays, _ = evaluate_predictive_atlas(
        ...
        target,
        spaces,
        static_space_name=f"local_pca_{int(required(cfg, 'evaluation.static_pca_dim'))}d",
        neighbors=int(required(cfg, "evaluation.neighbors")),
        static_caliper_candidates=int(
            required(cfg, "evaluation.static_caliper_candidates")
        ),
        crystalline_fraction_tolerance=float(
            required(cfg, "evaluation.crystalline_fraction_tolerance")
        ),
        pairwise_samples=int(required(cfg, "evaluation.pairwise_samples")),
        exact_mmd_pairs=int(required(cfg, "evaluation.exact_mmd_pairs")),
        bootstrap_samples=int(required(cfg, "evaluation.bootstrap_samples")),
        seed=int(required(cfg, "evaluation.seed")),
        validation_temperatures_K=OmegaConf.select(
            cfg, "evaluation.validation_temperatures_K", default=None
        ),
        validation_phases=OmegaConf.select(
            cfg, "evaluation.validation_phases", default=None
        ),
    )
    ...
    direct_gain = _bootstrap_gain(
        ...
        source_ids.tolist(),
        samples=int(required(cfg, "evaluation.bootstrap_samples")),
        seed=int(required(cfg, "evaluation.seed")) + 211,
    )
    witnesses = select_atlas_witnesses(
        ...
        count=int(required(cfg, "evaluation.witness_count")),
    )
    metrics = {
        "scientific_contract": {
            "expansion": "512 deterministic central atoms per parent",
            "independent_parent_count": len(snapshot.parents),
            "future_branches_per_parent": int(
                np.bincount(np.asarray(cache.branch_parent_index)).min()
            ),
