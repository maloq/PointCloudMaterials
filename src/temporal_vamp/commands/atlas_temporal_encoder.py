...
def run(config_path: str | Path) -> dict[str, Any]:
    ...
    student_root = resolve_path(required(cfg, "input.student_cache_run"))
    teacher_root = resolve_path(required(cfg, "input.teacher_cache_run"))
    history_root = resolve_path(required(cfg, "input.student_history_run"))
    ...
    if not np.array_equal(
        ...
        np.asarray(
            ShootingHistoryEmbeddingCache.load(
                resolve_path(required(cfg, "input.teacher_history_run"))
                / "history_embeddings"
    ...
    target = prepare_joint_path_target_data_from_kernel(
        teacher,
        kernel_path=resolve_path(required(cfg, "input.fixed_path_kernel")),
    ...
    initial_payload = torch.load(
        resolve_path(required(cfg, "initialization.previous_atlas_checkpoint")),
        map_location="cpu",
        weights_only=False,
    )
    ...
    teacher_pca = CovariancePCA.fit(
        ...
        dimension=int(required(cfg, "evaluation.static_pca_dim")),
    )
    spaces["teacher_local_pca_32d"] = teacher_pca.transform(
        teacher_local, dimension=int(required(cfg, "evaluation.static_pca_dim"))
    )
    previous_archive = resolve_path(required(cfg, "input.previous_coordinates"))
    ...
    direct_gain = _bootstrap_gain(
        ...
        seed=int(required(cfg, "evaluation.seed")) + 311,
    )
    ...
    metrics = {
        "scientific_contract": {
            ...
            "fixed_path_kernel": str(resolve_path(required(cfg, "input.fixed_path_kernel"))),
