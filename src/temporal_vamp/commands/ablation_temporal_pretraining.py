...
def main(argv=None) -> None:
    ...
    shooting_targets = prepare_distributional_target_data(
        ...
        selection_source_velocity_seeds=[
            int(value) for value in required(cfg, "split.shooting_selection_velocity_seeds")
        ],
    ...
    entries = discover_simulation_catalog(
        resolve_path(required(cfg, "ordinary.catalog.root")),
        campaign_globs=[str(value) for value in required(cfg, "ordinary.catalog.campaign_globs")],
        cache_root=resolve_path(required(cfg, "ordinary.catalog.cache_root")),
        required_atom_count=int(required(cfg, "ordinary.catalog.required_atom_count")),
        required_potential_parameter_sha256=str(
            required(cfg, "ordinary.catalog.required_potential_parameter_sha256")
        ),
        required_crystal_seed=OmegaConf.select(
            cfg, "ordinary.catalog.required_crystal_seed", default=None
        ),
        require_periodic=bool(required(cfg, "ordinary.catalog.require_periodic")),
    )
    optimization_seeds = [
        int(value) for value in required(cfg, "split.ordinary_optimization_velocity_seeds")
    ]
    selection_seeds = [
        int(value) for value in required(cfg, "split.ordinary_selection_velocity_seeds")
    ]
    excluded_seeds = [
        int(value) for value in required(cfg, "split.ordinary_excluded_velocity_seeds")
    ]
    ...
    expected_run_count = int(required(cfg, "ordinary.expected_included_run_count"))
    ...
    dataset = TemporalBinaryContextDataset(
        ...
        horizons_ps=[float(value) for value in required(cfg, "target.horizons_ps")],
        anchor_stride_frames=int(required(cfg, "ordinary.anchor_stride_frames")),
        ...
        trajectory_cache_size=int(required(cfg, "ordinary.trajectory_cache_size")),
    )
    snapshot = {
    ...
    ordinary_targets = prepare_ordinary_pretraining_targets(
        ...
        optimization_velocity_seeds=optimization_seeds,
        selection_velocity_seeds=selection_seeds,
        pca_dim_per_horizon=int(required(cfg, "pretraining.pca_dim_per_horizon")),
    )
    ...
    if args.stage in {"all", "pretrain"}:
        pretrained = pretrain_spatial_context_backbones(
            ...
            learning_rate=float(required(cfg, "pretraining.learning_rate")),
            weight_decay=float(required(cfg, "pretraining.weight_decay")),
            batch_size=int(required(cfg, "pretraining.batch_size")),
            maximum_epochs=int(required(cfg, "pretraining.maximum_epochs")),
            patience=int(required(cfg, "pretraining.patience")),
    ...
    baseline_metrics_path = resolve_path(required(cfg, "comparison.ablation3_metrics"))
