...
def main(argv=None) -> None:
    ...
    reference_cache = ShootingEmbeddingCache.load(
        resolve_path(required(cfg, "shooting.reference_embedding_cache"))
    )
    ...
    dynamics = ShootingDynamicalFeatureCache.load(
        resolve_path(required(cfg, "shooting.dynamical_feature_cache"))
    )
    ...
    ballistic_result = evaluate_ballistic_rollout(
        ...
        ballistic_pca_dimensions=[
            int(value) for value in required(cfg, "ridge.ballistic_pca_dimensions")
        ],
        ridge_alphas=[float(value) for value in required(cfg, "ridge.alphas")],
        neighbors=int(required(cfg, "evaluation.neighbors")),
        seed=int(required(cfg, "evaluation.seed")),
