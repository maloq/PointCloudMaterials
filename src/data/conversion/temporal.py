#!/usr/bin/env python3
"""Convert strictly complete non-shooting LAMMPS trajectories to verified binaries."""
...
def _write_json_atomic(path: Path, value: dict[str, Any]) -> None:
    ...
    if temporary.exists():
        raise FileExistsError(
            f"Refusing to reuse migration temporary file: {temporary}"
        )
    ...
    with source.open("r", encoding="utf-8") as handle:
        for frame_index in range(prefix_frame_count):
            ...
            if marker != "ITEM: TIMESTEP\n":
                raise RuntimeError(
                    f"{source}: expected timestep header for prefix frame"
                    f" {frame_index}, got {marker.rstrip()!r}."
                )
            timestep = int(handle.readline())
            if handle.readline() != "ITEM: NUMBER OF ATOMS\n":
                raise RuntimeError(
                    f"{source}: missing atom-count header at step {timestep}."
                )
            observed_atoms = int(handle.readline())
            if observed_atoms != atom_count:
                raise RuntimeError(
                    f"{source}: prefix step {timestep} has"
                    f" {observed_atoms} atoms, expected {atom_count}."
                )
            bounds_header = handle.readline()
            if bounds_header != "ITEM: BOX BOUNDS pp pp pp\n":
                raise RuntimeError(
                    f"{source}: unsupported prefix box header at step"
                    f" {timestep}: {bounds_header.rstrip()!r}."
                )
            bounds = np.asarray(
                [
            ...
            if atom_header != "ITEM: ATOMS id type x y z\n":
                raise RuntimeError(
                    f"{source}: unsupported prefix atom columns at step"
                    f" {timestep}: {atom_header.rstrip()!r}."
                )
            table = np.loadtxt(handle, max_rows=atom_count)
            if table.shape != (atom_count, 5):
                raise RuntimeError(
                    f"{source}: prefix atom table at step {timestep} has"
                    f" shape={table.shape}, expected={(atom_count, 5)}."
                )
            ids = table[:, 0].astype(np.int64)
            if not np.array_equal(ids, expected_ids):
                raise RuntimeError(
                    f"{source}: prefix atom IDs are not exactly"
                    f" 1..{atom_count} at step {timestep}."
                )
            ...
            if not np.array_equal(
                ...
                raise RuntimeError(
                    f"{source}: prefix atom types are not the"
                    f" repository-required Al type 1 at step {timestep}."
                )
            prefix[frame_index] = (
...
def _completed_replicas(campaign_root: Path) -> tuple[Path, ...]:
    ...
    if status.get("state") != "complete":
        raise RuntimeError(
            ...
            f" root={campaign_root}, state={status.get('state')!r}."
        )
    replicas = {
        path.parent.resolve()
    ...
    if not replicas:
        raise FileNotFoundError(
            "Complete campaign contains neither text nor migrated trajectory"
            f" artifacts: {campaign_root}"
        )
...
def _load_verified_coordinate_archive(
    ...
    if not isinstance(artifact_hashes, dict):
        raise TypeError(
            "Completed replica analysis has no artifacts_sha256 object:"
            f" {analysis_path}"
        )
    ...
    if (
        ...
        raise ValueError(
            "Completed replica analysis has no valid trajectory.npz SHA-256:"
            f" {analysis_path}"
        )
    ...
    with np.load(archive_path, allow_pickle=False) as archive:
        ...
        if missing:
            raise KeyError(
                f"Coordinate archive {archive_path} is missing arrays"
                f" {missing}."
            )
    ...
    if positions.dtype != np.dtype("float32"):
        raise TypeError(
            "Repository coordinate archive positions must be float32, got "
...
def _validate_archive_against_dump(
    *,
    ...
    if tuple(scan.atom_columns) != ("id", "type", "x", "y", "z"):
        raise ValueError(
            "Non-shooting migration requires repository position columns "
            f"('id','type','x','y','z'), got {scan.atom_columns} in {source}."
        )
    if positions.ndim != 3 or positions.shape[1:] != (scan.num_atoms, 3):
        raise ValueError(
            f"Coordinate archive has invalid position shape={positions.shape};"
            f" expected (frames, {scan.num_atoms}, 3), source={source}."
        )
    ...
    if prefix_frame_count < 0:
        raise ValueError(
            "Coordinate archive has more frames than its text source:"
            f" archive={archive_frame_count}, dump={scan.frame_count},"
            f" source={source}."
        )
    if step.shape != (archive_frame_count,):
        raise ValueError(
            "Coordinate archive step shape disagrees with archive positions:"
            f" step={step.shape}, position_frames={archive_frame_count},"
            f" source={source}."
        )
    ...
    if not np.array_equal(step, relative_dump_steps):
        raise ValueError(
            "Coordinate archive cadence disagrees with text dump timesteps:"
            f" source={source}."
        )
    if cells.shape != (archive_frame_count, 3, 3):
        raise ValueError(
            f"Coordinate archive cells have shape={cells.shape},"
            f" expected={(archive_frame_count, 3, 3)} in"
            f" {source.parent / 'trajectory.npz'}."
        )
    ...
    if not np.array_equal(cells, diagonal_cells):
        raise ValueError(
            f"Migration requires orthogonal repository cells: {source.parent}."
        )
    ...
    if not np.allclose(box_lengths, cell_lengths, rtol=0.0, atol=2.0e-5):
        raise ValueError(
            "Coordinate archive cell lengths disagree with text dump bounds:"
            f" {source}."
        )
    if not np.all(np.isfinite(positions)):
        raise ValueError(
            "Coordinate archive positions contain non-finite values:"
            f" {source.parent}."
        )
    if prefix_frame_count == 0:
    ...
    if complete_positions.shape != (scan.frame_count, scan.num_atoms, 3):
        raise RuntimeError(
            f"Combined trajectory has shape={complete_positions.shape},"
            f" expected={(scan.frame_count, scan.num_atoms, 3)} for {source}."
        )
    complete_lengths = np.asarray(
        scan.box_high - scan.box_low, dtype=np.float32
    )
    for frame_index in range(scan.frame_count):
        frame_lengths = complete_lengths[frame_index]
        wrapped = np.mod(
            complete_positions[frame_index], frame_lengths[None, :]
...
def convert_replica(
    replica_dir: Path, *, delete_source: bool = False
) -> dict[str, Any]:
    source = replica_dir / _TEXT_FILENAME
    target = binary_path_for_dump(source)
    if target.name != _BINARY_DIRNAME:
        raise RuntimeError(
            f"Unexpected canonical binary name for {source}: {target.name}"
        )
    ...
    if target.is_dir():
        ...
        if (
            ...
            and source_record.get("deleted_at") is None
        ):
    ...
    if not source.is_file() or source.stat().st_size <= 0:
        raise FileNotFoundError(
            "Replica has neither a source text dump nor a complete binary:"
            f" {replica_dir}"
        )
    archive_path, archive_sha256, step, positions, cells = (
        _load_verified_coordinate_archive(replica_dir)
    )
    archive_positions_sha256 = _array_sha256(positions)
    scan, complete_positions, positions_sha256 = (
        _validate_archive_against_dump(
            source=source,
            step=step,
            positions=positions,
            cells=cells,
    ...
    archive_record = {
        ...
        "unarchived_prefix_frame_count": int(
            scan.frame_count - positions.shape[0]
        ),
    }
    ...
    binary = write_temporal_lammps_binary(
        ...
        provenance={
            ...
            "coordinate_source": (
                "repository-produced checksum-bound trajectory.npz"
            ),
        },
    )
    checksums = binary.verify_checksums()
    if checksums["positions"] != positions_sha256:
        raise RuntimeError(
            "Stored positions are not identical to the checksum-bound"
            f" coordinate archive: {target}"
        )
    report = _migration_document(
        binary=binary,
        source_record=source_record,
...
def _parse_args(argv=None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Delete each trajectory.lammpstrj only after complete binary"
            " verification."
        ),
    )
...
def main(argv=None) -> None:
    ...
    for raw_root in args.campaign_roots:
        ...
        print(
            "[temporal-migration]"
            f" campaign={campaign_root} replicas={len(replicas)}",
            flush=True,
        )
        ...
        for replica_index, replica_dir in enumerate(replicas):
            print(
                "[temporal-migration]"
                f" replica={replica_index + 1}/{len(replicas)} "
                f"path={replica_dir}",
                flush=True,
            )
            reports.append(
                convert_replica(replica_dir, delete_source=args.delete_source)
            )
        campaign_report = {
            "format": FORMAT_NAME,
            "schema_version": 1,
            ...
            "source_size_bytes": sum(
                int(report["source_lammpstrj"]["size_bytes"])
                for report in reports
            ),
            "binary_size_bytes": sum(
                int(report["binary_size_bytes"]) for report in reports
            ),
            "deleted_source_bytes": sum(
                int(report["source_lammpstrj"]["size_bytes"])
                for report in reports
                if report["source_lammpstrj"]["deleted"]
            ),
            "all_sources_deleted": all(
                bool(report["source_lammpstrj"]["deleted"])
                for report in reports
            ),
        }
        _write_json_atomic(
            campaign_root / "temporal_binary_migration_float32.json",
            campaign_report,
        )
        campaign_reports.append(campaign_report)
    total_source = sum(
        int(report["source_size_bytes"]) for report in campaign_reports
    )
    total_binary = sum(
        int(report["binary_size_bytes"]) for report in campaign_reports
    )
    print(
        json.dumps(
            {
                ...
                "replica_count": sum(
                    int(report["replica_count"]) for report in campaign_reports
                ),
                "source_size_bytes": total_source,
                "binary_size_bytes": total_binary,
                "freed_bytes": (
                    sum(
                        report["deleted_source_bytes"]
