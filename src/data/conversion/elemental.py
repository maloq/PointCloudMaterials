...
def convert(
    ...
    *,
    ...
    if metadata['state'] != 'dynamics_complete':
        raise RuntimeError(
            f'Only validated complete dynamics can be converted: {branch}'
        )
    ...
    if scratch.exists():
        raise FileExistsError(
            f'Interrupted conversion requires inspection: {scratch}'
        )
    ...
    with source.open() as handle:
        for frame in range(frames):
            header = TemporalLAMMPSDumpDataset._read_frame_header(
                handle, source_path=source
            )
            if (
                header is None
                or header['num_atoms'] != atoms
                or header['timestep'] != steps[frame]
            ):
                raise RuntimeError(
                    f'Wrong frame/atom count/timestep at frame {frame}:'
                    f' {source}'
                )
            ...
            if (
                table.shape != (atoms, 5)
                or not np.array_equal(table[:, 0], ids)
                or not np.all(table[:, 1] == 1)
            ):
                raise RuntimeError(
                    f'Changed IDs/types or incomplete frame {frame}: {source}'
                )
            low[frame], high[frame] = header['box_low'], header['box_high']
            lengths = high[frame] - low[frame]
            wrapped = np.mod(
                table[:, 2:5].astype(np.float32) - low[frame], lengths
            )
            wrapped = np.minimum(
                wrapped, np.nextafter(lengths, np.zeros(3, dtype=np.float32))
            )
            ...
            if frame % 100 == 0:
                print(
                    f'{branch.name}: decoded {frame + 1}/{frames} frames',
                    flush=True,
                )
    ...
    binary = write_temporal_lammps_binary(
        branch / f'trajectory_binary_{storage_dtype}',
        positions=positions,
        timesteps=steps,
        box_low=low,
        box_high=high,
        atom_ids=ids,
        atom_types=np.ones(atoms, dtype=np.int32),
        atom_columns=('id', 'type', 'x', 'y', 'z'),
        source={'trajectory_lammpstrj': str(source), 'sha256': source_hash},
        provenance={
            'producer': 'src.simulation.campaigns.elemental',
            'metadata': metadata,
            'position_semantic_sha256': semantic_hash.hexdigest(),
    ...
    if actual_hash.hexdigest() != semantic_hash.hexdigest():
        raise RuntimeError(
            f'Binary coordinates differ from decoded source: {branch}'
        )
    del positions
    report = {
        'state': 'complete',
    ...
    (branch / 'binary_conversion.json').write_text(
    ...
    if delete_source:
        ...
        if (current.st_size, current.st_mtime_ns) != (
            ...
            raise RuntimeError(
                'Source changed during conversion; refusing deletion:'
                f' {source}'
            )
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        action='store_true',
        help=(
            'Remove raw text only after checksum and coordinate verification.'
        ),
    )
    parser.add_argument(
