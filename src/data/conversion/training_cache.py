...
def convert_file(path: Path):
    if not (
        path.name in ('clouds.npy', 'benchmark_clouds.npy')
    ...
    return convert_array(
        ...
        last_dim=3,
        interpretation=(
            'Lossy storage of derived local coordinates; decode to float32 for'
            ' geometry.'
        ),
    )
...
def convert_array(path: Path, *, last_dim: int, interpretation: str):
    ...
    if values.dtype != np.float32 or values.shape[-1] != last_dim:
        raise ValueError(
            f'Expected float32 cache with last dimension {last_dim}: {path},'
    ...
    if temporary.exists():
        raise FileExistsError(
            f'Inspect interrupted conversion before retry: {temporary}'
        )
    ...
    for start in range(0, len(values), step):
        ...
        if (
            ...
            or np.abs(chunk).max() > np.finfo(np.float16).max
        ):
            raise ValueError(
                'Non-finite or float16-overflowing coordinates at row'
                f' {start}: {path}'
            )
    source_hash = sha256(path)
    target = np.lib.format.open_memmap(
        temporary, mode='w+', dtype=np.float16, shape=values.shape
    )
    maximum_error = 0.0
    square_error = 0.0
    for start in range(0, len(values), step):
    ...
    record = dict(
        state='verified_before_replace',
        path=str(path),
        shape=list(values.shape),
        source_dtype='float32',
        storage_dtype='float16',
        source_sha256=source_hash,
        float16_sha256=sha256(temporary),
        max_absolute_error=maximum_error,
        rmse=(square_error / values.size) ** 0.5,
        source_allocated_bytes=stat.st_blocks * 512,
        source_payload_bytes=values.nbytes,
        target_payload_bytes=values.size * 2,
        target_allocated_bytes=temporary.stat().st_blocks * 512,
        interpretation=interpretation,
    )
    del target, values
    if (path.stat().st_size, path.stat().st_mtime_ns) != (
        stat.st_size,
        stat.st_mtime_ns,
    ):
        raise RuntimeError(
            f'Cache changed during conversion; original retained: {path}'
        )
    write_json(record_path, record)
...
def main(argv=None):
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            'JSON with explicit files and optional wait_for_processes guards.'
        ),
    )
    ...
    try:
        if 'wait_for_processes' in plan:
            write_json(
                status,
                dict(
                    state='waiting_for_training_queue',
                    processes=plan['wait_for_processes'],
                ),
            )
            for process in plan['wait_for_processes']:
                if socket.gethostname() != process['host']:
                    raise RuntimeError(
                        f'Process guard belongs to {process["host"]}, not this'
                        ' host'
                    )
        ...
        for index, name in enumerate(plan['files']):
            ...
            if index % 10 == 0:
                print(
                    f'Converted {index + 1}/{len(plan["files"])}: {path}',
                    flush=True,
                )
        write_json(
            status,
        ...
        write_json(
            status,
            dict(state='failed', completed=len(records), error=repr(error)),
        )
        raise
