...
def history_clouds(trajectory, centers, neighbor_ids, anchor, steps):
    ...
    if frames[0] < 0 or frames[-1] >= trajectory.frame_count:
        raise ValueError(
            f"History {frames.tolist()} exceeds trajectory {trajectory.root}."
        )
    ...
    for frame in frames:
        ...
        positions = np.mod(
            trajectory.positions[frame, neighbors].astype(np.float64) - low,
            lengths,
        )
        origin = np.mod(
            trajectory.positions[frame, centers].astype(np.float64) - low,
            lengths,
        )
        ...
        error = max(
            error, float(np.abs(stored.astype(np.float64) - offsets).max())
        )
...
def prepare(cfg):
    ...
    if manifest['protocol'] != 'thermal80':
        raise ValueError(
            "History supervision requires the verified thermal80 cache:"
            f" {source}"
        )
    ...
    for record in manifest['shards']:
        ...
        signature = dict(
            paired_manifest_sha256=sha256(source_dir / 'manifest.json'),
        ...
        if completion.exists():
            ...
            if saved['signature'] != signature:
                raise ValueError(
                    f"History configuration/source changed: {directory}"
                )
            for name, digest in saved['checksums'].items():
                if sha256(directory / name) != digest:
                    raise ValueError(
                        f"Changed history artifact: {directory / name}"
                    )
            records.append(saved)
            continue
        write_json(
            root / 'status.json', dict(state='preparing', shard=record['name'])
        )
        trajectory = TemporalLAMMPSBinaryTrajectory.load(record['path'])
        steps = np.rint(offsets / record['cadence_ps']).astype(np.int64)
        np.testing.assert_allclose(
            steps * record['cadence_ps'], offsets, rtol=0, atol=1e-9
        )
        if steps[-1] != 0 or np.any(np.diff(steps) <= 0):
            raise ValueError(
                f"History steps must increase to the anchor: {steps}"
            )
        ...
        history = np.lib.format.open_memmap(
            directory / 'histories.npy',
            mode='w+',
            dtype=np.float16,
            shape=(len(ids), 4, len(steps), 80, 3),
        )
        error = 0.0
        for view in range(4):
        ...
        history.flush()
        np.save(
            directory / 'targets.npy', np.load(source_dir / 'tda.npy')[:, :4]
        )
        ...
        positions = np.stack(
            [
                trajectory.positions[f, i].astype(np.float32)
                for f, i in zip(frames[:, 0], ids[:, 0])
            ]
        )
        ...
        saved = dict(
            ...
            count=len(ids),
            signature=signature,
            trajectory_manifest_sha256=sha256(
                Path(record['path']) / 'manifest.json'
            ),
            cadence_ps=record['cadence_ps'],
            frame_min=int(frames.min() + steps[0]),
            frame_max=int(frames.max()),
            local_quantization_max_A=error,
            anchor_matches_paired_cache_exactly=True,
    ...
    for train in (r for r in records if r['split'] == 'train'):
        for val in (
            r
            for r in records
            if r['split'] == 'val' and r['source'] == train['source']
        ):
            if train['frame_max'] >= val['frame_min']:
                raise ValueError(
                    f"Shared-source history windows overlap: {train['name']},"
                    f" {val['name']}"
                )
    write_json(
        root / 'manifest.json',
        dict(
            protocol='temporal80',
            shards=records,
            paired_manifest=str(source),
            paired_manifest_sha256=sha256(source),
            frame_offsets_ps=offsets.tolist(),
            training_views=[0],
            analysis_views=[0, 1, 2, 3],
        ),
    )
    write_json(
        root / 'status.json',
        dict(
            state='complete',
            shards=len(records),
            anchors=sum(r['count'] for r in records),
        ),
    )
...
class Histories:
    ...
    def __init__(self, cfg, scaling=None, device='cuda'):
        ...
        self.raw_targets = np.concatenate(
            [
                np.load(Path(r['directory']) / 'targets.npy')
                for r in self.records
            ]
        )
        self.materials = np.concatenate(
            [
                np.full(r['count'], r['material'], np.int64)
                for r in self.records
            ]
        )
        ...
        self.clouds = torch.from_numpy(
            np.concatenate(
                [
                    np.load(Path(r['directory']) / 'histories.npy')
                    for r in self.records
                ]
            )
        ).to(device)
    ...
    def set_scaling(self, scaling):
        targets = (
            (self.raw_targets - scaling['tda_mean'])
            @ scaling['tda_components'].T
        ) / scaling['tda_std']
        self.targets = torch.from_numpy(targets.astype(np.float32)).to(
            self.clouds.device
        )

    def get(self, indices, view=0):
        rows = torch.as_tensor(
            indices, dtype=torch.long, device=self.clouds.device
        )
        return (
            self.clouds[rows, view].float(),
            self.targets[rows, view],
            self.material[rows],
        )
