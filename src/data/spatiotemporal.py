...
class SpatiotemporalViewDataset(Dataset):
    def __init__(
        ...
        root: Path,
        split: str,
        temporal_lag_steps: int | None = None,
        tda_cache: Path | None = None,
    ):
        ...
        self.shards = [
            np.load(root / s['views'], mmap_mode='r') for s in shards
        ]
        self.rows = [
            (
                np.arange(len(x))
                if temporal_lag_steps is None
                else np.flatnonzero(
                    np.load(root / s['pairs'], mmap_mode='r')[:, 3]
                    == temporal_lag_steps
                )
            )
        ...
        if tda_cache is not None:
            ...
            if (
                targets['state'] != 'complete'
                or resolve_path(protocol['source_root']).resolve()
                != root.resolve()
                or protocol['temporal_lag_steps'] != temporal_lag_steps
            ):
                raise ValueError(
                    'TDA cache does not match the selected view producer:'
                    f' {tda_cache}'
                )
            ...
            for shard, rows in zip(shards, self.rows):
                ...
                np.testing.assert_array_equal(
                    rows, np.load(tda_cache / target['rows'])
                )
                self.tda.append(
                    np.load(tda_cache / target['targets'], mmap_mode='r')
                )
    ...
    def __getitem__(self, index):
        ...
        views = torch.from_numpy(
            self.shards[shard][self.rows[shard][offset]].astype(np.float32)
        )
        result = dict(
            points=views[0], spatial_points=views[1], temporal_points=views[2]
        )
        if self.tda is not None:
            result['tda_targets'] = torch.from_numpy(
                self.tda[shard][offset].copy()
            )
        return result
...
class SpatiotemporalViewDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        tda_path = (
            OmegaConf.select(self.cfg, 'tda.cache_dir')
            if OmegaConf.select(self.cfg, 'tda.enabled', default=False)
            else None
        )
        tda_cache = Path(tda_path) if tda_path is not None else None
        self.train_dataset = SpatiotemporalViewDataset(
            root, "train", lag, tda_cache
        )
        self.val_dataset = SpatiotemporalViewDataset(
            root, "val", lag, tda_cache
        )
    ...
    def val_dataloader(self):
        batch_size = getattr(
            self.cfg, "spatiotemporal_validation_batch_size", self.batch_size
        )
        return self._loader(
            self.val_dataset,
            shuffle=False,
...
def prepare_branch(task):
    ...
    if not np.array_equal(
        trajectory.timesteps, np.arange(source["frame_count"]) * expected_step
    ):
    ...
    for selection in source["splits"]:
        ...
        pairs = open_memmap(
            output / pairs_file, mode="w+", dtype=np.int64, shape=(count, 4)
        )
        ...
        for j, anchor in enumerate(anchors):
            ...
            candidates = nearest[nearest != selected[:, None]].reshape(
                centers, 8
            )
            spatial = candidates[
                np.arange(centers), rng.integers(0, 8, size=centers)
            ]
            batch = slice(j * centers, (j + 1) * centers)
            views[batch, 0] = local_views(
                points,
                tree,
                lengths,
                selected,
            ...
            lags = np.resize(
                np.asarray(source["lags"], dtype=np.int64), centers
            )
            rng.shuffle(lags)
            for lag in source["lags"]:
                frame = anchor + lag
                lengths_t = (
                    trajectory.box_high[frame] - trajectory.box_low[frame]
                )
                points_t, tree_t = periodic_tree(
                    trajectory.positions[frame], lengths_t
                )
                mask = lags == lag
            ...
            pairs[batch] = np.column_stack(
                (
                    trajectory.atom_ids[selected],
                    trajectory.atom_ids[spatial],
                    np.full(centers, anchor),
                    lags,
                )
            )
        views.flush()
...
def prepare_expanded(cfg):
    ...
    protocol = dict(
        config=cfg, base_manifest_sha256=sha256(base / 'manifest.json')
    )
    if (output / 'manifest.json').exists():
        ...
        for name, digest in saved['checksums'].items():
            if sha256(output / name) != digest:
                raise ValueError(
                    f'Expanded view checksum changed: {output / name}'
                )
    ...
    with ProcessPoolExecutor(max_workers=cfg['workers']) as pool:
        ...
        for index, source in enumerate(cfg['sources']):
            ...
            if part.exists():
                ...
                if saved['source'] != source:
                    raise ValueError(
                        f'Prepared source configuration changed: {part}'
                    )
                for name, digest in saved['checksums'].items():
                    if sha256(output / name) != digest:
                        raise ValueError(
                            f'Prepared view checksum changed: {output / name}'
                        )
                shards.extend(saved['shards'])
                checksums.update(saved['checksums'])
            else:
                future = pool.submit(
                    prepare_branch, (source, output, index, cfg['seed'])
                )
        ...
        for future in as_completed(pending):
            ...
            digests = {
                s[k]: sha256(output / s[k])
                for s in prepared
                for k in ('views', 'pairs')
            }
            write_json(
                part, dict(source=source, shards=prepared, checksums=digests)
            )
            shards.extend(prepared)
            checksums.update(digests)
            write_json(
                output / 'status.json',
                dict(state='preparing', completed_shards=len(shards)),
            )
    counts = {}
    for split in ('train', 'val'):
        counts[split] = {
            material: sum(
                int(
                    np.count_nonzero(
                        np.load(output / s['pairs'], mmap_mode='r')[:, 3] == 1
                    )
                )
                for s in shards
                if s['split'] == split and s['material'] == material
            )
    ...
    report = dict(
        state='complete',
        protocol=protocol,
        sources=original['sources'] + cfg['sources'],
        shards=shards,
        checksums=checksums,
        selected_lag_samples=counts,
        split_note=cfg['split_note'],
        cutoff_estimation=cfg['cutoff_estimation'],
        duplicate_trajectories_excluded=cfg['duplicate_trajectories_excluded'],
        num_points=80,
        views=original['views'],
        pair_columns=original['pair_columns'],
        temporal_lags_ps=[0.1],
        cache_storage_dtype='float16',
    )
    write_json(output / 'manifest.json', report)
    write_json(
        output / 'status.json',
        dict(state='complete', selected_lag_samples=counts),
    )
