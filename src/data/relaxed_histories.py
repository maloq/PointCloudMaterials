...
def prepare(cfg):
    ...
    if source['protocol'] != 'denoising80':
        raise ValueError(
            f'Expected prepared denoising80 MEAM targets: {source_path}'
        )
    protocol = dict(
        source_manifest=str(source_path.resolve()),
        source_sha256=sha256(source_path),
        radius_A=float(cfg.data.normalization_radius_A),
        frame_offsets_ps=list(cfg.data.frame_offsets_ps),
        temporal_lag_steps=int(cfg.data.temporal_lag_steps),
        neighbor_k=int(cfg.vicreg_neighbor_k),
        seed=int(cfg.data.preparation_seed),
        tda_components=32,
        block_scale_floor_fraction=float(cfg.tda.block_scale_floor_fraction),
    ...
    if (root / 'manifest.json').exists():
        ...
        if saved['protocol'] != protocol:
            raise ValueError(
                f'Prepared history protocol changed: {root}; choose a new'
                ' cache.'
            )
        for name, digest in saved['checksums'].items():
            if sha256(root / name) != digest:
                raise ValueError(
                    f'Changed prepared history artifact: {root / name}'
                )
    ...
    for context, original in enumerate(source['shards']):
        ...
        for name, digest in original['checksums'].items():
            if sha256(origin / name) != digest:
                raise ValueError(
                    f'Changed relaxed-target input: {origin / name}'
                )
        ...
        steps = np.rint(
            np.array(cfg.data.frame_offsets_ps) / source_cfg['cadence_ps']
        ).astype(np.int64)
        np.testing.assert_allclose(
            steps * source_cfg['cadence_ps'],
            cfg.data.frame_offsets_ps,
            rtol=0,
            atol=1e-9,
        )
        ...
        np.testing.assert_array_equal(
            anchor_ids[:, 0], trajectory.atom_ids[centers]
        )
        rng = np.random.default_rng(
            np.random.SeedSequence([cfg.data.preparation_seed, context])
        )

        low = trajectory.box_low[frame].astype(np.float64)
        lengths = trajectory.box_high[frame].astype(np.float64) - low
        points = np.mod(
            trajectory.positions[frame].astype(np.float64) - low, lengths
        )
        tree = cKDTree(points, boxsize=lengths)
        candidates = tree.query(
            points[centers], k=cfg.vicreg_neighbor_k + 1, workers=1
        )[1][:, 1:]
        spatial_centers = candidates[
            np.arange(len(centers)),
            rng.integers(0, cfg.vicreg_neighbor_k, len(centers)),
        ]
        spatial_neighbors = tree.query(
            points[spatial_centers], k=80, workers=1
        )[1]
        spatial, spatial_error = history_clouds(
            trajectory,
            spatial_centers,
            trajectory.atom_ids[spatial_neighbors],
            frame,
            steps,
        )
        ...
        points = np.mod(
            trajectory.positions[temporal_frame].astype(np.float64) - low,
            lengths,
        )
        temporal_neighbors = cKDTree(points, boxsize=lengths).query(
            points[centers], k=80, workers=1
        )[1]
        temporal, temporal_error = history_clouds(
            trajectory,
            centers,
            trajectory.atom_ids[temporal_neighbors],
            temporal_frame,
            steps,
        )
        ...
        records.append(
            dict(
                ...
                additional_quantization_max_A=max(
                    spatial_error, temporal_error
                ),
            )
        )
        write_json(
            ...
            dict(
                ...
                total=len(source['shards']),
            ),
        )
        print(
            'VICREG_HISTORY'
            f' {len(records)}/{len(source["shards"])} {original["name"]}',
            flush=True,
        )

    targets = np.concatenate(raw_targets)
    train = np.concatenate(
        [np.full(r['samples'], r['split'] == 'train') for r in records]
    )
    scaling = fit_targets(
        targets[train], 32, cfg.tda.block_scale_floor_fraction
    )
    ...
    write_json(
        ...
        dict(
            ...
            input_definition=(
                'Physical float16 offsets; loader divides by the declared'
                ' source radius. Each view has its own complete 80-atom'
                ' neighborhood; atom IDs are fixed within each history.'
            ),
            target_definition=(
                'Original relaxed target for the anchor view only. Spatial and'
                ' earlier views have no assigned relaxed target and'
                ' participate only in the original VICReg pairs.'
            ),
            scaling_fit=(
                'All training anchors only; no validation or test rows.'
            ),
        ),
    )
    write_json(
        root / 'status.json',
        dict(state='complete', contexts=len(records), examples=len(targets)),
    )
...
class RelaxedHistoryDataset(Dataset):
    def __init__(self, cfg, split):
        ...
        self.contexts = np.concatenate(
            [
                np.full(r['samples'], i, dtype=np.int64)
        ...
        self.indices = np.flatnonzero(
            np.isin(
                self.contexts,
                [i for i, r in enumerate(self.records) if r['split'] == split],
            )
        )
        self.starts = np.cumsum([0] + [r['samples'] for r in self.records])
        self.views = {
            i: np.load(root / r['views'], mmap_mode='r')
            for i, r in enumerate(self.records)
            if r['split'] == split
        }
        self.scaling = dict(np.load(root / 'scaling.npz'))
        self.raw_targets = np.load(root / 'targets.npy')
        self.targets = transform_target(
            self.raw_targets, self.scaling, cfg.tda.target
        ).astype(np.float32)
        ...
        if self.input_mode not in ('anchor', 'history'):
            raise ValueError(
                f'Unknown prepared-history input mode: {self.input_mode}'
            )
    ...
    def __getitem__(self, index):
        ...
        views = (
            self.views[context][row - self.starts[context]].astype(np.float32)
            / self.radius
        )
        if self.input_mode == 'anchor':
            views = views[:, -1]
        return dict(
            points=torch.from_numpy(views[0]),
            spatial_points=torch.from_numpy(views[1]),
            temporal_points=torch.from_numpy(views[2]),
            tda_targets=torch.from_numpy(self.targets[row]),
            row=row,
            context=context,
