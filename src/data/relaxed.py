...
def prepare_shard(record, cfg, pool):
    ...
    if (directory / 'manifest.json').exists():
        ...
        for name, digest in saved['checksums'].items():
            if sha256(directory / name) != digest:
                raise ValueError(f'Changed paired cache: {directory / name}')
        return saved
    source = Path(record['directory'])
    all_frames = np.load(source / 'frames.npy')
    selected = np.flatnonzero(
        np.isin(
            all_frames[:, 0],
            record['anchors'][: cfg['relaxation']['anchor_frames_per_source']],
        )
    )
    ids = np.load(source / 'ids.npy')[selected]
    ...
    count = len(ids)
    mode = 'r+' if (directory / 'clouds.npy').exists() else 'w+'
    clouds = np.lib.format.open_memmap(
    ...
    identities = np.lib.format.open_memmap(
        directory / 'neighbor_ids.npy',
        mode=mode,
        dtype=np.int32,
        shape=(count, 4, 80),
    )
    trajectory = TemporalLAMMPSBinaryTrajectory.load(record['path'])
    settings = cfg['relaxation']
    material = ('Al', 'Mg', 'Ta')[record['material']]
    physics = {**settings['controls'], **settings['materials'][material]}
    frame_reports = []
    started = time.monotonic()
    for frame in np.unique(frames):
        frame = int(frame)
        work = Path(settings['output']) / record['name'] / str(frame)
        completion = work / 'paired_complete.json'
        if completion.exists():
            frame_reports.append(json.loads(completion.read_text()))
            continue
        write_json(
            root / 'status.json',
            dict(
                state='relaxing_full_cell', shard=record['name'], frame=frame
            ),
        )
        if not (work / 'metadata.json').exists():
            relax_frame(trajectory, frame, work, physics)
        relaxed, metadata = read_relaxed(work)
        low = trajectory.box_low[frame].astype(float)
        lengths = trajectory.box_high[frame].astype(float) - low
        hot = np.mod(trajectory.positions[frame].astype(float) - low, lengths)
        row, view = np.nonzero(frames == frame)
        centers, inverse = np.unique(ids[row, view], return_inverse=True)
        a, b, neighbors, errors = paired_clouds(
        ...
        clouds[row, view] = a[inverse]
        clouds[row, view + 4] = b[inverse]
        targets[row, view] = tda[inverse]
        targets[row, view + 4] = tda[inverse]
        ...
        report = dict(
            ...
            local_offset_quantization_max_A=errors,
            mean_hot_relaxed_patch_displacement_A=float(
                np.linalg.norm(
                    a.astype(float) - b.astype(float), axis=-1
                ).mean()
            ),
        )
        write_json(completion, report)
    ...
    saved = dict(
        ...
        anchors_count=count,
        anchors=np.unique(frames[:, 0]).tolist(),
        ...
        checksums={name: sha256(directory / name) for name in files},
        ...
        target=(
            'Both states predict TDA of the relaxed version of their identical'
            ' 80 atom identities; computed from centered float16 offsets.'
        ),
    )
...
def prepare(cfg):
    ...
    if len(records) != len(cfg['relaxation']['shards']):
        raise ValueError('Configured relaxed source shards are missing')
    try:
        ...
        write_json(
            root / 'manifest.json',
            dict(
                shards=result, config=cfg, tda_points=80, protocol='thermal80'
            ),
        )
        write_json(
            root / 'status.json',
            dict(
                state='complete',
                shards=len(result),
                anchors=sum(r['anchors_count'] for r in result),
            ),
        )
