...
def prepare(cfg):
    ...
    protocol = dict(
        ...
        source_sha256=sha256(root / 'manifest.json'),
        temporal_lag_steps=cfg.data.temporal_lag_steps,
        points=80,
        reference_radius_A=cfg.encoder.kwargs.reference_radius_A,
        components=cfg.tda.components,
        fit_anchors=cfg.tda.fit_anchors,
        seed=cfg.seed_everything,
    )
    if (out / 'manifest.json').exists():
        saved = json.loads((out / 'manifest.json').read_text())
        saved_protocol = dict(saved['protocol'])
        saved_protocol['source_root'] = str(
            resolve_path(saved_protocol['source_root']).resolve()
        )
        if saved_protocol != protocol:
            raise ValueError(
                f'TDA cache protocol differs: {out}/manifest.json'
            )
    ...
    with ProcessPoolExecutor(max_workers=cfg.tda.workers) as pool:
        for record in source['shards']:
            ...
            rows = np.flatnonzero(
                np.load(pairs_path, mmap_mode='r')[:, 3]
                == cfg.data.temporal_lag_steps
            )
            clouds = np.load(views_path, mmap_mode='r')[rows].astype(
                np.float32
            )
            if clouds.shape[1:] != (3, 80, 3):
                raise ValueError(
                    f'Expected three 80-point views in {views_path}, got'
                    f' {clouds.shape}'
                )
            ...
            provenance = dict(
                source_views=record['views'],
                views_sha256=sha256(views_path),
                pairs_sha256=sha256(pairs_path),
                protocol=protocol,
            )
            if part_manifest.exists():
                part = json.loads(part_manifest.read_text())
                if part['provenance'] != provenance or part[
                    'sha256'
                ] != sha256(raw_path):
                    raise ValueError(
                        f'Invalid completed TDA shard: {part_manifest}'
                    )
            else:
                ...
                write_json(
                    part_manifest,
                    dict(provenance=provenance, sha256=sha256(raw_path)),
                )
            ...
            shards.append(
                dict(
                    source_views=record['views'],
                    split=record['split'],
                    rows=f'{prefix}.rows.npy',
                    images=raw_path.name,
                    targets=f'{prefix}.targets.npy',
                    samples=len(rows),
                )
            )
            write_json(
    ...
    selected = np.random.default_rng(cfg.seed_everything + 100).choice(
        int(ends[-1]), cfg.tda.fit_anchors, replace=False
    )
    fit = []
    for index, shard in enumerate(train):
        start = int(ends[index - 1]) if index else 0
        local = (
            selected[(selected >= start) & (selected < ends[index])] - start
        )
    ...
    scale = np.maximum(np.sqrt(pca.explained_variance_), 1e-5).astype(
        np.float32
    )
    np.savez(
        out / 'scaling.npz',
    ...
    for shard in shards:
        ...
        target = ((raw - pca.mean_) @ pca.components_.T / scale).astype(
            np.float32
        )
        if not np.isfinite(target).all():
            raise FloatingPointError(
                f'Nonfinite transformed TDA targets: {shard["source_views"]}'
            )
        np.save(out / shard['targets'], target)
        for key in ('targets', 'rows', 'images'):
            checksums[shard[key]] = sha256(out / shard[key])
        split = shard['split']
        total, count = baseline.get(split, (0.0, 0))
        baseline[split] = (
            total + float(np.square(target, dtype=np.float64).sum()),
            count + target.size,
        )
    write_json(
        out / 'manifest.json',
        dict(
            ...
            constant_training_mean_mse={
                k: v[0] / v[1] for k, v in baseline.items()
            },
            target_definition=(
                'Full 80-point alpha-complex H0/H1/H2 images in shared'
                ' normalized reference units; training-only PCA32 whitening;'
                ' unaugmented views.'
            ),
        ),
    )
    write_json(
        ...
        dict(
            state='complete',
            shards=len(shards),
            target_views=sum(3 * s['samples'] for s in shards),
            seconds=time.monotonic() - started,
        ),
    )
...
def main():
    ...
    source.add_argument(
        '--config', help='Resolved training YAML with data and tda settings.'
    )
    source.add_argument(
        '--config-name',
        help=(
            'Compose an existing Hydra training configuration under configs/.'
        ),
    )
