...
def export(config):
    ...
    if saved['manifest']['protocol'] != 'neighborhood_jepa_regularization_order_v3':
    ...
    write_json(output/'provenance.json', dict(source=str(source), source_sha256=sha256(source),
        ...
        representation='Native invariant128, including trained per-observation normalization; no predictor.'))
...
def verify(config):
    ...
    batch = snapshot_batch(points, tree, points[ids], scale=config['scales']['Al'], material='Al', architecture='mace')
    ...
    for atom in ids:
        ...
        keep=np.linalg.norm(x,axis=1)<8; rows=rows[keep]; x=x[keep]
    ...
    reverse=snapshot_batch(points,tree,points[ids[::-1]],scale=config['scales']['Al'],material='Al',architecture='mace')
    replay=model.encode({k:v.cuda() for k,v in reverse.items()}).flip(0)
    torch.testing.assert_close(actual,replay,rtol=2e-5,atol=2e-6)
    singleton=[]
    for atom in ids:
        one=snapshot_batch(points,tree,points[[atom]],scale=config['scales']['Al'],material='Al',architecture='mace')
    ...
    write_json(output/'static-verification.json',dict(state='complete',step=saved['step'],input_tensors_exact=len(batch),execution="eager",singleton_max_error=float((actual-singleton).abs().max()),native_eager_max_error=float((actual-reference).abs().max()),reorder_max_error=float((actual-replay).abs().max()),representation='invariant128'))
