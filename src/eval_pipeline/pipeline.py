...
from .predictors import PREDICTOR_REGISTRY, Predictor
from .metrics.silhouette import SilhouetteMetric
from .metrics.ari import ARIMetric
from .metrics.rotational_consistency import RotationalConsistencyMetric
from .metrics.base import SingleRunMetric, MultiRunMetric
from .config_merger import load_training_config, merge_and_save
...
def _build_dataloader(cfg: DictConfig, model_type: str):
    ...
    if model_type == "autoencoder":
        ...
        return create_autoencoder_dataloader(cfg, file_paths, shuffle=False)
    if model_type == "spd":
        from src.training_methods.spd.eval_spd import create_spd_dataloader

        return create_spd_dataloader(cfg, file_paths, shuffle=False)
    if model_type == "soap":
        from src.training_methods.autoencoder.eval_autoencoder import (
            create_autoencoder_dataloader,
        )
...
def run(eval_config_path: str) -> Dict[str, Any]:
    ...
    dataloader = _build_dataloader(train_cfg, eval_cfg.prediction.model_type)
