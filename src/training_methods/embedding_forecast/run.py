...
def datasets(config, manifest):
    return {split: WindowDataset(config['data']['cache'], manifest, split,
        config['history_ps'], config['horizons_ps'][-1], config['stride_ps'], config['anchor_history_ps'])
...
def evaluate_checkpoint(directory, device):
    ...
    model = EmbeddingForecaster(manifest['embedding_dim'], data['train'].history_steps,
    ...
    loader = window_loader(data['test'], settings['batch_size'], settings['workers'], False, payload['seed'])
...
def train(config, variant, seed, device, *, resume=False, epochs_per_invocation=None):
    ...
    if resume:
        ...
        if saved != dict(config=config, variant=variant, seed=seed):
    ...
    train_loader = window_loader(data['train'], settings['batch_size'], settings['workers'], True, seed)
    val_loader = window_loader(data['val'], settings['batch_size'], settings['workers'], False, seed)
    ...
    criterion = 'nll' if model.distribution == 'low_rank_gaussian' else 'mse'
    ...
    implementation = {name: sha256(Path(__file__).parent / name)
                      for name in ('data.py', 'model.py', 'metrics.py', 'run.py', 'augmentation.py')}
    if checkpoint is not None:
        if checkpoint['implementation_sha256'] != implementation:
            raise ValueError(f'Resume implementation changed; use the saved execution source: {directory}')
        ...
        for variant in variants:
            for seed in seeds:
                if args.stage == 'evaluate':
                    evaluate_checkpoint(Path(config['output']) / f"{variant['name']}-seed{seed}", device)
                else:
                    train(config, variant, seed, device, resume=args.resume,
                          epochs_per_invocation=args.epochs_per_invocation)
