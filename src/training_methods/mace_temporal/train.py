...
class TemporalLearner(nn.Module):
    def __init__(self, cfg):
        ...
        self.tda = nn.Sequential(
            nn.Linear(self.encoder.invariant_dim, 256),
            nn.SiLU(),
...
def encode(model, x, material, microbatch):
    return torch.cat(
        [
            model.encoder(
                x[start : start + microbatch],
                material[start : start + microbatch],
            )
...
def cached_step(model, batch, cfg):
    ...
    if not torch.isfinite(loss):
        raise FloatingPointError(
            f"Nonfinite temporal objective for history batch {tuple(x.shape)}"
        )
    loss.backward()
    for start in range(0, len(x), size):
        actual = model.encoder(
            x[start : start + size], material[start : start + size]
        )
...
def fit_scaling(model, data, cfg, out):
    ...
    rows = np.random.default_rng(cfg['seed']).choice(
        data.indices['train'], cfg['scaling_anchors'], replace=False
    )
    features = []
    with torch.no_grad():
        for start in range(0, len(rows), cfg['microbatch_size']):
            selected = torch.as_tensor(
                rows[start : start + cfg['microbatch_size']], device='cuda'
            )
            x = data.clouds[selected, 0].float()
            m = data.material[selected].repeat_interleave(x.shape[1])
            features.append(
                model.encoder.mace.raw_features(x.flatten(0, 1), m)
                .cpu()
                .numpy()
            )
    ...
    for name in ('feature_mean', 'feature_std'):
        getattr(model.encoder.mace, name).copy_(
            torch.from_numpy(scaling[name])
        )
    ...
    write_json(
        ...
        dict(
            fit_split='train',
            tda_fit_anchors=len(targets),
            feature_fit_histories=len(rows),
...
def validate(model, data, cfg):
    ...
    result['by_material'] = {
        name: dict(
            tda_mse=float(
                (prediction[material == i] - target[material == i])
                .square()
                .mean()
            )
        )
...
def preflight(model, data, cfg, out):
    rows = np.concatenate(
        [
            data.indices['train'][data.materials[data.indices['train']] == m][
                :2
            ]
            for m in range(3)
        ]
    ...
    for name, parameter in model.named_parameters():
        if parameter.grad is not None:
            torch.testing.assert_close(
                parameter.grad, expected[name], rtol=3e-3, atol=1e-4, msg=name
            )
            error = max(
                error, float((parameter.grad - expected[name]).abs().max())
            )
    model.zero_grad(set_to_none=True)
    model.eval()
    ...
    if not torch.isfinite(by_frame).all() or not (by_frame > 0).all():
        raise AssertionError(
            f"History frame gradients missing/nonfinite: {by_frame}"
        )
    model.zero_grad(set_to_none=True)
    write_json(
        out / 'preflight.json',
        dict(
            state='passed',
            replay_gradient_max_error=error,
            coordinate_gradient_l1_by_frame=by_frame.tolist(),
            optimizer_updates_retained=0,
        ),
    )
...
def train(model, data, cfg, out, wandb_run):
    if (out / 'initial.pt').exists():
        raise FileExistsError(
            f"Training would overwrite {out}; choose a new experiment output."
        )
    ...
    torch.save(
        dict(model=model.state_dict(), config=cfg, validation=initial),
        out / 'initial.pt',
    )
    write_json(out / 'initial_validation.json', initial)
    wandb_run.log(
        {'training_step': 0, **flatten_metrics('validation', initial)}
    )
    best = float('inf')
    best_epoch = None
    step = seen = 0
        ...
        for epoch in range(1, cfg['epochs'] + 1):
            ...
            for start in range(0, len(order), cfg['batch_size']):
                ...
                if time.monotonic() - started > cfg['max_training_seconds']:
                    ...
                    raise TimeoutError(
                        'Temporal training exceeded its configured budget;'
                        ' partial checkpoint retained.'
                    )
            validation = validate(model, data, cfg)
            record = dict(
                epoch=epoch,
                step=step,
                train={k: v / len(rows) for k, v in totals.items()},
    ...
    write_json(
        ...
        dict(
            ...
            frame_exposures=seen * len(cfg['encoder']['frame_offsets_ps']),
            seconds=time.monotonic() - started,
            best_epoch=best_epoch,
            best_tda_mse=best,
            selection='minimum held-out relaxed-anchor TDA MSE',
...
def run(cfg, stage):
    if stage in ('analysis', 'all'):
        raise ValueError(
            'The standalone temporal analysis has been retired. Use original'
    ...
    if stage in ('train', 'all') and (out / 'initial.pt').exists():
        raise FileExistsError(
            f"Training artifacts already exist in {out}; choose a fresh"
            " output."
        )
    if cfg['encoder']['dropout'] != 0:
        raise ValueError(
            'Exact temporal gradient replay requires transformer dropout=0.'
        )
    ...
    if seconds <= 0:
        raise TimeoutError(
            f"Allocation safety deadline has passed: {cfg['deadline']}"
        )
    ...
    try:
        with tracked_run(
            ...
            if stage in ('prepare', 'all'):
                write_json(
                    out / 'status.json', dict(state='preparing_histories')
                )
                prepare(cfg)
            if stage in ('train', 'preflight', 'all'):
                write_json(
                    out / 'status.json', dict(state='scaling_and_preflight')
                )
