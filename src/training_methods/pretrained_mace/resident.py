...
class GPUQuadruplets:
    ...
    def __init__(self, data, scaling):
        self.offsets = np.r_[
            0, np.cumsum([r['anchors_count'] for r in data.records])
        ]
        count = int(self.offsets[-1])
        views, points = data.clouds[0].shape[1:3]
        self.clouds = torch.empty(
            (count, views, points, 3), dtype=torch.float16, device='cuda'
        )
        self.targets = torch.empty(
            (count, views, len(scaling['tda_std'])),
            dtype=torch.float32,
            device='cuda',
        )
        self.conditions = torch.empty(
            (count, 5), dtype=torch.float32, device='cuda'
        )
        self.materials = torch.empty(count, dtype=torch.long, device='cuda')
        for i, record in enumerate(data.records):
            sl = slice(self.offsets[i], self.offsets[i + 1])
            self.clouds[sl].copy_(
                torch.from_numpy(np.array(data.clouds[i], copy=True))
            )
            target = (
                (data.tda[i] - scaling['tda_mean'])
    ...
    def get(self, indices):
        rows = torch.from_numpy(
            self.offsets[indices[:, 0]] + indices[:, 1]
        ).to('cuda')
        return (
            self.clouds[rows].float(),
            self.targets[rows],
