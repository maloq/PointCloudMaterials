...
def prepare_existing(cfg):
    ...
    producer = {
        str(p): sha256(p)
        for p in (Path(__file__), Path('src/analysis/liquid_structure.py'))
    }
    filenames = (
        'histories.npy',
        'relaxed.npy',
    ...
    with ProcessPoolExecutor(max_workers=cfg['workers']) as pool:
        for item in cfg['reuse_records']:
            if (
                item['source_index'] in assignments
                and assignments[item['source_index']] != item['split']
            ):
                raise ValueError(
                    "Source crosses training/evaluation splits:"
                    f" {item['name']}"
                )
            ...
            provenance = dict(
                selection=item,
                input_manifest_sha256=sha256(item['manifest']),
                producer=producer,
                original_split=original['split'],
            )
            if item['format'] == 'temporal80':
                paired = verified_manifest(item['paired_manifest'])
                provenance['paired_manifest_sha256'] = sha256(
                    item['paired_manifest']
                )
                provenance['offsets'] = original['signature'][
                    'frame_offsets_ps'
                ]
                ...
                raise ValueError(
                    f"Unsupported owned cache format: {item['format']}"
                )
            ...
            write_json(
                ...
                dict(
                    state='reusing_completed_targets',
                    completed_shards=len(records),
                    total_shards=len(cfg['reuse_records']),
                    shard=item['name'],
                ),
            )
            if item['format'] == 'denoising80':
                ...
                histories = np.load(source / 'histories.npy', mmap_mode='r')[
                    :, 0
                ]
                ...
                np.testing.assert_array_equal(
                    targets,
                    np.load(paired_dir / 'tda.npy', mmap_mode='r')[:, 0],
                )
                frames = np.load(source / 'frames.npy')[:, 0]
                np.testing.assert_array_equal(
                    frames, np.tile(frames[0], (len(frames), 1))
                )
                hot_targets = np.stack(
                    list(
                        pool.map(
                            persistence_image,
                            histories[:, -1].astype(np.float32),
                            chunksize=32,
                        )
                    )
                )
                arrays = dict(
                    ...
                    centers=np.load(source / 'ids.npy')[:, 0],
                    neighbor_ids=np.load(source / 'neighbor_ids.npy')[:, 0],
                    frames=frames[0],
                )
            ...
            saved = dict(
                ...
                checksums={
                    name: sha256(directory / name) for name in filenames
                },
            )
    ...
    write_json(
        root / 'manifest.json',
        dict(
            protocol='denoising80_reuse',
            shards=records,
            context=cfg['data_context'],
            limitations=cfg['data_limitations'],
            target=(
                'Existing converged full-cell targets; original hot-selected'
                ' atom identities; no target averaging.'
            ),
            sampling=(
                'Frozen list of available artifacts; whole-source split'
