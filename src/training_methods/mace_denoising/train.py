...
from src.training_methods.mace_denoising.data import (
    prepare,
    cache_features,
    signature,
)
from src.experiment_runner.artifacts import write_json
from src.models.encoders.mace_denoising import (
    ResidualFrameFusion,
    AtomTemporalFusion,
)
...
class FrozenData:
    def __init__(self, cfg):
        self.records = json.loads(
            (Path(cfg['feature_cache']) / 'manifest.json').read_text()
        )['shards']
        self.targets_raw = np.concatenate(
            [
                np.load(Path(r['data_directory']) / 'targets.npy')
                for r in self.records
            ]
        )
        self.hot_targets = np.concatenate(
            [
                np.load(Path(r['data_directory']) / 'hot_targets.npy')
                for r in self.records
            ]
        )
        self.contexts = np.concatenate(
            [
                np.full(r['count'], i, np.int64)
                for i, r in enumerate(self.records)
            ]
        )
        self.sources = np.concatenate(
            [
                np.full(r['count'], r['source_index'], np.int64)
                for r in self.records
            ]
        )
        self.temperatures = np.concatenate(
            [np.full(r['count'], r['temperature_K']) for r in self.records]
        )
        self.times = torch.tensor(
            np.concatenate(
                [
                    np.tile(r['provenance']['offsets'], (r['count'], 1))
                    for r in self.records
                ]
            ),
            dtype=torch.float32,
            device='cuda',
        ) / abs(cfg['frame_offsets_ps'][-2])
        self.indices = {
        ...
        self.pooled = torch.from_numpy(
            np.concatenate(
                [
                    np.load(Path(r['directory']) / 'pooled.npy')
                    for r in self.records
                ]
            )
        ).cuda()
        ...
        offset = 0
        for r in self.records:
            self.nodes[offset : offset + r['count']].copy_(
                torch.from_numpy(np.load(Path(r['directory']) / 'nodes.npy'))
            )
            offset += r['count']
        ...
        scaling_signature = signature(
            dict(
                ...
                components=cfg['tda_components'],
                floor_fraction=cfg['block_scale_floor_fraction'],
            )
        )
        if path.exists():
            if (
                json.loads(path.with_suffix('.json').read_text())['signature']
                != scaling_signature
            ):
                raise ValueError(
                    f'Target scaling configuration or features changed: {path}'
                )
            self.scaling = dict(np.load(path))
        else:
            self.scaling = fit_targets(
                self.targets_raw[self.indices['train']],
                cfg['tda_components'],
                cfg['block_scale_floor_fraction'],
            )
            train_pooled = self.pooled[self.indices['train'], :-1]
            self.scaling.update(
                pooled_mean=train_pooled.mean((0, 1)).cpu().numpy(),
                pooled_std=train_pooled.std((0, 1), correction=0)
                .clamp_min(0.01)
                .cpu()
                .numpy(),
            )
            ...
            for chunk in np.array_split(self.indices['train'], 64):
                ...
                count += len(values)
            mean = total / count
            std = (squares / count - mean.square()).clamp_min(0.0001).sqrt()
            self.scaling.update(
                node_mean=mean.float().cpu().numpy(),
                node_std=std.float().cpu().numpy(),
            )
            np.savez(path, **self.scaling)
            write_json(
                path.with_suffix('.json'),
                dict(
                    signature=scaling_signature,
                    fit_split='train',
                    anchors=len(self.indices['train']),
                    block_std=self.scaling['block_std'].tolist(),
                    block_effective_scale=self.scaling['block_scale'].tolist(),
                    pca_cumulative_variance=self.scaling['pca_variance_ratio']
                    .cumsum()
                    .tolist(),
                    atom_features=(
                        'Cached raw float16, normalized by training node'
                        ' moments; pooling cache is float32.'
                    ),
                ),
            )
        self.pooled = (
            self.pooled
            - torch.as_tensor(self.scaling['pooled_mean'], device='cuda')
        ) / torch.as_tensor(self.scaling['pooled_std'], device='cuda')
        self.node_mean = torch.as_tensor(
            self.scaling['node_mean'], device='cuda'
        )
        self.node_std = torch.as_tensor(
            self.scaling['node_std'], device='cuda'
        )
        self.targets = {
            kind: (
                torch.from_numpy(
                    transform_target(
                        self.targets_raw, self.scaling, kind
                    ).astype(np.float32)
                ).cuda()
            )
    ...
    def get(self, rows, architecture, intervention='real'):
        ...
        nodes = (
            (self.nodes[rows].float() - self.node_mean) / self.node_std
            if architecture.startswith('atom_')
            else None
        )
...
class Predictor(nn.Module):
    def __init__(self, cfg, variant):
        ...
        if self.architecture == 'transformer':
            self.fusion = PretrainedMACETemporalEncoder(
                cfg['pretrained_checkpoint'],
                cfg['frame_offsets_ps'],
                time_scale_ps=abs(cfg['frame_offsets_ps'][-2]),
            ...
            self.fusion = AtomTemporalFusion(
                cfg['frame_offsets_ps'],
                anchor_only=self.architecture == 'atom_anchor',
            )
        elif self.architecture not in ('anchor', 'mean', 'relaxed'):
            raise ValueError(
                f'Unknown denoising architecture: {self.architecture}'
            )
...
def predict(model, data, rows, cfg, intervention='real'):
    ...
    size = (
        cfg['atom_batch_size']
        if model.architecture.startswith('atom_')
        else cfg['batch_size']
    )
    for start in range(0, len(rows), size):
        output, z = model(
            *data.get(
                rows[start : start + size], model.architecture, intervention
            )
        )
...
def validation_loss(model, data, cfg):
    ...
    return float(
        topology_loss(
            ...
            data.targets[model.kind][rows].cpu(),
            model.kind,
        )
    )
...
def preflight(cfg, data):
    ...
    for architecture in ('residual', 'atom_temporal'):
        model = Predictor(
            cfg, dict(architecture=architecture, target='blocks')
        ).cuda()
        pooled, nodes, times = data.get(rows[:4], architecture)
        torch.testing.assert_close(
            model.encode(pooled, nodes, times), pooled[:, -1], rtol=0, atol=0
        )
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
        model.train()
        loss = topology_loss(
            model(pooled, nodes, times)[0],
            data.targets['blocks'][rows[:4]],
            'blocks',
        )
        loss.backward()
        optimizer.step()
        ...
        derivative = (
            pooled.grad.abs().sum((0, 2))
            if nodes is None
            else nodes.grad.abs().sum((0, 2, 3))
        )
        if not torch.isfinite(derivative).all() or not (derivative > 0).all():
            raise AssertionError(
                f'Missing history gradients in {architecture}: {derivative}'
            )
        reports[architecture] = dict(
            initial_skip_exact=True, gradient_l1_by_frame=derivative.tolist()
        )
    ...
    if losses[-1] >= 0.5 * losses[0]:
        raise AssertionError(
            f'Tiny-set topology fit failed: {losses[0]} -> {losses[-1]}'
        )
    reports['tiny_set'] = dict(
        examples=len(rows),
    ...
    write_json(
        Path(cfg['output']) / 'preflight.json',
        dict(state='passed', checks=reports),
    )
...
def train_one(cfg, variant, seed, data):
    ...
    training_signature = signature(
        {key: value for key, value in cfg.items() if key != 'deadline'}
    )
    if completion.exists():
        saved = json.loads(completion.read_text())
        if (
            saved['signature'] != training_signature
            or saved['variant'] != variant
            or saved['seed'] != seed
            or saved['state'] != 'complete'
        ):
            raise ValueError(
                f'Completed run does not match requested variant: {directory}'
            )
        return
    if (directory / 'last.pt').exists():
        raise FileExistsError(
            f'Interrupted ablation in {directory}; explicit restart'
    ...
    if warmstart:
        source = (
            Path(cfg['output'])
            / 'runs'
            / f'anchor_blocks_seed{seed}'
            / 'best.pt'
        )
        checkpoint = torch.load(
            source, map_location='cuda', weights_only=False
        )
        head = {
            key: value
            for key, value in checkpoint['model'].items()
            if key.startswith(('trunk.', 'heads.'))
        }
    ...
    with log_path.open('w', buffering=1) as log:
        for epoch in range(1, cfg['epochs'] + 1):
            ...
            totals = dict(tda=0.0, regularization=0.0)
            for start in range(0, len(order), cfg['batch_size']):
                rows = order[start : start + cfg['batch_size']]
                optimizer.zero_grad(set_to_none=True)
                micro = (
                    cfg['atom_batch_size']
                    if variant['architecture'].startswith('atom_')
                    else len(rows)
                )
                for j in range(0, len(rows), micro):
                    selected = rows[j : j + micro]
                    prediction, z = model(
                        *data.get(selected, variant['architecture'])
                    )
                    task = topology_loss(
                        prediction,
                        data.targets[variant['target']][selected],
                        variant['target'],
                    )
                    ...
                    if variant['variance'] or variant['covariance']:
                        ...
                        reg = (
                            variant['variance'] * variance
                            + variant['covariance'] * covariance
                        )
                    loss = task + reg
                    if not torch.isfinite(loss):
                        raise FloatingPointError(
                            f'Nonfinite loss in {directory}, epoch {epoch},'
                            f' step {steps}'
                        )
                    (loss * len(selected) / len(rows)).backward()
                    totals['tda'] += float(task.detach()) * len(selected)
                    totals['regularization'] += float(reg.detach()) * len(
                        selected
                    )
    ...
    write_json(
        ...
        dict(
            ...
            anchor_exposures=exposures,
            warmstart_anchor_checkpoint=warmstart,
            seconds=time.monotonic() - started,
            selection=(
                'Validation topology loss only. Test labels are excluded from'
...
def run(cfg, stage):
    if stage in ('analysis', 'all'):
        raise ValueError(
            'The standalone denoising analysis has been retired. Use original'
            ' VICReg checkpoints with python -m src.analysis.pipeline.'
            ' Historical non-Lightning checkpoints require their recorded'
            ' source snapshot.'
        )
    out = Path(cfg['output'])
    ...
    remaining = int(
        datetime.fromisoformat(cfg['deadline']).timestamp() - time.time()
    )
    if remaining <= 0:
    ...
    def interrupted(signum, frame):
        raise InterruptedError(
            f'Denoising run interrupted by signal {signum}; see retained'
            ' artifacts.'
        )
        ...
        with tracked_run(
            ...
            if stage in ('prepare', 'all'):
                if cfg['protocol'] == 'denoising80_reuse':
                    from src.training_methods.mace_denoising.existing_data import (
                        prepare_existing,
                    )
            ...
            if stage in (
                ...
                if stage == 'potential-audit' or (
                    stage == 'all' and cfg['protocol'] == 'denoising80_reuse'
                ):
                    if cfg['protocol'] != 'denoising80_reuse':
                        raise ValueError(
                            'The paired potential audit requires the explicit'
                            ' denoising80_reuse protocol.'
                        )
                    from src.analysis.mace_potential_audit import run_audit

                    exports = json.loads(
                        (out / 'analysis/metrics.json').read_text()
                    )['exports']
