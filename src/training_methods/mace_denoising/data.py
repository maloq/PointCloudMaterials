...
def completed(directory, expected):
    ...
    if saved['signature'] != expected:
        raise ValueError(
            f'Configuration or producer changed for {path}; use a new cache.'
        )
...
def prepare(cfg):
    ...
    for source in cfg['sources']:
        ...
        if lineage in seen:
            raise ValueError(
                f'Repeated independent melt seed {lineage}; source splits'
                ' would leak.'
            )
        seen.add(lineage)
        expected = {
    ...
    producer = {
        ...
        for p in (
            Path(__file__),
            Path('src/data/histories.py'),
            Path('src/data/relaxed.py'),
            Path('src/analysis/liquid_structure.py'),
    ...
    with ProcessPoolExecutor(max_workers=cfg['workers']) as pool:
        for source_index, source in enumerate(cfg['sources']):
            ...
            np.testing.assert_allclose(
                np.diff(trajectory.timesteps) * source['timestep_ps'],
                source['cadence_ps'],
                rtol=0,
                atol=1e-9,
            )
            steps = np.rint(
                np.array(cfg['frame_offsets_ps']) / source['cadence_ps']
            ).astype(np.int64)
            np.testing.assert_allclose(
                steps * source['cadence_ps'],
                cfg['frame_offsets_ps'],
                rtol=0,
                atol=1e-9,
            )
            campaign = json.loads(
                Path(source['campaign_manifest']).read_text()
            )
            lib, parameters = cfg['relaxation']['potential_files']
            if (
                sha256(lib) != campaign['potential']['library_sha256']
                or sha256(parameters)
                != campaign['potential']['parameter_sha256']
            ):
                raise ValueError(
                    "Relaxation potential differs from source"
                    f" {source['name']}"
                )
            ...
            for frame in source['anchors']:
                ...
                provenance = dict(
                    ...
                    source_manifest_sha256=sha256(
                        trajectory.root / 'manifest.json'
                    ),
                    centers=cfg['centers_per_frame'],
                    seed=cfg['seed'],
                    offsets=cfg['frame_offsets_ps'],
                ...
                write_json(
                    ...
                    dict(
                        state='preparing_targets',
                        completed_frames=len(records),
                        total_frames=sum(
                            len(s['anchors']) for s in cfg['sources']
                        ),
                        frame=name,
                    ),
                )
                ...
                rng = np.random.default_rng(
                    np.random.SeedSequence([cfg['seed'], source_index, frame])
                )
                centers = rng.choice(
                    trajectory.atom_count,
                    cfg['centers_per_frame'],
                    replace=False,
                )
                ...
                anchor, quenched, neighbors, errors = paired_clouds(
                    hot, relaxed - low, lengths, centers
                )
                identities = trajectory.atom_ids[neighbors]
                history, history_error = history_clouds(
                    trajectory, centers, identities, frame, steps
                )
                ...
                arrays = dict(
                    ...
                    relaxed=quenched,
                    targets=target,
                    hot_targets=hot_target,
                    centers=centers,
                    neighbor_ids=identities,
                    frames=frame + steps,
                )
                ...
                saved = dict(
                    ...
                    frame=frame,
                    count=len(centers),
                    local_quantization_max_A=max(*errors, history_error),
                    fmax_eV_per_A=metadata['fmax_eV_per_A'],
                    relaxation_seconds=metadata['seconds'],
                    checksums={
                        f'{key}.npy': sha256(directory / f'{key}.npy')
                        for key in arrays
                    },
                )
    ...
    write_json(
        root / 'manifest.json',
        dict(
            protocol='denoising80',
            shards=records,
            frame_offsets_ps=cfg['frame_offsets_ps'],
            sampling=(
                'Uniform centers; fixed times; no outcome-based selection.'
            ),
            target=(
                'Relaxed full-cell anchor, fixed hot-selected 80 identities;'
                ' centered float16 offsets.'
            ),
        ),
    )
...
def cache_features(cfg):
    ...
    records = json.loads((Path(cfg['cache']) / 'manifest.json').read_text())[
        'shards'
    ]
    root = Path(cfg['feature_cache'])
    root.mkdir(parents=True, exist_ok=True)
    model = (
        PretrainedMACEEncoder(
    ...
    torch.save(
        dict(
            ...
            checkpoint_sha256=sha256(cfg['pretrained_checkpoint']),
    ...
    for index, record in enumerate(records):
        ...
        sig = signature(
            dict(
                data_manifest_sha256=sha256(source / 'manifest.json'),
                checkpoint_sha256=sha256(cfg['pretrained_checkpoint']),
                performance=cfg['performance'],
                encoder_sha256=sha256(
                    'src/models/encoders/pretrained_mace.py'
                ),
            )
        )
        saved = completed(directory, sig)
        ...
        for start in range(0, len(flat), cfg['feature_batch_size']):
            x = torch.from_numpy(
                flat[start : start + cfg['feature_batch_size']].astype(
                    np.float32
                )
            ).cuda()
            values.append(
                model.raw_node_features(
                    x, torch.zeros(len(x), dtype=torch.long, device='cuda')
                )
                .cpu()
                .numpy()
            )
        nodes = np.concatenate(values).reshape(
            len(clouds), clouds.shape[1], 80, 256
        )
        pooled = nodes.mean(2)
        hot_nodes = nodes[:, :-1].astype(np.float16)
        if not np.isfinite(hot_nodes).all():
            raise FloatingPointError(
                f'Frozen atom feature float16 overflow in {directory}'
            )
        ...
        saved = dict(
            record,
            directory=str(directory),
            data_directory=str(source),
            signature=sig,
            atom_feature_quantization_max=float(
                np.abs(nodes[:, :-1] - hot_nodes.astype(np.float32)).max()
            ),
            checksums={
                f'{key}.npy': sha256(directory / f'{key}.npy')
                for key in ('pooled', 'nodes')
            },
        )
        write_json(directory / 'manifest.json', saved)
        result.append(saved)
    ...
    write_json(
        ...
        dict(
            protocol='denoising80',
            shards=result,
            pretrained_checkpoint_sha256=sha256(cfg['pretrained_checkpoint']),
