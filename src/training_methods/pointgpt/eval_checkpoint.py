...
def load_pointgpt_model(
    checkpoint_path: str,
    cuda_device: int = 0,
    cfg: DictConfig | None = None,
) -> tuple[PointGPTModule, DictConfig, str]:
    """Restore the PointGPT module together with its Hydra config and device."""
    if cuda_device < 0:
    ...
    model: PointGPTModule = load_model_from_checkpoint(
        checkpoint_path,
        cfg,
        device=device,
        module=PointGPTModule,
    )
...
def run_checkpoint_evaluation(
    ...
    *,
    ...
    model, cfg, device = load_pointgpt_model(
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Re-run validation/test for a trained PointGPT checkpoint.",
    )
    parser.add_argument(
        "checkpoint_path",
        type=str,
