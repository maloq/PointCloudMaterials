...
import numpy as np
import torch
from src.training_methods.shared.mace_performance import (
    encode_views,
    replay_chunks,
)
...
def variance_covariance(z):
    ...
    covariance = (
        cov.square().sum() - cov.diagonal().square().sum()
    ) / z.shape[1]
...
def objective(model, z, target, temporal_mask, settings, epoch):
    groups = z.reshape(
        len(z), 2 if 'thermal' in settings else 1, 3, z.shape[-1]
    )
    anchor, spatial, temporal = groups.unbind(2)
    spatial_mse = (anchor - spatial).square().mean()
    temporal_mse = (
        (anchor[temporal_mask] - temporal[temporal_mask]).square().mean()
    )
    ...
    terms = dict(
        invariance=settings['invariance'] * 0.5 * (spatial_mse + temporal_mse),
        variance=settings['variance'] * variance,
        covariance=settings['covariance'] * covariance,
    )
    metrics = dict(
    ...
    if 'thermal' in settings:
        ...
        terms['thermal'] = settings['thermal'] * thermal
        metrics['hot_relaxed_mse'] = thermal
    if epoch >= settings['tda_start_epoch']:
        tda = (model.tda(z) - target).square().mean()
        terms['tda'] = settings['tda'] * tda
        metrics['tda_mse'] = tda
...
def cached_step(model, batch, mask, cfg, epoch):
    ...
    with torch.no_grad():
        z = encode_views(
            model, x, material, cfg['microbatch_size'], geometries
        )
    z.requires_grad_(True)
    loss, parts = objective(model, z, target, mask, cfg['loss'], epoch)
    if not bool(torch.isfinite(loss)):
        raise FloatingPointError(f'Nonfinite plain MACE loss: {parts}')
    loss.backward()
    for actual, sl in replay_chunks(
        model,
        x.flatten(0, 1),
        material.repeat_interleave(len(views)),
        cfg['microbatch_size'],
        geometries,
    ):
...
def lr_factor(step, *, start, total, warmup, peak, minimum):
    ...
    elapsed = step - start
    if elapsed < warmup:
        return 0.05 + 0.95 * elapsed / warmup
    progress = min((elapsed - warmup) / (total - start - warmup), 1.0)
    return minimum / peak + (1 - minimum / peak) * 0.5 * (
...
def make_scheduler(optimizer, cfg, steps_per_epoch):
    ...
    encoder = partial(
        lr_factor,
        start=0,
        warmup=cfg['scheduler']['warmup_epochs'] * steps_per_epoch,
        peak=cfg['learning_rate'],
        **common,
    )
    head = partial(
        lr_factor,
        start=(cfg['loss']['tda_start_epoch'] - 1) * steps_per_epoch,
        warmup=cfg['scheduler']['head_warmup_epochs'] * steps_per_epoch,
        peak=cfg['head_learning_rate'],
        **common,
    )
...
def representation_ratios(z, material, eligible):
    ...
    for i, name in enumerate(('Al', 'Mg', 'Ta')):
        ...
        shuffle = torch.as_tensor(
            np.random.default_rng(123 + i).permutation(len(a)), device=z.device
        )
        ...
        result[name] = dict(
            spatial_ratio=float(spatial), temporal_ratio=float(temporal)
        )
    return result
