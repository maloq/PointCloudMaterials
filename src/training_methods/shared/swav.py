...
class SwAVLoss(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if self.num_prototypes <= 0:
            raise ValueError(
                f"swav_num_prototypes must be > 0, got {self.num_prototypes}."
            )
        if self.prior_type not in {"uniform", "powerlaw", "custom"}:
            raise ValueError(
                "swav_prior_type must be one of ['uniform', 'powerlaw',"
                f" 'custom'], got {prior_type!r}."
            )
        if self.prior_type == "powerlaw" and self.prior_exponent <= 0.0:
            raise ValueError(
                "swav_prior_exponent must be > 0 for"
                f" swav_prior_type='powerlaw', got {self.prior_exponent}."
            )

        explicit_prior = self._build_explicit_prior(prior_probs)
        if explicit_prior.numel() > 0 and self.prior_type != "custom":
            raise ValueError(
                "swav_prior_probs was provided, so swav_prior_type must be"
                f" 'custom' or omitted; got {self.prior_type!r}."
            )
        if explicit_prior.numel() == 0 and self.prior_type == "custom":
            raise ValueError(
                "swav_prior_type='custom' requires swav_prior_probs."
            )
        self.register_buffer(
            "_explicit_prior", explicit_prior, persistent=True
        )
        ...
        if self.enabled and self.weight > 0.0:
            if self.input_dim is None:
                raise ValueError(
                    "SwAV requires a resolved encoder latent dimension."
                )
    ...
    def from_config(cls, cfg, *, input_dim: int | None):
        ...
        return cls(
            ...
            sinkhorn_iterations=int(
                getattr(cfg, "swav_sinkhorn_iterations", 20)
            ),
            start_epoch=int(getattr(cfg, "swav_start_epoch", 0)),
            freeze_prototypes_steps=int(
                getattr(cfg, "swav_freeze_prototypes_steps", 0)
            ),
    ...
    def _build_explicit_prior(self, prior_probs) -> torch.Tensor:
        ...
        if prior.numel() != self.num_prototypes:
            raise ValueError(
                "swav_prior_probs length must match swav_num_prototypes: got"
                f" {prior.numel()} values for"
                f" {self.num_prototypes} prototypes."
            )
        if not torch.isfinite(prior).all():
            raise ValueError(
                f"swav_prior_probs must be finite, got {prior.tolist()}."
            )
        if (prior <= 0.0).any():
            raise ValueError(
                "swav_prior_probs must be strictly positive, got"
                f" {prior.tolist()}."
            )
        prior_sum = prior.sum()
        if prior_sum <= 0.0:
            raise ValueError(
                "swav_prior_probs must have positive sum, got"
                f" {prior.tolist()}."
            )
    ...
    def _assignment_prior(self, *, device, dtype) -> torch.Tensor:
        ...
        raise RuntimeError(
            f"Unsupported SwAV prior type at runtime: {self.prior_type!r}."
        )
    ...
    def normalize_prototypes(self) -> None:
        if self.prototypes is None:
            raise RuntimeError(
                "Cannot normalize SwAV prototypes before they are initialized."
            )
    ...
    def _prototype_logits(self, features: torch.Tensor) -> torch.Tensor:
        if self.projector is None or self.prototypes is None:
            raise RuntimeError(
                "SwAV projector/prototypes are not initialized."
            )
    ...
    @torch.no_grad()
    def _sinkhorn(
        self, logits: torch.Tensor, *, iterations: int | None = None
    ) -> torch.Tensor:
        sinkhorn_iterations = (
            self.sinkhorn_iterations if iterations is None else int(iterations)
        )
    ...
    def compute_loss(
        ...
        *,
        ...
        logits_by_view = [
            self._prototype_logits(features) for features in view_features
        ]
        loss_terms = []
        ...
        for assign_idx, assignment_logits in enumerate(logits_by_view):
            ...
            assignment_prototype_masses.append(
                self._global_assignment_mass(assignments)
            )
            ...
            for pred_idx, prediction_logits in enumerate(logits_by_view):
                ...
                log_probs = F.log_softmax(
                    prediction_logits / self.temperature, dim=1
                )
                subloss_terms.append(
                    -(assignments * log_probs).sum(dim=1).mean()
                )
            loss_terms.append(torch.stack(subloss_terms, dim=0).mean())

        loss = torch.stack(loss_terms, dim=0).mean()
        metrics = {
            "swav_assignment_entropy": (
                torch.stack(assignment_entropies, dim=0).mean()
            ),
        }
        prototype_mass = torch.stack(assignment_prototype_masses, dim=0).mean(
            dim=0
        )
        prototype_prior = self._assignment_prior(
        ...
        prototype_prior_kl = (
            prototype_mass
            * (
                prototype_mass.clamp_min(1e-12).log()
                - prototype_prior.clamp_min(1e-12).log()
            )
        ).sum()
    ...
    def forward(
        self, features: torch.Tensor, *, profile_logits: bool = False
    ) -> torch.Tensor:
