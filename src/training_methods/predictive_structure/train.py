...
class BenchmarkData:
    def __init__(self, cfg, out):
        ...
        self.meta = dict(np.load(base / 'metadata.npz'))
        ...
        self.counts = torch.tensor(
            np.load(base / 'edge_counts.npy'), device='cuda'
        )
        self.material = torch.tensor(self.meta['material'], device='cuda')
        self.temperature = torch.tensor(
            self.meta['temperature'][:, None]
            == np.array([400.0, 450.0, 500.0])[None],
            device='cuda',
            dtype=torch.float32,
        )
        self.indices = {
            s: torch.tensor(
                np.flatnonzero(self.meta['split'] == s), device='cuda'
            )
            for s in ('train', 'val', 'test')
        }
        self.by_material = {
            s: [ids[self.material[ids] == m] for m in range(3)]
            for s, ids in self.indices.items()
        }
        saved = dict(np.load(base / 'evaluation/split_and_targets.npz'))
        n = len(self.clouds)
        self.future_mask = torch.zeros(n, device='cuda', dtype=torch.bool)
        self.future_mask[saved['rows']] = True
        self.future = torch.zeros(n, 72, device='cuda')
        target = np.concatenate(
            [saved[f].reshape(len(saved['rows']), -1) for f in FAMILIES],
            axis=1,
        )
        self.future[saved['rows']] = torch.tensor(
            target, device='cuda', dtype=torch.float32
        )
        self.future_indices = {
            s: ids[self.future_mask[ids]] for s, ids in self.indices.items()
        }
        raw = np.concatenate(
            (
                np.load(base / 'order.npy'),
                np.load(base / 'embeddings/TDA_16.npy'),
                np.load(base / 'embeddings/SOAP.npy')[:, :64],
            ),
            axis=1,
        ).astype(np.float64)
        means = []
        stds = []
        current = np.empty_like(raw)
        for m in range(3):
            train = (self.meta['split'] == 'train') & (
                self.meta['material'] == m
            )
            mean = raw[train].mean(0)
            std = raw[train].std(0)
            std = np.maximum(std, 1e-3 * np.median(std))
            current[self.meta['material'] == m] = (
                raw[self.meta['material'] == m] - mean
            ) / std
            means.append(mean)
            stds.append(std)
        self.current = torch.tensor(
            current, dtype=torch.float32, device='cuda'
        )
        np.savez(
            out / 'current_target_scaling.npz',
            mean=np.stack(means),
            std=np.stack(stds),
        )
        self.fixed = {}
        for name in cfg['models']:
            ...
            values = (
                np.load(base / 'order.npy')
                if name == 'CoarseBOO'
                else np.load(base / 'embeddings' / f'{name}.npy')
            )
        ...
        write_json(
            ...
            dict(
                train_by_material={
                    m: len(ids)
                    for m, ids in zip(
                        ('Al', 'Mg', 'Ta'), self.by_material['train']
                    )
                },
                future_counts={
                    s: len(ids) for s, ids in self.future_indices.items()
                },
                supervision=(
                    'Current BOO(8), TDA(16), SOAP(64), plus repeated-future'
                    ' mean topology(48), order(18), mobility(6). No PTM'
                    ' labels.'
                ),
                source_splits={
                    s: np.unique(saved['source'][saved['split'] == s]).tolist()
                    for s in ('train', 'val', 'test')
                },
                test_policy=(
                    'Previously examined test sources retained; no test loss'
                    ' used for fitting, early stopping or learning-rate'
                    ' selection'
                ),
            ),
        )
...
class Predictor(nn.Module):
    def __init__(self, name, cfg, data):
        ...
        base = ROOT / cfg['benchmark']
        if name == 'MACE':
            self.encoder = ReferenceMACEEncoder(accelerated=True)
        elif name == 'SchNet':
            self.encoder = SchNetEncoder()
        elif name == 'DensityMLP':
            self.encoder = DensityMLPEncoder()
            scaling = torch.load(
                base / 'density_scaling.pt', weights_only=True
            )
            ...
            kwargs = OmegaConf.to_container(
                settings.encoder.kwargs, resolve=True
            )
        ...
        self.normalization = nn.BatchNorm1d(dim, eps=1e-8, momentum=0.05)
    ...
    def features(self, ids, data, clouds=None):
        if self.name not in NEURAL:
            raw = data.fixed[self.name][ids]
        else:
            ...
            if self.name == 'GeoFrame':
                ...
                raw = self.encoder.forward_features(
                    x[:, :80] / self.radii[data.material[ids], None, None]
                )
            else:
                raw = self.encoder(
                    x, data.material[ids], data.edges[ids], data.counts[ids]
                )
    ...
    def forward(self, ids, data, clouds=None):
        ...
        return (
            z,
            self.current_head(torch.cat((z, types), 1)),
            self.future_head(torch.cat((z, data.temperature[ids]), 1)),
        )
...
def current_loss(prediction, target, material):
    ...
    return torch.stack(
        [
            torch.stack(
                [
                    (prediction[material == m, s] - target[material == m, s])
                    .square()
                    .mean()
...
def future_errors(prediction, target):
    return torch.stack(
        [
            (prediction[:, s] - target[:, s]).square().mean(1)
            for s in FUTURE_SLICES
        ],
        dim=1,
    )
...
def validation(model, data, batch_size):
    model.eval()
    ids = data.indices['val']
    current = []
    future = []
    for part in ids.split(batch_size):
        _, c, f = model(part, data)
        current.append(c)
        future.append(f)
    c = torch.cat(current)
    f = torch.cat(future)
    a = current_loss(c, data.current[ids], data.material[ids])
    keep = data.future_mask[ids]
    b = future_errors(f[keep], data.future[ids[keep]]).mean(0)
...
def check_model(model, data):
    ...
    with torch.no_grad():
        ...
        if model.name in ('MACE', 'SchNet', 'DensityMLP', 'GeoFrame'):
            axis = torch.tensor([0.3, 0.4, 0.5], device='cuda')
            angle = axis.norm()
            axis = axis / angle
...
def train_trial(
    name, seed, rate, cfg, data, directory, *, continue_to_plateau=False
):
    if not continue_to_plateau:
        directory.mkdir(parents=True, exist_ok=False)
    torch.manual_seed(seed)
    model = Predictor(name, cfg, data).cuda()
    ...
    if continue_to_plateau:
        ...
        records = [
            json.loads(line)
            for line in (directory / 'epochs.jsonl').read_text().splitlines()
        ]
        start_epoch = records[-1]['epoch']
        limit = max(limit, start_epoch + 300)
        best = previous['selection_score']
        best_epoch = previous['best_epoch'] - 1
        for r in records[cfg['geometry_warmup_epochs'] + 4 :]:
            if r['selection_score'] < anchor - 1e-4:
                anchor = r['selection_score']
                last_improvement = r['epoch'] - 1
        ...
        rng.manual_seed(seed + 1000 + 1000003 * start_epoch)
        noise_rng.manual_seed(seed + 2000 + 1000003 * start_epoch)
        write_json(
            ...
            dict(
                ...
                maximum_epoch=limit,
                reason=(
                    'Accumulated validation improvements must reset'
                    ' early-stopping patience'
                ),
                sample_seed=seed + 1000 + 1000003 * start_epoch,
                noise_seed=seed + 2000 + 1000003 * start_epoch,
            ),
        )
        del saved
    for epoch in range(start_epoch, limit):
        tick = time.monotonic()
        model.train()
        current_total = future_total = 0.0
        ramp = (
            0.0
            if epoch < cfg['geometry_warmup_epochs']
            else min(1.0, (epoch - cfg['geometry_warmup_epochs'] + 1) / 5)
        )
        for _ in range(steps):
            selected = [
                future_ids[
                    torch.randint(
                        len(future_ids), (fb,), device='cuda', generator=rng
                    )
                ]
            ]
            for pool, n in zip(data.by_material['train'], sizes):
                selected.append(
                    pool[
                        torch.randint(
                            len(pool), (n,), device='cuda', generator=rng
                        )
                    ]
                )
            ...
            if name in NEURAL:
                noise = (
                    torch.randn(
                        (len(ids), 193, 3), device='cuda', generator=noise_rng
                    )
                    * cfg['jitter_std_A']
                ).clamp(-cfg['jitter_clip_A'], cfg['jitter_clip_A'])
                noise[:, 0] = 0
            ...
            loss = (
                1.0 if ramp == 0 else cfg['current_weight']
            ) * structure + ramp * dynamic
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    f'{name}/{seed}/lr={rate}/epoch={epoch}: nonfinite task'
                    ' loss'
                )
            ...
            optimizer.step()
            current_total += float(structure.detach())
            future_total += float(dynamic.detach())
        cv, fv = validation(model, data, bs)
        score = float(fv.mean() + cfg['validation_current_weight'] * cv)
        record = dict(
            epoch=epoch + 1,
            train_current=current_total / steps,
            train_future=future_total / steps,
            validation_current=cv,
            validation_future=fv.tolist(),
        ...
        write_json(
            ...
            dict(state='running', best_epoch=best_epoch + 1, **record),
        )
        if epoch % 10 == 0:
            print(name, seed, rate, record, flush=True)
        if (
            epoch + 1 >= cfg['minimum_epochs']
            and epoch - last_improvement >= cfg['early_stopping_patience']
        ):
            break
    torch.save(payload, directory / 'last.pt')
...
def complete_plateau_audit(cfg, out, data, selected):
    ...
    for i, t in enumerate(trials):
        ...
        audit.append(
            dict(
                ...
                initial_plateau_age=age,
                continued=age < cfg['early_stopping_patience'],
            )
        )
        if age < cfg['early_stopping_patience']:
            trials[i] = train_trial(
                t['name'],
                t['seed'],
                t['learning_rate'],
                cfg,
                data,
                directory,
                continue_to_plateau=True,
            )
            if not trials[i]['converged']:
                raise RuntimeError(
                    'Continuation did not reach a validation plateau:'
                    f' {directory}'
                )
    ...
    for name in cfg['models']:
        best = min(
            (t for t in trials if t['name'] == name and t['seed'] == 123),
            key=lambda t: t['selection_score'],
        )
        ...
        for seed in cfg['seeds'][1:]:
            matches = [
                ...
                if t['name'] == name
                and t['seed'] == seed
                and t['learning_rate'] == best['learning_rate']
            ]
            if matches:
                ...
                directory = (
                    out
                    / 'trials'
                    / f'{name}_seed{seed}_lr{best["learning_rate"]:g}'
                )
                t = train_trial(
                    name, seed, best['learning_rate'], cfg, data, directory
                )
                trials.append(t)
                selected.append(t)
                write_json(out / 'trials.json', trials)
    write_json(out / 'selected_runs.json', selected)
    write_json(out / 'convergence_audit.json', audit)


def run(cfg, out):
    data = BenchmarkData(cfg, out)
    trials = []
    selected = []
...
def main():
    ...
    if (out / 'status.json').exists():
        raise FileExistsError(
            f'Run already initialized: {out}. Use a new output directory.'
        )
