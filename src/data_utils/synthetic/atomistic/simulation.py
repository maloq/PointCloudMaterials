...
class _TraceRecorder:
    ...
    def sample(self) -> None:
        ...
        self.pressure_GPa.append(
            float(-np.trace(self.atoms.get_stress(voigt=False)) / 3.0 / units.GPa)
        )
...
def _run_npt(
    ...
    *,
    ...
    if steps:
        ...
        dynamics.attach(recorder.sample, interval=config.dynamics.sample_interval)
        progress(
...
def simulate_systems(
    ...
    *,
    ...
    liquid_mask, slab_bounds = _liquid_slab_mask(
        interface, config.system.liquid_slab_fraction
    )
    ...
    # interface.  Resetting velocities implements the rapid quench used by the
    # repository's reference trajectory and preserves a checkable preparation time.
    _initialize_velocities(
        interface, config.dynamics.target_temperature_K, interface_rng
    )
    interface_trace = _run_npt(
        interface,
        config=config,
        temperature_K=config.dynamics.target_temperature_K,
