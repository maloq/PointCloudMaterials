...
def main():
    ...
    reference_coords = {}
    for name in names:
        artifacts = analysis_artifacts(full / name)
        metrics = json.loads((artifacts / "analysis_metrics.json").read_text())
        ...
        labels = []
        for frame in frames:
            path = artifacts / "snapshots" / frame["output_name"] / "md_space/local_structure_coords_clusters.npz"
        ...
        with np.load(artifacts / "analysis_inference_cache.npz") as data:
