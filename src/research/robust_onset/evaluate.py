...
def infer_patches(model,patch_list,chunk,device,deadline=None):
    ...
    for start in range(0,len(patch_list),chunk):
        ...
        bank=GraphBank(graph_arrays(patch_list[start:start+chunk],model.encoder.cutoff),model.encoder,device)
    ...
    for i,s in enumerate(plan['sources']):
        ...
        with np.load(p) as data:
            ...
            for x,center in zip(patches(a),a['centers'],strict=True):
                keep=np.flatnonzero(np.linalg.norm(x,axis=1)<model.encoder.radius)
...
def run(study,name,device='cuda',deadline=None):
    ...
    model=Model(saved['encoder_config'],arm['tensor_pool'],conditions.shape[1]).to(device)
    model.load_state_dict(saved['model']);model.eval()
    with np.load(study.cache/f'{arm["input"]}-graphs.npz') as a:arrays=dict(a)
    bank=GraphBank(arrays,model.encoder,device)
    ...
    with torch.no_grad():
        ...
        threshold=threshold_at_fpr(bins[tune]<5,probabilities[tune,-1],sources[tune])
        result['onset']={str(t):weighted_scores(bins[dev]<=k,probabilities[dev,k],sources[dev],threshold if k==4 else None)
                         for k,t in enumerate([.75,3,6,9,12])}
    ...
    chosen=np.asarray(chosen);clean=[patches(arrays)[i] for i in chosen]
...
def collect(study):
    ...
    for arm in study.config['arms']:
        ...
        m=json.loads((p/'metrics.json').read_text());onset=m['onset']['12'];d=m['dynamics']['domains']['all']['lags']['0.75']
        primary=next(n for n in m['noise'] if n['expected_relative_rms']==.005)
        rows.append(dict(arm=name,selected_step=m['selected_step'],AP12=onset['average_precision'],
            prevalence=onset['prevalence'],Brier12=onset['brier'],recall_at_tuning_5pct_FPR=onset['recall'],
    ...
    snapshot_metric_docs(study.root,'robust_onset')
    if rows:
    ...
    paired=[];base=predictions.get('B-onset')
    if base is not None:
        ...
        for name,p in predictions.items():
            ...
            for w in draws:
                ...
                values.append(average_precision_score(y,p['risk'][:,-1],sample_weight=w)-
                              average_precision_score(y,base['risk'][:,-1],sample_weight=w))
            lo,hi=np.quantile(values,[.025,.975])
            paired.append(dict(arm=name,reference='B-onset',delta_AP12_low=float(lo),delta_AP12_high=float(hi),valid_draws=len(values)))
    ...
    baseline_rows=[dict(model=name,AP12=m['horizons']['12.0']['average_precision'],
        Brier12=m['horizons']['12.0']['brier'],selected_step=m['best_step']) for name,m in baselines['scores'].items()]
    ...
    lines=['# Robust onset encoder screen','',f'{len(rows)}/{len(study.config["arms"])} evaluations complete.',
        '', 'One seed; 25 fitting / 5 tuning / 15 reused development roots. Development has 18 onset windows among 643 at-risk windows. This is screening, not an untouched test.',
        '', 'Primary AP is the source-weighted 12 ps sustained local-onset AP. Checkpoint selection uses tuning AP subject to present/current/future retention versus initialization. Frozen probes use NLL selection separately.',
        '', '| Arm | AP12 | Brier12 | Noise RMS | Input noise / spacing | Jump RMS, 0.75 ps |',
        '|---|---:|---:|---:|---:|---:|']
    lines += [f'| {r["arm"]} | {r["AP12"]:.4f} | {r["Brier12"]:.4f} | {r["noise_rms"]:.4f} | {r["input_noise_percent"]:.3f}% | {r["jump_rms_075"]:.4f} |' for r in rows]
    lines += ['', 'Separate matched descriptor/temperature probes (tuning-NLL selected):', '',
        '| Input/readout | AP12 | Brier12 |', '|---|---:|---:|']
    lines += [f'| {r["model"]} | {r["AP12"]:.4f} | {r["Brier12"]:.4f} |' for r in baseline_rows]
