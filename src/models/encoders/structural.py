...
class StructuralMACE(nn.Module):
    def __init__(self, channels=16, backend='cueq'):
        super().__init__()
    ...
    def forward(self,batch):
        x=batch['packed_positions']; graph=batch['node_graph']; edges=batch['edges']
        ...
        for i,(interaction,product) in enumerate(zip(self.interactions,self.products,strict=True)):
            ...
            h=normalize_atom_features(product(node_feats=h,sc=sc,node_attrs=attrs))*w[:,None]
        return self.pool(h,x,graph,len(batch['log_scale']))
...
class StructuralGATr(nn.Module):
    def __init__(self,mv_channels=8,scalar_channels=128,layers=2):
        super().__init__()
        self.mv_channels=mv_channels; self.scalar_channels=scalar_channels
        self.species=nn.Embedding(len(ATOMIC_NUMBERS),16)
        self.input=EquiLinear(1,mv_channels,in_s_channels=20,out_s_channels=scalar_channels)
        attention=SelfAttentionConfig(num_heads=4,pos_encoding=False)
        def block():return GATrBlock(mv_channels,scalar_channels,attention,MLPConfig())
        self.spatial=nn.ModuleList([block() for _ in range(layers)])
        self.temporal=nn.ModuleList([block() for _ in range(layers)])
        self.time_embedding=nn.ModuleList([nn.Linear(2,scalar_channels) for _ in range(layers)])
        # Small open gates permit temporal gradients from the first update.
...
class StructuralModel(nn.Module):
    def __init__(self,architecture,backend='cueq'):
        ...
        self.encoder=StructuralMACE(backend=backend) if architecture=='mace' else StructuralGATr()
        self.projector=nn.Sequential(nn.Linear(128,256),nn.SiLU(),nn.Linear(256,64))
        self.physical=nn.Sequential(nn.Linear(128,128),nn.SiLU(),nn.Linear(128,85))
        self.tda=nn.Sequential(nn.Linear(128,128),nn.SiLU(),nn.Linear(128,144))
        self.predictor=nn.Sequential(nn.Linear(65,128),nn.SiLU(),nn.Linear(128,64))
