...
def build_model(cfg: DictConfig):
    """Factory that **returns the encoder and decoder *separately***.

    The previous implementation returned a single monolithic autoencoder
    module.  To allow the LightningModule to compile and manage the
    encoder and decoder independently we now construct the two parts and
    return them as a pair ``(encoder, decoder)``.
    """
    model_type   = cfg.type
    ...
    # Radii configuration for spherical decoders
    if num_points < 24:
        R1 = cfg.get("R1", 0.54)
        R2 = cfg.get("R2", 0.75)  # ignored for 1-shell decoders
    ...
    #  Build encoder
    # ------------------------------------------------------------------
    if model_type == "PnAE_MLP":
        encoder = MLPEncoder(num_points, latent_size)
    elif model_type in {"PnAE_Fold", "PnAE_FoldSphere", "PnAE_FoldSphere_Attn"}:
        encoder = PointNetAEEncoder(latent_size, dropout_rate=dropout_rate)
    elif model_type == "PnAE_Fold_S":
        encoder = PointNetAEEncoderSmall(latent_size, dropout_rate=dropout_rate)
    else:
    ...
    # ------------------------------------------------------------------
    if model_type == "PnAE_MLP":
        decoder = MLPDecoder(num_points, latent_size)
    elif model_type == "PnAE_Fold":
        decoder = FoldingDecoderTwoStep(num_points, latent_size, dropout_rate=dropout_rate)
    elif model_type == "PnAE_Fold_S":
        decoder = FoldingDecoderTwoStepSmall(num_points, latent_size, dropout_rate=dropout_rate)
    elif model_type == "PnAE_FoldSphere":
        decoder = FoldingDecoderSphereTwoShell(
            num_points,
            latent_size,
    ...
    elif model_type == "PnAE_FoldSphere_Attn":
        decoder = FoldingDecoderSphereTwoShellAttn(
            num_points,
            latent_size,
            dropout_rate=dropout_rate,
            R1=R1,
        ...
        # The earlier encoder branch already validated the type, so this
...
class PointNetAE_Folding_Sphere(PointNetAE):
    def __init__(self, num_points, latent_size, dropout_rate: float = 0.2,
                  R1: float = 0.5, R2: float = 1.0, n_shells: int = 2):
        super().__init__(num_points, latent_size, dropout_rate=dropout_rate)
        self.decoder = FoldingDecoderSphereTwoShell(num_points, latent_size,
                                                    dropout_rate=dropout_rate, R1=R1, R2=R2, n_shells=n_shells)


class PointNetAE_Folding_Sphere_Attn(PointNetAE):
    def __init__(self, num_points, latent_size, dropout_rate: float = 0.2,
                  R1: float = 0.5, R2: float = 1.0, n_shells: int = 2):
        super().__init__(num_points, latent_size, dropout_rate=dropout_rate)
        self.decoder = FoldingDecoderSphereTwoShellAttn(num_points, latent_size,
                                                    dropout_rate=dropout_rate, R1=R1, R2=R2, n_shells=n_shells)
...
class PointNetAE_Small(PointNetAE):
    ...
    def forward(self, x_input):
        # Encode the input point cloud
        latent, _, trans_feat_encoder = self.encoder(x_input)
        ...
        # Return: reconstructed_x, latent_representation (latent_code), logvar_or_none (None for AE), aux_outputs
        return reconstructed_x, latent, None, aux_outputs
...
class PointNetAEEncoder(nn.Module):
    """Encoder module used by the *regular* PointNet autoencoders.

    This is effectively the logic previously implemented inside
    ``PointNetAE.encoder`` but wrapped into a standalone ``nn.Module``
    so that it can be instantiated, registered as a child module of
    the LightningModule and (optionally) compiled independently from
    the decoder.
    """
    def __init__(self, latent_size: int, dropout_rate: float = 0.2):
        super().__init__()
        self.features_encoder = PointNetEncoder(
            feature_transform=True,
            channel=3,
            dropout_rate=dropout_rate,
        )
        # Dimension after global max-pool in PointNetEncoder is 512
    ...
    def forward(self, x: torch.Tensor):  # (B, 3, N)
        x_features, trans, trans_feat = self.features_encoder(x)
        latent = self.mlp(x_features)
        return latent, trans, trans_feat


class PointNetAEEncoderSmall(nn.Module):
    """Smaller encoder variant that pairs with the *Small* decoders."""
    def __init__(self, latent_size: int, dropout_rate: float = 0.2):
        super().__init__()
        self.features_encoder = PointNetEncoderSmall(feature_transform=True, channel=3)
        self.mlp = nn.Sequential(
