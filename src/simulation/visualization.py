...
def _normalize_visualization_target(
    ...
    if target == "":
        raise ValueError(
            "viz_target must be a non-empty string when provided."
        )
    supported_targets = {
        "closeup_paper",
...
def generate_visualizations(
    ...
    grain_ids = np.array(
        [
            (-1 if atom.get("grain_id") is None else int(atom["grain_id"]))
            for atom in atoms
        ],
        dtype=int,
    )
    ...
    if viz_target_norm == "global_diagonal_cut_paper":
        print(
            "  Generating visualizations (target=global_diagonal_cut_paper)..."
        )
    ...
    _render_structural_analysis(
        ...
        atoms_array,
        phases,
        output_dir / "figure_structural_analysis.png",
        rng,
    )
    ...
    _render_interface_analysis(
        ...
        grains,
        color_map,
        output_dir / "figure_interfaces.png",
        rng,
    )
...
def _render_structural_analysis(
    ...
    crystal_phases = [
        ...
        if 'crystal' in p.lower() or 'bcc' in p.lower() or 'fcc' in p.lower()
    ]
    liquid_phases = [
        p
        for p in unique_phases
        if 'liquid' in p.lower() or 'amorphous' in p.lower()
    ]
    ...
    ax1.axvline(
        1.0, color='red', linestyle=':', alpha=0.5, label='r = avg_nn_dist'
    )
    ...
    for phase_type, phase_list in [
        ('crystal', crystal_phases),
        ('liquid', liquid_phases),
    ]:
        if not phase_list:
            continue
        ...
        if np.sum(mask) > 50:
            ...
            ax2.plot(
                ...
                gr,
                linewidth=2,
                label=f'{phase_type.title()} phases',
    ...
    if len(angles) > 0:
        ...
        ax4.axvline(
            60, color='blue', linestyle=':', alpha=0.7, label='60° (FCC/ICO)'
        )
        ax4.axvline(
            90, color='green', linestyle=':', alpha=0.7, label='90° (BCC/FCC)'
        )
        ax4.axvline(
            109.5,
            color='purple',
    ...
    for phase_type, phase_list in [
        ...
        if np.sum(mask) > 50:
            phase_positions = positions[mask]
            coords = _compute_coordination(
                phase_positions, cutoff, sample_size=2000, rng=rng
            )
    ...
    if phase_coords:
        ...
        colors = [
            CRYSTAL_COLOR if 'Crystal' in l else LIQUID_COLOR for l in labels
        ]
...
def _render_statistics_panel(
    ...
    # Simplify labels
    short_labels = [
        p.replace('crystal_', '').replace('liquid_', 'liq_')[:15]
        for p in labels
    ]
    ...
    if grain_phases:
        ...
        short_labels = [
            p.replace('crystal_', '').replace('liquid_', 'liq_')[:12]
            for p in labels
        ]
        colors = [_phase_to_color(p) for p in labels]
...
def _render_slice_views(
    ...
    for col, (name, axis, pos, plot_axes, slice_axis) in enumerate(slices):
        ...
        if len(slice_positions) > 0:
            ...
            if len(slice_positions) > max_points:
                ...
                idx = np.random.choice(
                    len(slice_positions), max_points, replace=False
                )
            ...
            for phase in np.unique(slice_phases_sample):
                ...
                scatter = ax_top.scatter(
                    ...
                    edgecolors='black',
                    linewidths=0.35,
                    label=phase.replace('crystal_', '').replace(
                        'liquid_', 'liq_'
                    )[:12],
                )
                ...
                if col == 0 and phase not in phase_labels:
                    ...
                    phase_labels.append(
                        phase.replace('crystal_', '').replace(
                            'liquid_', 'liq_'
                        )[:12]
                    )
        ...
        ax_top.set_title(
            f'{name} Plane'
            f' ({slice_axis}={pos:.1f}Å)\n{len(slice_positions)} atoms',
            fontsize=10,
        )
    ...
    fig.suptitle(
        f'Cross-Section Views (slice thickness ≈ {slice_thickness:.1f}Å)',
        fontsize=13,
        fontweight='bold',
        y=0.98,
    )
...
def _render_grain_analysis(
    ...
    colors = [
        grain_color_map.get(gid, _DEFAULT_GRAY_RGBA) for gid in grain_ids_list
    ]
    ...
    if grain_sizes and len(seed_positions) > 0:
        ...
        ax2.scatter(
            distances, sizes, c=colors, s=60, edgecolors='black', alpha=0.7
        )
    ...
    if misorientations:
        ...
        ax3.axvline(
            15, color='red', linestyle='--', label='Low-angle GB (15°)'
        )
    ...
    if np.any(boundary_mask):
        ...
        if sample_size < len(boundary_pos):
            idx = np.random.choice(
                len(boundary_pos), sample_size, replace=False
            )
            boundary_pos = boundary_pos[idx]
            boundary_gids = boundary_gids[idx]

        boundary_colors = [
            grain_color_map.get(int(g), _DEFAULT_GRAY_RGBA)
    ...
    if grain_phase_counts:
        ...
        short_labels = [
            l.replace('crystal_', '').replace('liquid_', 'liq_')[:12]
            for l in labels
        ]
...
def _render_interface_analysis(
    ...
    for i in range(len(positions)):
        ...
        if (
            len(set(neighbor_phases)) > 1
            or len(set(neighbor_grains[neighbor_grains >= 0])) > 1
        ):
    ...
    if len(interface_positions) > 0:
        ...
        sample_idx = rng.choice(
            len(interface_positions), sample_size, replace=False
        )
    ...
    if np.sum(bulk_mask) > 100:
        r_bulk, gr_bulk = _compute_rdf(
            positions[bulk_mask], L, r_max=3 * avg_nn, rng=rng
        )
        ax3.plot(r_bulk / avg_nn, gr_bulk, 'b-', linewidth=2, label='Bulk')

    if np.sum(interface_mask) > 100:
        r_int, gr_int = _compute_rdf(
            interface_positions, L, r_max=3 * avg_nn, rng=rng
        )
    ...
    bulk_coords = (
        _compute_coordination(
            positions[bulk_mask], coord_cutoff, sample_size=2000, rng=rng
        )
        if np.sum(bulk_mask) > 50
        else np.array([])
    )
    int_coords = (
        _compute_coordination(
            interface_positions, coord_cutoff, sample_size=2000, rng=rng
        )
        if len(interface_positions) > 50
        else np.array([])
    )
    ...
    if len(bulk_coords) > 0 and len(int_coords) > 0:
        ...
        ax4.hist(
            ...
            bins=bins,
            alpha=0.6,
            label=f'Bulk (μ={np.mean(bulk_coords):.1f})',
            color='blue',
            density=True,
        )
        ax4.hist(
            ...
            bins=bins,
            alpha=0.6,
            label=f'Interface (μ={np.mean(int_coords):.1f})',
            color='red',
            density=True,
        )
        ax4.legend()
    ...
    if np.sum(bulk_mask) > 100 and np.sum(interface_mask) > 100:
        ...
        ax5.hist(
            distances, bins=50, color='purple', edgecolor='black', alpha=0.7
        )
        ax5.axvline(
            np.median(distances),
            color='red',
...
def _paper_family_color_for_phase(phase: str) -> str:
    ...
    if phase_text == "":
        raise ValueError(
            "phase must be a non-empty string when resolving paper family"
            " colors."
        )
...
def _sample_display_indices(
    ...
    return np.sort(
        rng.choice(n_total, size=sample_limit_int, replace=False).astype(
            int, copy=False
        )
    )
...
def _compute_group_centroids(
    ...
    if gids.shape != (pos.shape[0],):
        raise ValueError(
            f"group_ids must have shape ({pos.shape[0]},), got"
            f" {gids.shape} for positions shape {pos.shape}."
        )
    valid_mask = gids >= 0
    ...
    for gid in np.unique(gids[valid_mask]):
        ...
        if member_positions.shape[0] == 0:
            continue
        centroids.append(
            np.mean(member_positions, axis=0, dtype=np.float64).astype(
                np.float32
            )
        )
    ...
    if not centroids:
        return np.zeros((0, 3), dtype=np.float32), np.zeros((0,), dtype=int)
    return np.stack(centroids, axis=0).astype(
        np.float32, copy=False
    ), np.asarray(centroid_group_ids, dtype=int)
...
def _scatter_paper_global_panel(
    ...
    *,
    ...
    if pts.shape[0] != len(color_values):
        raise ValueError(
            f"positions length {pts.shape[0]} must match colors length"
            f" {len(color_values)} for panel {title!r}."
        )
...
def _compute_regular_sample_center_atoms(
    ...
    *,
    ...
    if pts.shape[0] == 0:
        raise ValueError(
            "Cannot compute regular sample centers from an empty atom array."
        )

    radius_use = float(radius)
    if radius_use <= 0.0:
        raise ValueError(
            f"Regular sample-center radius must be positive, got {radius}."
        )
    overlap_use = float(overlap_fraction)
    if not (0.0 <= overlap_use < 1.0):
        raise ValueError(
            "Regular sample-center overlap_fraction must satisfy 0 <="
            f" overlap_fraction < 1, got {overlap_fraction}."
        )

    stride = radius_use * (1.0 - overlap_use)
    if stride <= 0.0:
        raise ValueError(
            "Regular sample-center stride must be positive, got"
            f" radius={radius_use}, overlap_fraction={overlap_use}."
        )
    ...
    if np.any(min_center >= max_center):
        raise RuntimeError(
            ...
            f" min_center={min_center.tolist()},"
            f" max_center={max_center.tolist()}, radius={radius_use}."
        )
    ...
    if np.any(dims <= 0):
        raise RuntimeError(
            ...
            f" max_center={max_center.tolist()}, stride={stride}."
        )

    if bool(drop_edge_samples):
        ranges = [
            (1, int(dim) - 1) if int(dim) >= 3 else (0, 0)
    ...
    if any(start >= stop for start, stop in ranges):
        raise RuntimeError(
            "Regular sample-center grid has zero valid centers after applying"
            f" drop_edge_samples. dims={dims.tolist()}, ranges={ranges},"
            f" drop_edge_samples={bool(drop_edge_samples)}."
        )

    grid_axes = [
        np.arange(start, stop, dtype=np.float64) for start, stop in ranges
    ]
    ...
    if nearest_indices.size == 0:
        raise RuntimeError(
            "Regular sample-center reconstruction produced zero snapped"
            " centers after KD-tree snapping."
        )
...
def _compute_local_structure_sample_centers(
    ...
    if not isinstance(sampling_cfg, dict):
        raise TypeError(
            "sampling_cfg must be a dict when reconstructing local-structure"
            f" centers, got {type(sampling_cfg)!r}."
        )
    ...
    if sample_type != "regular":
        raise ValueError(
            "global_diagonal_cut_paper currently supports only regular"
            " synthetic local-structure sampling because random sample"
            " centers are not persisted in the generated dataset. Got"
            f" sample_type={sample_type!r}."
        )
    if "radius" not in sampling_cfg:
        raise ValueError(
            "sampling_cfg is missing required key 'radius' for regular"
            " sample-center reconstruction."
        )
    if "overlap_fraction" not in sampling_cfg:
        raise ValueError(
            "sampling_cfg is missing required key 'overlap_fraction' for"
            " regular sample-center reconstruction."
        )
...
def _compute_neighbor_grain_boundary_mask(
    ...
    *,
    ...
    if gids.shape != (pts.shape[0],):
        raise ValueError(
            f"grain_ids must have shape ({pts.shape[0]},), got"
            f" {gids.shape} for positions shape {pts.shape}."
        )
    cutoff_use = float(cutoff)
    if cutoff_use <= 0.0:
        raise ValueError(
            f"Boundary-neighbor cutoff must be positive, got {cutoff}."
        )
    ...
    pair_indices = np.asarray(
        tree.query_pairs(cutoff_use, output_type="ndarray"), dtype=int
    )
...
def _render_global_structure_diagonal_cut_paper(
    ...
    if len(atoms) == 0:
        raise ValueError(
            "Cannot render paper diagonal-cut overview because atoms is empty."
        )
    if sampling_cfg is None:
        raise ValueError(
            "Cannot render paper diagonal-cut overview without sampling_cfg"
            " because the center panels must reproduce synthetic"
            " local-structure sample centers."
        )

    mask = _diagonal_cut_mask(atom_positions, global_cfg.L)
    if not np.any(mask):
        raise RuntimeError(
            "Diagonal cut mask kept zero atoms; cannot render paper"
            " diagonal-cut overview."
        )
    ...
    phase_raw_colors = [
        ...
        for phase in filtered_phases.tolist()
    ]

    sampled_center_positions_all, sampled_center_atom_indices = (
        _compute_local_structure_sample_centers(
            atom_positions,
            sampling_cfg,
        )
    )
    center_mask = _diagonal_cut_mask(
        sampled_center_positions_all, global_cfg.L
    )
    if not np.any(center_mask):
        raise RuntimeError(
            "The reconstructed local-structure sample-center set has zero"
            " points inside the diagonal cut."
        )
    phase_center_positions = np.asarray(
        sampled_center_positions_all[center_mask], dtype=np.float32
    )
    phase_center_atom_indices = np.asarray(
        sampled_center_atom_indices[center_mask], dtype=int
    )
    full_to_filtered = np.full((atom_positions.shape[0],), -1, dtype=int)
    full_to_filtered[kept_indices] = np.arange(
        kept_indices.shape[0], dtype=int
    )
    phase_center_filtered_indices = np.asarray(
        full_to_filtered[phase_center_atom_indices], dtype=int
    )
    if np.any(phase_center_filtered_indices < 0):
        raise RuntimeError(
            "Some reconstructed local-structure centers fell inside the"
            " diagonal cut, but their snapped atom indices were not found in"
            " the diagonal-cut atom set. This indicates an internal indexing"
            " inconsistency."
        )
    phase_center_phases = np.asarray(phases, dtype=object)[
        phase_center_atom_indices
    ]
    phase_center_grain_ids = np.asarray(grain_ids, dtype=int)[
        phase_center_atom_indices
    ]
    phase_center_colors = [
        _paper_family_color_for_phase(str(phase))
    ...
    sampled_center_boundary_mask = np.asarray(
        boundary_mask[phase_center_filtered_indices], dtype=bool
    )
    boundary_center_positions = np.asarray(
        phase_center_positions[sampled_center_boundary_mask], dtype=np.float32
    )
    boundary_center_phases = np.asarray(phase_center_phases, dtype=object)[
        sampled_center_boundary_mask
    ]
    ...
    fig.suptitle(
        "Global Structure Overview (Diagonal Cut)",
        fontsize=17,
        fontweight="bold",
        y=0.975,
    )
    fig.subplots_adjust(
        left=0.02, right=0.985, bottom=0.03, top=0.93, wspace=0.03, hspace=0.12
    )
...
def _render_global_structure_core(
    ...
    if total_atoms <= phase_sample_limit:
        ...
        phase_indices = rng.choice(
            total_atoms, size=phase_sample_limit, replace=False
        )
    ...
    intermediate_mask = np.array(
        [str(p).startswith('intermediate_') for p in phase_array]
    )
    if np.any(intermediate_mask):
        inter_positions = _ensure_point_array(
            atom_positions[intermediate_mask]
        )
    ...
    ax4 = fig.add_subplot(2, 3, 4, projection='3d')
    boundary_mask, _ = _compute_boundary_indices(
        atom_positions, grain_ids, grains
    )
    if np.any(boundary_mask):
        ...
        if sample_size < len(boundary_positions):
            idx = rng.choice(
                len(boundary_positions), sample_size, replace=False
            )
    ...
    if dropouts:
        vacancy_positions = [
            e.get('vacancy_position', [0, 0, 0]) for e in dropouts[:500]
        ]
...
def _render_closeup_view(
    ...
    mask = np.all(
        (atom_positions >= 0) & (atom_positions <= corner_max), axis=1
    )
...
def _compute_closeup_corner_max(
    ...
    *,
    ...
    if not np.isfinite(box_size) or float(box_size) <= 0.0:
        raise ValueError(
            f"box_size must be positive and finite, got {box_size}."
        )
    if not np.isfinite(volume_divisor) or float(volume_divisor) <= 0.0:
        raise ValueError(
            "volume_divisor must be positive and finite, got"
            f" {volume_divisor}."
        )
    ...
    if shape.shape != (3,):
        raise ValueError(
            f"region_shape must have shape (3,), got {shape.shape}."
        )
    if np.any(~np.isfinite(shape)) or np.any(shape <= 0.0):
        raise ValueError(
            "region_shape must contain positive finite values, got"
            f" {shape.tolist()}."
        )

    scale = ((1.0 / float(volume_divisor)) / float(np.prod(shape))) ** (
        1.0 / 3.0
    )
...
def _render_closeup_paper_view(
    ...
    *,
    ...
    if atom_positions.ndim != 2 or atom_positions.shape[1] != 3:
        raise ValueError(
            "atom_positions must have shape (N, 3), got"
            f" {atom_positions.shape}."
        )
    if atom_positions.shape[0] == 0:
        raise ValueError(
            "Cannot render a paper close-up figure because atom_positions is"
            " empty."
        )
    ...
    mask = np.all(
        (atom_positions >= 0.0) & (atom_positions <= corner_max[None, :]),
        axis=1,
    )
    closeup_indices = np.flatnonzero(mask)
    if closeup_indices.size == 0:
        raise RuntimeError(
            ...
            f" volume_divisor=1/{volume_divisor} and extents"
            f" {corner_max.tolist()}."
        )
    ...
    ax1.set_title(
        f"Corner close-up (1/{volume_divisor} volume,"
        f" {closeup_indices.size:,} atoms)"
    )
    ...
    fig.subplots_adjust(
        left=0.03, right=0.99, bottom=0.03, top=0.91, wspace=0.08
    )
    output_path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(output_path, bbox_inches="tight")
...
def _render_local_base_gallery(
    ...
    *,
    ...
    base_phases = sorted(
        {p for p in phases if not str(p).startswith('intermediate_')}
    )
    if not base_phases:
        return
    ...
    positions_tree_use = (
        positions_tree
        if positions_tree is not None
        else cKDTree(atom_positions)
    )
    use_objects = bool(
        len(atoms) > 0
        and hasattr(global_cfg, 'objects_per_grain')
        and 'object_id' in atoms[0]
    )
    if use_objects:
...
def _load_reference_point_clouds(
    output_dir: pathlib.Path,
) -> Dict[str, np.ndarray]:
...
def _paper_family_specs(
    ...
    for spec in specs:
        ...
        if pure_source == "reference":
            ...
            raise ValueError(
                f"Unsupported pure_source {pure_source!r} for paper family"
                f" spec {spec!r}."
            )
        if (
            perturbed_phase is not None
            and perturbed_phase not in available_set
        ):
    ...
    if missing_items:
        raise ValueError(
            "Paper local-base figure requires all crystal family pairs to be"
            f" available. Missing entries: {missing_items}."
        )
...
def _prepare_reference_local_points(
    ...
    *,
    ...
    if pts.ndim != 2 or pts.shape[1] != 3:
        raise ValueError(
            f"reference_points must have shape (N, 3), got {pts.shape}."
        )
    if pts.shape[0] == 0:
    ...
    if local.shape[0] == 0:
        raise RuntimeError(
            "No local points remain after selecting a centered reference"
            " neighborhood."
        )
...
def _match_local_structure_scale(
    ...
    *,
    ...
    if source.ndim != 2 or source.shape[1] != 3:
        raise ValueError(
            f"source_points must have shape (N, 3), got {source.shape}."
        )
    if target.ndim != 2 or target.shape[1] != 3:
        raise ValueError(
            f"target_points must have shape (N, 3), got {target.shape}."
        )
    ...
    source_scale = float(
        np.percentile(np.linalg.norm(source, axis=1), float(radius_percentile))
    )
    target_scale = float(
        np.percentile(np.linalg.norm(target, axis=1), float(radius_percentile))
    )
    if not np.isfinite(source_scale) or source_scale <= 1e-8:
        raise ValueError(
            "Computed invalid source scale while matching local structure"
            f" scale: source_scale={source_scale},"
            f" radius_percentile={radius_percentile}."
        )
    if not np.isfinite(target_scale) or target_scale <= 1e-8:
        raise ValueError(
            "Computed invalid target scale while matching local structure"
            f" scale: target_scale={target_scale},"
            f" radius_percentile={radius_percentile}."
        )
    return (source * (target_scale / source_scale)).astype(
        np.float32, copy=False
    )
...
def _ensure_connected_edges(
    ...
    if len({_find(i) for i in range(n_points)}) != 1:
        raise RuntimeError(
            "Failed to construct a connected edge graph for the paper"
...
def _save_structure_xyz(
    ...
    *,
    ...
    if pts.shape[0] == 0:
        raise ValueError(
            f"Cannot save empty XYZ structure to {output_dir / file_name}."
        )
...
def _compute_local_structure_half_span(points: np.ndarray) -> float:
    ...
    if not np.isfinite(span) or span <= 0.0:
        raise ValueError(
            f"Computed invalid structure span {span} for points shape"
            f" {pts.shape}."
        )
    return 0.5 * span
...
def _compute_radial_colormap_colors(
    ...
    *,
    ...
    if pts.shape[0] == 0:
        raise ValueError(
            "Cannot compute colormap colors for an empty point cloud."
        )
    ...
    if radius_scale <= 1e-12:
        ...
        t = np.clip(dists / radius_scale, 0.0, 1.0).astype(
            np.float32, copy=False
        )
    ...
    if colors.shape[1] < 3:
        raise RuntimeError(
            f"Colormap {cmap_name!r} returned an invalid color array with"
            f" shape {colors.shape}."
        )
...
def _draw_local_structure_panel(
    ...
    *,
    ...
    if local_oriented.ndim != 2 or local_oriented.shape[1] != 3:
        raise ValueError(
            f"points must have shape (N, 3), got {local_oriented.shape}."
        )
    if local_colors.shape != (local_oriented.shape[0], 3):
        raise ValueError(
            "point_colors must have shape (N, 3) matching points. Got points"
            f" shape {local_oriented.shape} and point_colors shape"
            f" {local_colors.shape}."
        )
...
def _compute_paper_display_half_span(
    ...
    for record in records:
        ...
        if pure_geometry is None:
            raise ValueError(
                "Paper record is missing pure_geometry:"
                f" keys={sorted(record.keys())}."
            )
        half_spans.append(
            _compute_local_structure_half_span(pure_geometry["points"])
        )
        perturbed_geometry = record.get("perturbed_geometry")
        if perturbed_geometry is not None:
            half_spans.append(
                _compute_local_structure_half_span(
                    perturbed_geometry["points"]
                )
            )
    if not half_spans:
        raise ValueError(
            "Cannot compute paper display half-span because no panel"
            " geometries were prepared."
        )
    # Tighter framing so local neighborhoods read less dense at fixed point count.
    return float(max(half_spans) * 0.80)


def _layout_local_base_paper_axes(
    fig: Any, axes_arr: np.ndarray
) -> Dict[str, float]:
    n_rows, n_cols = axes_arr.shape
    if n_cols != 2:
        raise ValueError(
            "Expected exactly 2 columns for paper local-base layout, got"
            f" shape {axes_arr.shape}."
        )
    ...
    if axis_height <= 0.0:
        raise RuntimeError(
            "Invalid paper local-base layout: computed non-positive"
            f" axis_height {axis_height} for n_rows={n_rows}."
        )

    for row_idx in range(n_rows):
        y0 = top - (row_idx + 1) * axis_height - row_gap * row_idx
        axes_arr[row_idx, 0].set_position(
            [left_x, y0, axis_width, axis_height]
        )
        axes_arr[row_idx, 1].set_position(
            [right_x, y0, axis_width, axis_height]
        )
...
def _render_local_base_paper_gallery(
    ...
    *,
    ...
    if len(atom_positions) == 0:
        raise ValueError(
            "Cannot render paper local-base figure because atom_positions is"
            " empty."
        )
    ...
    for spec in family_specs:
        ...
        if pure_source == "reference":
            ...
            if not pure_atoms_for_scale:
                raise ValueError(
                    "Paper local-base figure requires unperturbed dataset"
                    f" samples to scale-match reference phase {pure_phase!r},"
                    " but none were found."
                )
            pure_scale_rep = _select_local_environment_closest_to_reference(
                ...
                target_count=target_count,
                candidate_limit=min(
                    len(pure_atoms_for_scale), max(candidate_limit, 1024)
                ),
            )
            ...
            if not pure_atoms:
                raise ValueError(
                    "Paper local-base figure could not find any pure samples"
                    f" for phase {pure_phase!r}."
                )
            ...
            pure_points = np.asarray(
                pure_rep["local_points"], dtype=np.float32
            )
        else:
            raise ValueError(
                f"Unsupported pure_source {pure_source!r} while rendering the"
                " paper local-base figure."
            )
        ...
        if pure_oriented.shape[0] != int(target_count):
            raise RuntimeError(
                "Paper local-base figure expected the pure structure to"
                f" contain {target_count} points, got"
                f" {pure_oriented.shape[0]} for phase {pure_phase!r}."
            )
        ...
        if not perturbed_atoms:
            raise ValueError(
                "Paper local-base figure could not find any perturbed samples"
                f" for phase {perturbed_phase!r}."
            )
        ...
        perturbed_geometry = _prepare_local_structure_geometry(
            representative["local_points"]
        )
        perturbed_oriented = np.asarray(
            perturbed_geometry["points"], dtype=np.float32
        )
        if perturbed_oriented.shape[0] != int(target_count):
            raise RuntimeError(
                "Paper local-base figure expected the perturbed structure to"
                f" contain {target_count} points, got"
                f" {perturbed_oriented.shape[0]} for phase"
                f" {perturbed_phase!r}."
            )
...
def _render_local_galleries(
    ...
    intermediate_phases = sorted(
        {p for p in phases if str(p).startswith('intermediate_')}
    )
...
def _extract_local_neighborhood_points(
    ...
    *,
    ...
    if center_pos.shape != (3,):
        raise ValueError(
            "center_atom['position'] must resolve to shape (3,), got"
            f" {center_pos.shape} for atom keys={sorted(center_atom.keys())}."
        )

    radius_use = (
        float(radius)
        if radius is not None
        else 2.0 * float(global_cfg.avg_nn_dist)
    )
    if radius_use <= 0.0:
        raise ValueError(
            f"Neighborhood radius must be positive, got {radius_use}."
        )
    ...
    if idxs_arr.size == 0:
        raise RuntimeError(
            ...
            f" final_index={center_atom.get('final_index', '?')} at"
            f" position={center_pos.tolist()}."
        )

    coords = (
        np.asarray(atom_positions[idxs_arr], dtype=np.float32)
        - center_pos[None, :]
    )
    ...
    if coords.shape[0] == 0:
        raise RuntimeError(
            "No local coordinates remain after distance sorting for center"
            f" atom final_index={center_atom.get('final_index', '?')}."
        )
...
def _compute_local_environment_descriptor(
    ...
    *,
    ...
    if pts.ndim != 2 or pts.shape[1] < 3:
        raise ValueError(
            f"local_points must have shape (N, >=3), got {pts.shape}."
        )
    ...
    keep = radial[: min(descriptor_length, radial.size)].astype(
        np.float32, copy=False
    )
    ...
    if not np.isfinite(scale) or scale <= 1e-8:
        raise RuntimeError(
            "Failed to normalize local environment descriptor because the"
            f" local radial scale is invalid: scale={scale},"
            f" descriptor_length={descriptor_length}."
        )
...
def _select_representative_local_environment(
    ...
    *,
    ...
    if not phase_atoms:
        raise ValueError(
            f"Cannot select a representative for empty phase {phase!r}."
        )
    ...
    if not candidate_records:
        raise RuntimeError(
            "Representative selection produced no candidate local"
            f" environments for phase {phase!r}."
        )
    ...
    descriptor_dist = np.linalg.norm(
        descriptor_matrix - descriptor_center, axis=1
    )
...
def _select_local_environment_closest_to_reference(
    ...
    *,
    ...
    if not phase_atoms:
        raise ValueError(
            "Cannot select a reference-matched environment for empty phase"
            f" {phase!r}."
        )
    ...
    if reference_local.ndim != 2 or reference_local.shape[1] != 3:
        raise ValueError(
            "reference_points must have shape (N, 3), got"
            f" {reference_local.shape}."
        )
    ...
    if best_record is None:
        raise RuntimeError(
            "Reference-guided representative selection produced no candidates"
            f" for phase {phase!r}."
        )
...
def _compute_center_to_edge_colors(
    ...
    *,
    ...
    colors = (
        center_rgb[None, :] * (1.0 - t[:, None])
        + edge_rgb[None, :] * t[:, None]
    )
...
def _resolve_local_coordination_shell(
    ...
    *,
    ...
    if n_points < 2:
        return {
            ...
            "distance_matrix": np.full(
                (n_points, n_points), np.inf, dtype=np.float32
            ),
    ...
    max_shell_neighbors = min(
        max(min_shell_neighbors, int(max_shell_neighbors)), n_points - 1
    )
    dmat = np.linalg.norm(pts[:, None, :] - pts[None, :, :], axis=-1)
    ...
    for point_idx in range(n_points):
        ...
        if candidate_dist.size > shell_count:
            cutoff = 0.5 * (
                float(candidate_dist[shell_count - 1])
                + float(candidate_dist[shell_count])
            )
        else:
...
def _build_local_coordination_edges(
    ...
    *,
    ...
    local_shell_counts = np.asarray(
        shell["shell_neighbor_counts"], dtype=np.int32
    )
    ...
    if edge_mode_use == "coordination_shell":
        ...
        median_degree = int(
            np.clip(
                np.rint(np.median(local_shell_counts)),
                3,
                max(3, max_shell_neighbors),
            )
        )
...
def _draw_edges(
    ...
    *,
    ...
    if point_colors is not None:
        point_rgb = np.clip(
            np.asarray(point_colors, dtype=np.float32)[:, :3], 0.0, 1.0
        )
    ...
    for edge in edges:
        ...
        if point_rgb is not None:
            edge_rgb = 0.5 * (
                point_rgb[int(edge[0])] + point_rgb[int(edge[1])]
            )
...
def _set_equal_axes_3d(
    ax: Any, coords: np.ndarray, *, half_span: Optional[float] = None
) -> None:
    ...
    if half_span is None:
        ...
        if not np.isfinite(half) or half <= 0.0:
            raise ValueError(
                f"half_span must be positive and finite, got {half_span}."
            )
...
def _compute_cluster_style_pca_orientation_basis(
    ...
    *,
    ...
    if not np.isfinite(det_basis):
        raise ValueError(
            "Invalid PCA basis determinant while orienting representative"
            f" points: det={det_basis}."
        )
    if det_basis < 0.0:
        basis[:, 2] *= -1.0
        det_basis = float(np.linalg.det(basis))
    if det_basis <= 0.0:
        raise ValueError(
            "Failed to build a right-handed PCA basis for representative"
            f" points: det={det_basis}."
        )
    return centered, basis, eigvals, int(center_idx), float(det_basis)


def _orient_points_for_representative_view(
    points: np.ndarray,
) -> Tuple[np.ndarray, Dict[str, Any]]:
    centered, basis, eigvals, center_idx, det_basis = (
        _compute_cluster_style_pca_orientation_basis(
            points,
            eps=1e-8,
        )
    )
...
def _render_local_representative_gallery(
    ...
    *,
    ...
    phase_to_atoms: Dict[str, List[Dict[str, Any]]] = {
        str(phase): [] for phase in selected_phases
    }
    for atom in atoms:
    ...
    phase_list = [
        phase for phase in selected_phases if phase_to_atoms[str(phase)]
    ]
    if not phase_list:
        return
    ...
    fig = plt.figure(
        figsize=(3.45 * n_cols, 3.5 * n_rows), dpi=220, facecolor="white"
    )
...
def _render_local_gallery(
    ...
    use_objects = (
        hasattr(global_cfg, 'objects_per_grain')
        and len(atoms) > 0
        and 'object_id' in atoms[0]
    )
    ...
    for col, phase in enumerate(phase_list):
        if use_objects:
            ...
            for row in range(rows):
                ...
                short_phase = phase.replace('crystal_', '').replace(
                    'liquid_', 'liq_'
                )[:15]
            ...
            sample_indices = rng.choice(
                len(phase_atoms), size=num_samples, replace=False
            )
...
def _plot_local_neighborhood(
    ...
    max_dist = (
        np.max(distances)
        if len(distances) > 0
        else float(global_cfg.avg_nn_dist)
    )
    ...
    if len(coords) >= 4:
        ...
        if edge_type == 'delaunay':
            ...
            try:
                ...
                warnings.warn(
                    "Delaunay triangulation failed; falling back to KNN"
                    f" edges. Error: {exc}",
                    RuntimeWarning,
                    stacklevel=2,
                )
    ...
    if len(coords) > 0:
        ...
        limit = (
            data_limit if radius is None else max(float(radius), data_limit)
        )
    else:
        limit = (
            float(global_cfg.avg_nn_dist) if radius is None else float(radius)
        )
...
def _build_grain_color_map(grains: Sequence[Dict[str, Any]]) -> Dict[int, Any]:
    ...
    grain_ids = sorted(
        {int(g['grain_id']) for g in grains if g.get('grain_id') is not None}
    )
    if not grain_ids:
        return {}
    ...
    return {
        gid: tuple(map(float, cmap(i / denom)))
...
def _compute_boundary_indices(
    ...
    if atom_positions.size == 0 or grain_ids.size == 0:
        return np.zeros(len(atom_positions), dtype=bool), np.full(
            len(atom_positions), -1, dtype=int
        )
...
def _diagonal_cut_mask(
    atom_positions: np.ndarray, box_size: float
) -> np.ndarray:
