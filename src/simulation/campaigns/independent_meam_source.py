...
def _run_lammps(
    ...
    *,
    ...
    if launcher == "srun_pmi2":
        ...
        if "SLURM_JOB_ID" not in os.environ or srun is None:
            raise RuntimeError(
                "The srun_pmi2 source launcher requires a Slurm allocation and srun; "
                f"SLURM_JOB_ID={os.environ.get('SLURM_JOB_ID')!r}, srun={srun!r}."
            )
        command = [
            srun,
            "--mpi=pmi2",
    ...
    elif launcher == "local_mpiexec":
        if "SLURM_JOB_ID" in os.environ:
            raise RuntimeError(
                "The local source launcher refuses to run inside Slurm; "
                f"SLURM_JOB_ID={os.environ['SLURM_JOB_ID']!r}."
            )
        mpiexec = shutil.which("mpiexec")
        if mpiexec is None:
            raise FileNotFoundError(
                "The local source launcher requires pointnet mpiexec on PATH."
            )
        command = [
            mpiexec,
            "-n",
        ...
        raise ValueError(
            f"Unsupported source launcher {launcher!r}; expected 'srun_pmi2' or "
            "'local_mpiexec'."
        )
    ...
    *,
    ...
    if launcher == "srun_pmi2":
        ...
        if allocated != MPI_RANKS:
            raise RuntimeError(
                f"SLURM_NTASKS={allocated}, expected {MPI_RANKS} for {run_dir}."
            )
    elif launcher == "local_mpiexec":
        if "SLURM_JOB_ID" in os.environ:
            raise RuntimeError(
                "run-local-task refuses to run inside Slurm; "
                f"SLURM_JOB_ID={os.environ['SLURM_JOB_ID']!r}."
            )
    else:
        raise ValueError(
    ...
    execution_mode = "slurm_srun_pmi2" if launcher == "srun_pmi2" else launcher
...
def _expanded_submitted_job_count() -> int:
    queued = subprocess.run(
        ["squeue", "-r", "-h", "-u", os.environ["USER"], "-o", "%i"],
        check=True,
        text=True,
        capture_output=True,
    )
    return len([line for line in queued.stdout.splitlines() if line.strip()])
...
def submit_next_wave(campaign_root: str | Path, start_index: int) -> dict[str, Any]:
    ...
    configured_concurrency = int(manifest["execution"]["array_concurrency"])
    expanded_submitted_jobs = _expanded_submitted_job_count()
    available_slots = NORMAL_QOS_MAX_SUBMITTED_JOBS - expanded_submitted_jobs
    wave_size = min(
        configured_concurrency,
        len(runs) - start_index,
        available_slots - 1,
    )
    if wave_size <= 0:
        raise RuntimeError(
            "Cannot submit a source array and its required successor without exceeding "
            f"the normal-QOS limit: submitted={expanded_submitted_jobs}, "
            f"limit={NORMAL_QOS_MAX_SUBMITTED_JOBS}, available={available_slots}."
        )
    stop_index = start_index + wave_size - 1
    ...
    record = {
        ...
        "configured_array_concurrency": configured_concurrency,
        "submitted_wave_size": wave_size,
        "expanded_submitted_jobs_before_submission": expanded_submitted_jobs,
        "normal_qos_max_submitted_jobs": NORMAL_QOS_MAX_SUBMITTED_JOBS,
    }
...
def _arguments(argv=None) -> argparse.Namespace:
    ...
    local_task = subparsers.add_parser("run-local-task")
    local_task.add_argument("--campaign-root", required=True, type=Path)
    local_task.add_argument("--task-index", required=True, type=int)
...
def main(argv=None) -> None:
    ...
    if args.action == "prepare":
        ...
        run_source_task(args.campaign_root, args.task_index, launcher="local_mpiexec")
