...
def _central_slice_mask(atoms: Atoms) -> np.ndarray:
    ...
    if np.count_nonzero(mask) < min(200, len(atoms)):
        mask = (
            np.abs(positions[:, 1] - 0.5 * cell_lengths[1])
            <= 0.2 * cell_lengths[1]
        )
    return mask
...
def _plot_structure(
    ...
    axis.set(
        ...
        title=(
            f"{title}\ncentral y slice,"
            f" T={config.dynamics.target_temperature_K:.0f} K,"
...
def _plot_thermodynamics(
    ...
    figure, axes = plt.subplots(
        3, 1, figsize=(8.0, 8.5), sharex=True, constrained_layout=True
    )
    axes[0].plot(trace.step, trace.temperature_K, color="#6a4c93")
    axes[0].axhline(
        config.dynamics.target_temperature_K, color="black", linestyle="--"
    )
    axes[0].set_ylabel("T (K)")
    axes[1].plot(trace.step, trace.pressure_GPa, color="#1982c4")
    axes[1].axhline(
        config.dynamics.pressure_GPa, color="black", linestyle="--"
    )
...
def _plot_diagnostics(
    ...
    ptm_values = [
        diagnostics.ptm_structure_fractions[name] for name in ptm_names
    ]
    label_values = [
        np.count_nonzero(labels.phase_names == name) / len(labels.phase_names)
    ...
    axes[0].set(
        ylim=(0.0, 1.0), ylabel="atom fraction", title="PTM audit (not labels)"
    )
    axes[1].bar(
        PHASE_NAMES,
        label_values,
        color=[PHASE_COLORS[name] for name in PHASE_NAMES],
    )
    axes[1].tick_params(axis="x", rotation=20)
    axes[1].set(
        ylim=(0.0, 1.0),
        ylabel="atom fraction",
        title="Preparation/context labels",
    )
    figure.suptitle(
...
def write_environment_visualizations(
    ...
    *,
    ...
    _plot_structure(
        visualization_dir / "structure_slice.png", atoms, labels, config, name
    )
    _plot_thermodynamics(
        visualization_dir / "thermodynamic_trace.png",
        trace,
        len(atoms),
        config,
        name,
    ...
    _plot_diagnostics(
        visualization_dir / "structure_diagnostics.png",
        labels,
        diagnostics,
        name,
    )
...
def write_benchmark_overview(root: Path, environment_names: list[str]) -> None:
    ...
    for axis in axes_array[len(environment_names) :]:
        axis.axis("off")
    figure.suptitle(
        "Force-driven aluminium phase-context benchmark", fontsize=16
    )
