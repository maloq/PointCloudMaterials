...
def _load_source_atoms(
    ...
    if observed_selection_sha256 != config.potential_selection_report_sha256:
        raise RuntimeError(
            "Potential-selection report changed after jFFS configuration"
            f" load: path={config.potential_selection_report},"
            f" loaded_sha256={config.potential_selection_report_sha256},"
            f" observed_sha256={observed_selection_sha256}. Reload the"
            " configuration and revalidate the selected generator before"
            " running."
        )
    ...
    if observed_generator_sha256 != config.selected_generator_config_sha256:
        raise RuntimeError(
            "Selected source generator config changed after jFFS"
            f" configuration load: path={config.generator.config_path},"
            f" selected_sha256={config.selected_generator_config_sha256},"
            f" observed_sha256={observed_generator_sha256}."
        )
    ...
    if manifest.get("source_kind") != "immutable_homogeneous_liquid_only":
        raise RuntimeError(
            f"{manifest_path}: jFFS requires"
            " source_kind='immutable_homogeneous_liquid_only', got"
            f" {manifest.get('source_kind')!r}. Generate a dedicated 500 K NPT"
            " liquid source; do not couple rare-event shots to an active"
            " phase-context run."
        )
    ...
    if crystalline_fraction > maximum_crystalline_fraction:
        raise RuntimeError(
            f"{metadata_path}: source crystalline"
            f" fraction={crystalline_fraction:.6f} "
    ...
    if len(non_liquid):
        raise RuntimeError(
            f"{atom_table_path}: jFFS homogeneous nucleation requires a"
            f" bulk-liquid source, but {len(non_liquid)} atoms do not have"
    ...
    with np.load(trajectory_path) as trajectory:
        ...
        if len(frame_indices) != 1:
            raise RuntimeError(
                f"{trajectory_path}: expected exactly one source frame at"
                f" step={config.source_frame_step}, found"
        ...
        positions_A = np.asarray(stored_positions_A, dtype=np.float64)
        cell_A = np.asarray(
    ...
    if atom_table["position"].shape != stored_positions_A.shape:
        raise RuntimeError(
            f"{atom_table_path}: position"
            f" shape={atom_table['position'].shape} does not match"
            f" {trajectory_path} endpoint shape={stored_positions_A.shape}."
        )
    if not np.array_equal(atom_table["position"], stored_positions_A):
        ...
        raise RuntimeError(
            f"{atom_table_path}: final atom-table positions do not exactly"
            " match source trajectory endpoint"
            f" step={config.source_frame_step};"
    ...
    if not np.isclose(
        ...
        raise RuntimeError(
            f"{trajectory_path}: source frame cell"
            f" determinant={cell_volume_A3:.12g} A^3 differs from recorded"
            f" volume={recorded_volume_A3:.12g} A^3."
        )
    expected_count = len(build_initial_solid(config.generator))
    if positions_A.shape != (expected_count, 3):
        raise RuntimeError(
            f"{trajectory_path}: source positions have"
            f" shape={positions_A.shape}, expected {(expected_count, 3)} from"
    ...
    source_evidence = {
        ...
        "source_generator_config": {
            ...
            "parsed_config_sha256": _canonical_sha256(
...
def generate_jumpy_ffs(
    ...
    *,
    ...
    validate_potential_qualification(
        ...
        context=(
            f"jumpy FFS force-only shot calculator {config.dataset_name!r}"
        ),
    ...
    if calculator is None:
        ...
        if not callable(property_mode_setter):
            raise TypeError(
                ...
                " set_md_property_mode('forces') fast path. Fixed-cell NVT"
                " must not silently compute unused stress every MD step."
            )
        ...
        if observed_property_mode != config.shot_md_property_mode:
            raise RuntimeError(
                "Configured jFFS calculator rejected the required forces-only"
                " MD mode: observed"
                f" md_property_mode={observed_property_mode!r}."
            )
        execution_identity["jumpy_ffs_runtime"] = {
            "md_property_mode": config.shot_md_property_mode,
            "stress_evaluated_during_nvt_steps": False,
            "reason": (
                "fixed-cell Langevin-NVT requires forces but not virial stress"
            ),
        }
    else:
        execution_identity["jumpy_ffs_runtime"] = {
            ...
            "reason": (
                "injected test/controlled-study calculator; generic ASE"
                " contract"
            ),
        }
    ...
    initial_state = engine.initialize_state(
        source_atoms, random_seed=velocity_seed
    )
    ...
    scientific_scope = {
        ...
        "supported_claim": (
            "A weighted jFFS rate for this exact finite periodic cell,"
            " fixed-volume Langevin-NVT dynamics, integer connected-PTM"
            " cluster coordinate, interface set, CV cadence, and"
            " manifest-bound MLIP."
        ),
        "required_before_physical_aluminium_rate_claim": (
            "Validate interface placement and overlap, progress-coordinate"
            " committors, basin stationarity, CV-cadence convergence,"
            " shot-count convergence, finite-size convergence, fixed-volume"
            " sensitivity, and MLIP melting/transport/solid-liquid-interface"
            " kinetics."
        ),
        "uncertainty_method": (
            "Paired circular moving-block bootstrap of consecutive basin"
            " inter-exit times and complete per-exit descendant-tree success"
            " probabilities; the configured block length is measured in basin"
            " exits."
        ),
        "boundary_uncertainty_policy": (
            "If every observed descendant tree fails or every tree succeeds,"
            " retain the weighted point estimate but report confidence"
            " interval, standard error, and confidence level as undefined. A"
            " degenerate all-zero/all-one bootstrap is not evidence of zero"
            " uncertainty."
        ),
        "restart_checkpoint_policy": (
            "The connected-PTM CV is evaluated at every configured CV"
            " interval. Exact outward-crossing states are committed"
            " immediately; ordinary basin progress is committed only at"
            " basin_checkpoint_interval_steps. Equilibration and active shots"
            " follow their own explicit checkpoint intervals, with phase"
            " boundaries, trial starts, returns, and interface landings"
            " committed immediately. A crash discards all later volatile"
            " observations and restarts from the complete phase-space and"
            " PCG64 Markov state in the last durable checkpoint. Statistical"
            " correctness does not assume that CUDA/CuEq force kernels"
        ...
        "warning": (
            ...
            if potential_is_qualified
                ...
                "The selected potential has no qualifying kinetics validation"
                " report; the numerical jFFS rate is exploratory and must not"
                " be presented as a quantitative aluminium nucleation rate."
            )
        ),
    }
    progress(
