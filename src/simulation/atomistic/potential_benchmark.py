...
def _float_list(value: Any, context: str, path: Path) -> tuple[float, ...]:
    if not isinstance(value, list) or not value:
        raise TypeError(
            f"{path}: {context} must be a non-empty list, got {value!r}."
        )
    result = tuple(float(item) for item in value)
    if not np.isfinite(result).all():
        raise ValueError(
            f"{path}: {context} contains non-finite values: {result}."
        )
    return result


def load_potential_benchmark_config(
    path: str | Path,
) -> PotentialBenchmarkConfig:
    ...
    _reject_unknown(
        eos_raw,
        {"volume_scales", "supercell_repetitions"},
        "equation_of_state",
        config_path,
    )
    _reject_unknown(
    ...
    if not isinstance(model_configs_raw, list) or len(model_configs_raw) < 2:
        raise ValueError(
            f"{config_path}: model_configs must list at least two generator"
            f" configs for a comparison, got {model_configs_raw!r}."
        )
    model_configs = tuple(_repo_path(item) for item in model_configs_raw)
    for model_config in model_configs:
        if not model_config.is_file():
            raise FileNotFoundError(
                f"{config_path}: model generator config does not exist:"
                f" {model_config}."
            )

    scales = _float_list(
        eos_raw.get("volume_scales"),
        "equation_of_state.volume_scales",
        config_path,
    )
    if len(scales) < 5 or any(scale <= 0.0 for scale in scales):
        raise ValueError(
            f"{config_path}: equation_of_state.volume_scales must contain at"
            f" least five positive values bracketing 1.0, got {scales}."
        )
    if not min(scales) < 1.0 < max(scales):
        raise ValueError(
            f"{config_path}: equation_of_state.volume_scales must bracket 1.0,"
            f" got {scales}."
        )
    ...
    if (
        ...
        raise ValueError(
            f"{config_path}: equation_of_state.supercell_repetitions must be"
            f" three integers >= 2, got {repetitions_raw!r}."
        )

    timesteps = _float_list(
        nve_raw.get("timesteps_fs"), "nve.timesteps_fs", config_path
    )
    if len(timesteps) < 2 or any(timestep <= 0.0 for timestep in timesteps):
        raise ValueError(
            f"{config_path}: nve.timesteps_fs must contain at least two"
            f" positive values for a convergence check, got {timesteps}."
        )
    sample_interval_fs = _positive_float(
        nve_raw.get("sample_interval_fs"),
        "nve.sample_interval_fs",
        config_path,
    )
    for timestep in timesteps:
        ...
        if not np.isclose(ratio, round(ratio), rtol=0.0, atol=1.0e-12):
            raise ValueError(
                f"{config_path}:"
                f" nve.sample_interval_fs={sample_interval_fs} must be an"
                f" integer multiple of every timestep; {timestep} fs gives"
                f" ratio {ratio}."
            )
    ...
    if not isinstance(application_raw, dict):
        raise TypeError(
            f"{config_path}: application_structures must map labels to extxyz"
            f" paths, got {type(application_raw).__name__}."
        )
    application_structures = {
    ...
    for label, structure_path in application_structures.items():
        if not structure_path.is_file():
            raise FileNotFoundError(
                f"{config_path}: application structure {label!r} does not"
                f" exist: {structure_path}."
            )
    ...
    if dft_raw is None:
        ...
        if not isinstance(dft_raw, dict):
            raise TypeError(
                f"{config_path}: dft_reference must be null or a provenance"
                f" mapping, got {type(dft_raw).__name__}."
            )
        _reject_unknown(
            dft_raw,
            {
        ...
        if not extxyz_path.is_file():
            raise FileNotFoundError(
                f"{config_path}: dft_reference.extxyz does not exist:"
                f" {extxyz_path}."
            )
        string_values = {
        ...
        empty_fields = sorted(
            key for key, value in string_values.items() if not value
        )
        if empty_fields:
            raise ValueError(
                f"{config_path}: dft_reference fields must be non-empty:"
                f" {empty_fields}."
            )
        ...
        if minimum_frames_per_state < 5:
            raise ValueError(
                f"{config_path}: dft_reference.minimum_frames_per_state must"
                " be >= 5 independent configurations for every state, got"
    ...
    if (
        ...
        or not states_raw
        or any(not isinstance(item, str) or not item for item in states_raw)
    ):
        raise TypeError(
            f"{config_path}: required_dft_states must be a non-empty list of"
            f" names, got {states_raw!r}."
        )
    if len(set(states_raw)) != len(states_raw):
        raise ValueError(
            f"{config_path}: required_dft_states contains duplicates:"
            f" {states_raw!r}."
        )
    ...
    if len(pressure_range) != 2 or pressure_range[0] > pressure_range[1]:
        raise ValueError(
            f"{config_path}: qualification_scope.pressure_range_GPa must be"
            f" [lower, upper], got {pressure_range}."
        )
    state_ranges_raw = scope_raw.get("state_temperature_ranges_K")
    if not isinstance(state_ranges_raw, dict):
        raise TypeError(
            f"{config_path}: qualification_scope.state_temperature_ranges_K"
            " must map every required DFT state to [lower, upper], got"
            f" {type(state_ranges_raw).__name__}."
        )
    ...
    for state, state_range_raw in state_ranges_raw.items():
        ...
        if (
            len(state_range) != 2
            or state_range[0] <= 0.0
            or state_range[0] > state_range[1]
        ):
            raise ValueError(
                f"{config_path}: qualification temperature range for"
                f" {state!r} must be positive [lower, upper], got"
                f" {state_range}."
            )
    ...
    if not isinstance(authorized_claims_raw, dict):
        raise TypeError(
            f"{config_path}: qualification_scope.authorized_claims must be a"
            f" mapping with exactly {list(POTENTIAL_CLAIM_NAMES)}, got"
            f" {type(authorized_claims_raw).__name__}."
        )
    missing_claims = sorted(
        set(POTENTIAL_CLAIM_NAMES) - set(authorized_claims_raw)
    )
    unknown_claims = sorted(
        set(authorized_claims_raw) - set(POTENTIAL_CLAIM_NAMES)
    )
    if missing_claims or unknown_claims:
        raise KeyError(
            f"{config_path}: qualification_scope.authorized_claims must"
            f" contain exactly {list(POTENTIAL_CLAIM_NAMES)};"
            f" missing={missing_claims}, unknown={unknown_claims}."
        )
    if any(
        type(authorized_claims_raw[claim]) is not bool
        for claim in POTENTIAL_CLAIM_NAMES
    ):
        raise TypeError(
            f"{config_path}: every qualification_scope.authorized_claims value"
            f" must be an exact boolean, got {authorized_claims_raw!r}."
        )
    authorized_claims = {
    ...
    if authorized_claims["kinetics"]:
        raise ValueError(
            f"{config_path}: this benchmark cannot authorize kinetics because"
            " it does not validate liquid transport coefficients or"
            " kinetic-time rescaling; set"
    ...
    if maximum_timestep_fs not in timesteps:
        raise ValueError(
            f"{config_path}:"
            f" qualification_scope.maximum_timestep_fs={maximum_timestep_fs} must"
            f" be explicitly included in nve.timesteps_fs={list(timesteps)}."
        )

    melting_raw = raw.get("melting_scan_summaries")
    if not isinstance(melting_raw, dict):
        raise TypeError(
            f"{config_path}: melting_scan_summaries must map model_name to a"
            " list of velocity_summary.json paths, got"
            f" {type(melting_raw).__name__}."
        )
    ...
    for model_name, summary_paths_raw in melting_raw.items():
        if not isinstance(summary_paths_raw, list) or not summary_paths_raw:
            raise TypeError(
                f"{config_path}: melting_scan_summaries.{model_name} must be a"
                f" non-empty list of paths, got {summary_paths_raw!r}."
            )
        summary_paths = tuple(_repo_path(item) for item in summary_paths_raw)
        for summary_path in summary_paths:
            if not summary_path.is_file():
                raise FileNotFoundError(
                    f"{config_path}: melting scan for model"
                    f" {model_name!r} does not exist: {summary_path}."
                )
    ...
    if not isinstance(random_seed, int) or isinstance(random_seed, bool):
        raise TypeError(
            f"{config_path}: nve.random_seed must be an integer, got"
            f" {random_seed!r}."
        )
    melting_source_url = str(
        melting_reference_raw.get("source_url", "")
    ).strip()
    if not melting_source_url:
        raise ValueError(
            f"{config_path}: melting_reference.source_url must identify the"
            " physical reference used to judge the model melting point."
        )
...
def _phase_cell(
    ...
    *,
    ...
    raise ValueError(
        f"Unsupported phase {phase!r}; expected fcc, hcp, or bcc."
    )
    ...
    *,
    ...
    if not np.isfinite(sampled_values).all():
        raise FloatingPointError(
            f"{phase}: EOS scan produced non-finite values."
        )
    minimum_index = int(np.argmin(energies_per_atom_eV))
    if minimum_index in {0, len(volume_scales) - 1}:
        raise RuntimeError(
            f"{phase}: minimum sampled energy occurs at the edge of the EOS"
            f" volume scan (scale={volume_scales[minimum_index]}). Widen the"
            " configured volume bracket."
        )
    eos = EquationOfState(
        volumes_per_atom_A3, energies_per_atom_eV, eos="birchmurnaghan"
    )
    equilibrium_volume_A3, equilibrium_energy_eV, bulk_modulus_eV_per_A3 = (
        eos.fit()
    )
    if (
        ...
        raise RuntimeError(
            ...
            f" energy={equilibrium_energy_eV},"
            f" bulk_modulus={bulk_modulus_eV_per_A3}."
        )
    if (
        not min(volumes_per_atom_A3)
        < equilibrium_volume_A3
        < max(volumes_per_atom_A3)
    ):
        raise RuntimeError(
            f"{phase}: fitted equilibrium volume"
            f" {equilibrium_volume_A3:.8f} A^3/atom lies outside sampled range"
            f" [{min(volumes_per_atom_A3):.8f},"
    ...
    if phase == "hcp" and _hcp_c_over_a is None:
        ...
        for ratio in ratios:
            hcp_a_A = (
                4.0 * equilibrium_volume_A3 / (np.sqrt(3.0) * float(ratio))
            ) ** (1.0 / 3.0)
            atoms = bulk(
                symbol,
                "hcp",
...
def nve_energy_conservation(
    ...
    *,
    ...
    atoms = bulk(symbol, "fcc", a=lattice_constant_A, cubic=True).repeat(
        repetitions
    )
...
def nve_energy_conservation_from_atoms(
    ...
    *,
    ...
    if not np.isclose(
        total_steps * timestep_fs, duration_ps * 1000.0, atol=1.0e-10
    ):
        raise ValueError(
            f"NVE duration {duration_ps} ps is not an integer number of"
            f" {timestep_fs} fs steps."
        )
    ...
    while dynamics.nsteps < total_steps:
        ...
        energies_eV_per_atom.append(
            float(atoms.get_total_energy() / len(atoms))
        )
    ...
    if (
        not np.isfinite(energy_array).all()
        or not np.isfinite(temperature_array).all()
    ):
        raise FloatingPointError(
            "NVE run produced non-finite energy or temperature at"
            f" timestep={timestep_fs} fs."
        )
    slope_eV_per_atom_ps, intercept_eV_per_atom = np.polyfit(
        time_array, energy_array, 1
    )
    ...
    numeric_values = [
        value for key, value in result.items() if key not in {"sample_count"}
    ]
    if not np.isfinite(numeric_values).all():
        raise FloatingPointError(
            f"NVE diagnostics are non-finite at timestep={timestep_fs} fs:"
            f" {result}."
        )
    return result
...
def _evaluate_structure(
    ...
    if (
        ...
        raise FloatingPointError(
            "Application-structure MLIP evaluation is non-finite."
        )
    summary = {
...
def verlet_cache_parity(
    ...
    *,
    ...
    missing = [
        name for name in required_attributes if not hasattr(calculator, name)
    ]
    if missing:
        raise TypeError(
            "Production cache parity requires VerletSkinMACECalculator"
            " attributes; "
    ...
    perturbed = base.copy()
    rng = np.random.default_rng(random_seed)
    perturbed.positions += rng.normal(
        0.0, 0.015, size=perturbed.positions.shape
    )
    ...
    if reuse_after <= reuse_before:
        raise RuntimeError(
            "Cache parity perturbation did not exercise graph reuse; reduce"
            " the configured perturbation or inspect the Verlet validity"
            " bound."
        )
    ...
    if not np.isfinite(list(result.values())).all():
        raise FloatingPointError(
            f"Verlet cache parity produced non-finite metrics: {result}."
        )
    return result
...
def _geometry_sha256(atoms: Atoms) -> str:
    ...
    if not np.isfinite(wrapped_fractional).all():
        raise FloatingPointError(
            "Cannot hash a geometry with non-finite positions."
        )
    ...
    for name, array in zip(
        ...
        digest.update(b"\0")
        digest.update(
            np.asarray(contiguous.shape, dtype=np.dtype("<i8")).tobytes()
        )
    ...
    *,
    ...
    for frame_index, atoms in enumerate(frames):
        ...
        if state not in state_temperatures:
            raise ValueError(
                f"{path}: DFT reference frame {frame_index} has"
                f" state={state!r}; every frame must use one of the required"
                f" states {required_states}."
            )
        configuration_id = atoms.info.get("configuration_id")
        if not isinstance(configuration_id, str) or not configuration_id:
            raise ValueError(
                f"{path}: DFT reference frame {frame_index} must define a"
                " non-empty unique Atoms.info['configuration_id']."
            )
        if configuration_id in configuration_ids:
            raise ValueError(
                f"{path}: duplicate DFT configuration_id={configuration_id!r};"
                " repeated frames do not count as independent validation"
                " configurations."
            )
        ...
        if duplicate_configuration_id is not None:
            raise ValueError(
                f"{path}: DFT configuration_id={configuration_id!r} duplicates"
                " the exact periodic geometry of"
                f" configuration_id={duplicate_configuration_id!r} (geometry_sha256={geometry_sha256});"
                " changing configuration_id does not make a repeated frame"
                " independent."
            )
        ...
        if (
            ...
            raise ValueError(
                f"{path}: frame {configuration_id!r} must define finite"
                f" numeric Atoms.info['temperature_K'], got {temperature_K!r}."
            )
        if (
            ...
            raise ValueError(
                f"{path}: frame"
                f" {configuration_id!r} target_pressure_GPa={target_pressure_GPa!r} is"
                " not finite and within qualification scope"
        ...
        for info_key, expected_value in expected_info.items():
            if atoms.info.get(info_key) != expected_value:
                raise ValueError(
                    f"{path}: frame"
                    f" {configuration_id!r} {info_key}={atoms.info.get(info_key)!r},"
                    f" expected {expected_value!r} from the benchmark's DFT"
                    " provenance."
                )
        for info_key, expected_value in (
            ...
            if not isinstance(observed_value, (int, float)) or not np.isclose(
                ...
                raise ValueError(
                    f"{path}: frame"
                    f" {configuration_id!r} {info_key}={observed_value!r},"
                    f" expected {expected_value}."
                )
        numbers = np.asarray(atoms.numbers)
        if len(atoms) < 2 or not np.all(numbers == expected_atomic_number):
            raise ValueError(
                f"{path}: frame {configuration_id!r} must contain at least two"
                f" and only {chemical_symbol} atoms;"
                f" atomic_numbers={sorted(set(numbers.tolist()))}."
            )
        if not bool(np.all(atoms.pbc)):
            raise ValueError(
                f"{path}: frame {configuration_id!r} must be periodic in all"
                " axes for bulk, interface, stress, and NPT validation;"
        ...
        if not np.isfinite(positions).all() or not np.isfinite(cell).all():
            raise FloatingPointError(
                f"{path}: frame {configuration_id!r} has non-finite positions"
                " or cell."
            )
        if float(np.linalg.det(cell)) <= 0.0:
            raise ValueError(
                f"{path}: frame {configuration_id!r} has non-positive cell"
                f" determinant {float(np.linalg.det(cell))}."
            )
        ...
        if missing:
            raise ValueError(
                f"{path}: DFT reference frame"
                f" {frame_index} state={state!r} lacks calculator results"
                f" {missing}; energy, forces, and stress are required for NPT"
                " MD use."
            )
        ...
        if (
            ...
            raise FloatingPointError(
                f"{path}: frame {configuration_id!r} has invalid/non-finite"
                " reference energy, forces, or stress."
            )
    for state, temperatures in state_temperatures.items():
        if len(temperatures) < reference.minimum_frames_per_state:
            raise ValueError(
                f"{path}: state={state!r} has {len(temperatures)} independent"
                " frames, below "
                "dft_reference.minimum_frames_per_state="
                f"{reference.minimum_frames_per_state}."
            )
        ...
        if (
            min(temperatures) > required_range[0]
            or max(temperatures) < required_range[1]
        ):
            raise ValueError(
                f"{path}: state={state!r} temperature coverage"
                f" [{min(temperatures)}, {max(temperatures)}] K does not span"
                f" qualification range {required_range}."
            )
        ...
        if (
            ...
            raise ValueError(
                ...
                f" [{min(pressures)}, {max(pressures)}] GPa does not span"
                f" qualification range {scope.pressure_range_GPa}."
            )
    evidence = {
        "path": str(path),
...
def dft_error_metrics(
    frames: list[Atoms], calculator: object
) -> dict[str, Any]:
    ...
    for atoms in frames:
        ...
        reference_forces = np.asarray(
            atoms.get_forces(), dtype=np.float64
        ).copy()
        reference_stress = np.asarray(
            atoms.get_stress(voigt=True), dtype=np.float64
        ).copy()
        ...
        predicted_stress = np.asarray(
            predicted.get_stress(voigt=True), dtype=np.float64
        )
        if (
            ...
            raise FloatingPointError(
                "MLIP prediction is non-finite for DFT configuration "
        ...
        stress_errors_GPa.append(
            (predicted_stress - reference_stress) / units.GPa
        )
    ...
    def summarize(indices: list[int]) -> dict[str, float | int]:
        ...
        selected_stress = np.stack(
            [stress_errors_GPa[index] for index in indices]
        )
        values = np.concatenate(
            [
                selected_energy.ravel(),
                selected_forces.ravel(),
                selected_stress.ravel(),
            ]
        )
        if not np.isfinite(values).all():
            raise FloatingPointError(
                "Non-finite residual reached DFT error aggregation."
            )
        return {
...
def _melting_evidence(
    ...
    *,
    ...
    if paths is None:
        return None, [
            "no direct-coexistence temperature scan summaries were supplied"
        ]
    try:
        from scipy.stats import t as student_t
    except ImportError as exc:
        raise ImportError(
            "Melting-evidence verification requires scipy in the pointnet"
            " environment."
        ) from exc
    ...
    def verify_file_digest(
        ...
        *,
        ...
        if expected_digest != observed_digest:
            raise RuntimeError(
                f"{summary_path}: {context} SHA-256 mismatch for {file_path}:"
                f" report={expected_digest!r}, observed={observed_digest!r}."
            )
        return file_path
    ...
    def assert_close(
        ...
        *,
        ...
        if (
            ...
            raise RuntimeError(
                f"{summary_path}: self-reported {context}={observed!r} does"
                f" not equal the value {expected:.16g} recomputed from"
                " per-replica velocities."
            )
    ...
    def verify_run_artifacts(
        ...
        *,
        ...
        if replica_run.get("run_name") != run_name:
            raise RuntimeError(
                f"{summary_path}: replica"
                f" {replica_index} run_name={replica_run.get('run_name')!r},"
                f" expected {run_name!r} from the parsed transition config."
            )
        ...
        if (
            ...
            raise ValueError(
                f"{summary_path}: {run_name} must bind exactly the per-run"
                " artifacts "
        ...
        for filename in sorted(required_artifacts):
            ...
            if not isinstance(record, dict) or set(record) != {
                ...
                raise TypeError(
                    f"{summary_path}: {run_name} artifact {filename} must"
                    " contain exactly path and sha256."
                )
            ...
            if artifact_path != expected_path:
                raise RuntimeError(
                    f"{summary_path}: artifact {filename} for"
                    f" {run_name} resolves to {artifact_path}, expected"
                    f" {expected_path} from the parsed config."
                )
            artifact_paths[filename] = artifact_path

        with artifact_paths["metadata.json"].open(
            "r", encoding="utf-8"
        ) as handle:
        ...
        if metadata.get("schema_version") != 2:
            raise ValueError(
                f"{artifact_paths['metadata.json']}: expected"
                f" schema_version=2, got {metadata.get('schema_version')!r}."
            )
        if metadata.get("branch") != asdict(branch):
            raise RuntimeError(
                f"{artifact_paths['metadata.json']}: branch metadata differs"
                " from the parsed transition config."
            )
        expected_replica = {
            "index": replica_index,
        ...
        if metadata.get("source") != expected_source:
            raise RuntimeError(
                f"{artifact_paths['metadata.json']}: source provenance differs"
                " from the hash-bound source selected by the transition"
                " config."
            )
        physics = metadata.get("physics")
        if not isinstance(physics, dict):
            raise TypeError(
                f"{artifact_paths['metadata.json']}: physics must be a"
                " mapping."
            )
        metadata_calculator = physics.get("calculator")
        if not isinstance(metadata_calculator, dict):
            raise TypeError(
                f"{artifact_paths['metadata.json']}: physics.calculator must"
                " be a mapping."
            )
        if (
            metadata_calculator.get("implementation_class")
            ...
            raise RuntimeError(
                f"{artifact_paths['metadata.json']}: calculator, ensemble, or"
                " PTM settings do not match the benchmarked parsed protocol."
            )
        ...
        def load_thermodynamic_arrays(
            ...
            *,
            ...
            with np.load(artifact_path, allow_pickle=False) as archive:
                ...
                if missing:
                    raise ValueError(
                        f"{artifact_path}: missing thermodynamic arrays"
                        f" {missing}."
                    )
                arrays = {
            ...
            if not np.array_equal(step, expected_steps):
                raise RuntimeError(
                    f"{artifact_path}: stored steps={step.tolist()} do not"
                    " equal the parsed protocol"
                    f" grid={expected_steps.tolist()}."
                )
            ...
            if (
                ...
                raise ValueError(
                    ...
                    f" ({frame_count}, {len(prepared_interface.atoms)}, 3),"
            ...
            if (
                cell.shape != (frame_count, 3, 3)
                or not np.isfinite(cell).all()
            ):
                raise FloatingPointError(
                    f"{artifact_path}: cell_vectors_A must be finite with"
                    f" shape ({frame_count}, 3, 3), got {cell.shape}."
                )
            ...
            if np.any(determinants <= 0.0) or not np.allclose(
                ...
                raise RuntimeError(
                    f"{artifact_path}: volume_A3 is inconsistent with positive"
                    " cell determinants."
                )
            return arrays

        production = load_thermodynamic_arrays(
            artifact_paths["trajectory.npz"],
            final_step=branch.production_steps,
        )
        ...
        if boundary_mismatches:
            raise RuntimeError(
                f"{summary_path}: {run_name} equilibration and production"
                " artifacts are not one continuous integrator trace at their"
                f" shared boundary; differing arrays={boundary_mismatches}."
            )
        ...
        with np.load(
            ...
            missing_progress = sorted(
                required_progress - set(progress_archive.files)
            )
            if missing_progress:
                raise ValueError(
                    f"{artifact_paths['transition_progress.npz']}: missing"
                    f" arrays {missing_progress}."
                )
            progress_arrays = {
        ...
        if not np.array_equal(progress_arrays["step"], production["step"]):
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: step array"
                " differs from the hash-bound production trajectory."
            )
        expected_time_ps = (
        ...
        if not np.array_equal(progress_arrays["time_ps"], expected_time_ps):
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: time_ps is"
                " inconsistent with the parsed timestep and production steps."
            )
        ...
        if (
            ...
            raise FloatingPointError(
                f"{artifact_paths['transition_progress.npz']}: front/profile"
                " arrays have invalid shapes or non-finite values."
            )
        if not np.allclose(
            mean_advance,
            np.mean(signed_advance, axis=1),
            rtol=1.0e-12,
            atol=1.0e-12,
        ):
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}:"
                " mean_interface_advance_A does not equal the mean of the two"
                " spatial fronts."
            )
        steady_mask = (
        ...
        if np.count_nonzero(steady_mask) < 3:
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: fewer than"
                " three stored frames occur in the parsed steady fit"
                " interval."
            )
        ...
        if (
            ...
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: minimum fitted"
                " profile "
                f"contrast={minimum_contrast:.6g} is below parsed gate="
                f"{transition_config.analysis.minimum_profile_contrast:.6g}."
            )
        mean_fit = _linear_fit(
            expected_time_ps[steady_mask], mean_advance[steady_mask]
        )
        front_fits = tuple(
            _linear_fit(
                expected_time_ps[steady_mask],
                signed_advance[steady_mask, front_index],
            )
        ...
        if (
            ...
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: recomputed"
                " spatial-front fits violate the parsed quality gates."
            )
        ...
        if signed_change < branch.minimum_crystalline_fraction_change or (
            ...
            raise RuntimeError(
                f"{artifact_paths['transition_progress.npz']}: recomputed"
                " front direction does not satisfy"
                f" branch={branch.name!r} expectations."
            )
        ...
        for artifact_array_name, reanalyzed_value in (
            ...
            (
                "signed_interface_advance_A",
                reanalyzed.signed_interface_advance_A,
            ),
            ...
            if (
                observed_value.shape != reanalyzed_value.shape
                or not np.allclose(
                    observed_value,
                    reanalyzed_value,
                    rtol=1.0e-12,
                ...
                raise RuntimeError(
                    f"{artifact_paths['transition_progress.npz']}:"
                    f" {artifact_array_name} differs from PTM reanalysis of"
                    " the hashed coordinate trajectory."
                )
        if asdict(stationarity) != asdict(reanalyzed.stationarity):
            raise RuntimeError(
                f"{summary_path}: {run_name} standalone stationarity"
                " recomputation differs from full PTM transition reanalysis."
            )
        metadata_transition = metadata.get("transition")
        if not isinstance(metadata_transition, dict):
            raise TypeError(
                f"{artifact_paths['metadata.json']}: transition must be a"
                " mapping."
            )
        if metadata_transition.get("thermodynamic_stationarity") != asdict(
            ...
            raise RuntimeError(
                f"{artifact_paths['metadata.json']}: thermodynamic"
                " stationarity summary does not equal recomputation from"
                " hashed trajectories."
            )
        recomputed = {
        ...
        for stored_key, recomputed_key in (
            ...
            observed = np.asarray(
                progress_arrays[stored_key], dtype=np.float64
            )
            ...
            if observed.shape != (2,) or not np.allclose(
                ...
                raise RuntimeError(
                    f"{artifact_paths['transition_progress.npz']}:"
                    f" {stored_key}={observed.tolist()} differs from raw-array"
    ...
    for path in paths:
        ...
        if result.get("schema_version") != 2:
            raise ValueError(
                f"{path}: expected transition velocity summary"
                " schema_version=2 with hash-bound per-run artifacts, got"
        ...
        if not isinstance(calculator, dict) or not isinstance(protocol, dict):
            raise TypeError(
                f"{path}: calculator and protocol must be mappings."
            )
        if (
            calculator.get("model_sha256") != model_sha256
            or calculator.get("head") != head
        ):
            raise RuntimeError(
                f"{path}: melting scan identifies"
                f" model_sha256={calculator.get('model_sha256')!r},"
                f" head={calculator.get('head')!r}, but the benchmarked"
                f" potential uses model_sha256={model_sha256!r},"
                f" head={head!r}."
            )
        if (
            ...
            raise RuntimeError(
                f"{path}: melting scan calculator implementation/settings do"
                " not match the qualified production calculator; observed"
                f" implementation={calculator.get('implementation_class')!r},"
                f" settings={calculator.get('settings')!r}, expected"
                f" implementation={implementation_class!r},"
                f" settings={calculator_settings}."
            )
        if protocol.get("chemical_symbol") != chemical_symbol:
            raise RuntimeError(
                f"{path}: melting protocol"
                f" chemical_symbol={protocol.get('chemical_symbol')!r},"
                f" expected {chemical_symbol!r}."
            )
        ...
        if not isinstance(pressure_GPa, (int, float)) or not (
            ...
            raise RuntimeError(
                f"{path}: melting protocol pressure_GPa={pressure_GPa!r} lies"
                f" outside qualification scope {scope.pressure_range_GPa}."
            )
        if (
            ...
            raise RuntimeError(
                f"{path}: melting protocol timestep_fs={timestep_fs!r} exceeds"
                f" scoped maximum {scope.maximum_timestep_fs}."
            )
        ...
        if protocol.get("config_sha256") != canonical_config_sha256:
            raise RuntimeError(
                f"{path}:"
                f" protocol.config_sha256={protocol.get('config_sha256')!r} does"
        ...
        if runtime_generator_record is None:
            ...
            if not isinstance(runtime_generator_record, dict) or set(
                ...
                raise TypeError(
                    f"{path}: runtime_generator must contain exactly"
                    " config_file, config_file_sha256, and config_sha256."
                )
            ...
            if (
                runtime_generator_record["config_sha256"]
                != runtime_config_sha256
            ):
                raise RuntimeError(
                    f"{path}: runtime_generator.config_sha256 does not match"
                    " the canonical parsed runtime generator configuration."
                )
        ...
        if (
            ...
            raise RuntimeError(
                f"{path}: parsed transition config uses a different model or"
                " calculator setting than the benchmarked production"
                " potential."
            )
        ...
        if campaign_source_evidence is not None:
            ...
            if campaign_source_evidence != observed_source_evidence:
                raise RuntimeError(
                    f"{path}: campaign_source_evidence differs from the"
                    " currently hash-verified prepared source."
                )
        ...
        expected_protocol = {
            ...
            "chemical_symbol": (
                transition_config.generator.system.chemical_symbol
            ),
        ...
        if protocol != expected_protocol:
            ...
            raise RuntimeError(
                f"{path}: transition summary protocol differs from the parsed,"
                f" hash-bound configuration in keys={differing_keys}."
            )
        ...
        cell = np.asarray(
            protocol.get("prepared_source_cell_vectors_A"), dtype=np.float64
        )
        if (
            ...
            raise ValueError(
                f"{path}: melting protocol system identity is"
                " incomplete/invalid."
            )
        expected_atom_count = 4 * int(np.prod(repetitions))
        if atom_count != expected_atom_count:
            raise ValueError(
                f"{path}: atom_count={atom_count} is inconsistent with the"
                f" declared conventional FCC repetitions={repetitions};"
        ...
        minimum_fit_r_squared = float(
            analysis.get("minimum_velocity_fit_r_squared")
        )
        minimum_profile_contrast = float(
            analysis.get("minimum_profile_contrast")
        )
        ...
        if (
            ...
            raise ValueError(
                f"{path}: invalid transition protocol analysis={analysis!r}."
            )
        ...
        if (
            ...
            raise ValueError(
                f"{path}: protocol.replica_seeds is invalid:"
                f" {replica_seeds!r}."
            )
        ...
        if not isinstance(temperature_runs, list) or not isinstance(
            ...
            raise TypeError(
                f"{path}: protocol temperature_runs and temperatures must be"
                " lists."
            )
        if len(temperature_runs) != len(temperature_summaries):
            raise ValueError(
                f"{path}: protocol has {len(temperature_runs)} temperature"
                f" runs but summary has {len(temperature_summaries)}."
            )
        protocol_temperatures = np.asarray(
            [run.get("temperature_K") for run in temperature_runs],
            dtype=np.float64,
        )
        if not np.isfinite(protocol_temperatures).all() or np.any(
            np.diff(protocol_temperatures) <= 0.0
        ):
            raise ValueError(
                f"{path}: protocol temperature grid must be finite and"
                f" strictly increasing, got {protocol_temperatures.tolist()}."
            )
        for branch_index, (run, summary, configured_branch) in enumerate(
            ...
            if not isinstance(run, dict) or not isinstance(summary, dict):
                raise TypeError(
                    f"{path}: every temperature run/summary must be a mapping."
                )
            if run.get("name") != summary.get("name") or run.get(
                "temperature_K"
            ) != summary.get("temperature_K"):
                raise RuntimeError(
                    f"{path}: protocol run name/temperature"
                    f" {run.get('name')!r}/"
            ...
            if out_of_scope_states:
                raise RuntimeError(
                    f"{path}: coexistence temperature {temperature_K} K is"
                    " outside qualification scope for"
                    f" states={out_of_scope_states}."
                )
            replica_count = summary.get("replica_count")
            replica_runs = summary.get("runs")
            if not isinstance(replica_runs, list):
                raise TypeError(
                    f"{path}: temperature {temperature_K} K summary.runs must"
                    " be a list."
                )
            if replica_count != len(replica_runs) or replica_count != len(
                replica_seeds
            ):
                raise RuntimeError(
                    f"{path}: temperature {temperature_K} K"
                    f" replica_count={replica_count!r},"
                    f" len(runs)={len(replica_runs)}, and"
                    f" len(protocol.replica_seeds)={len(replica_seeds)} must"
                    " agree."
                )
            ...
            if not isinstance(production_steps, int) or production_steps <= 0:
                raise ValueError(
                    f"{path}: invalid production_steps={production_steps!r}."
                )
            ...
            if (
                ...
                raise ValueError(
                    f"{path}: invalid steady-state fit interval"
                    f" {fit_interval!r}."
                )
            ...
            for replica_run in replica_runs:
                if not isinstance(replica_run, dict):
                    raise TypeError(
                        f"{path}: every per-replica run must be a mapping."
                    )
                ...
                if (
                    ...
                    raise ValueError(
                        f"{path}: invalid/duplicate"
                        f" replica_index={replica_index!r} at temperature"
                        f" {temperature_K} K."
                    )
                if (
                    replica_run.get("configured_replica_seed")
                    != replica_seeds[replica_index]
                ):
                    raise RuntimeError(
                        f"{path}: replica {replica_index} configured seed does"
                        " not match protocol.replica_seeds."
                    )
                ...
                if (
                    ...
                    raise ValueError(
                        f"{path}: invalid/duplicate"
                        f" simulation_seed={simulation_seed!r} at temperature"
                        f" {temperature_K} K."
                    )
                ...
                for reported_key in (
                    ...
                    if observed.shape != (2,) or not np.allclose(
                        ...
                        raise RuntimeError(
                            ...
                            f" {reported_key}={observed.tolist()} differs from"
                            " hashed-artifact"
                            f" recomputation={expected.tolist()}."
                        )
                ...
                if (
                    numeric_run.shape != (9,)
                    or not np.isfinite(numeric_run).all()
                ):
                    raise FloatingPointError(
                        f"{path}: replica {replica_index} at {temperature_K} K"
                        " has non-finite/incomplete fit diagnostics."
                    )
                if (
                    ...
                    raise RuntimeError(
                        f"{path}: replica {replica_index} at {temperature_K} K"
                        " violates its reported analysis quality gates;"
                        f" diagnostics={numeric_run.tolist()},"
                        f" analysis={analysis}."
                    )
                if replica_run.get("fit_step_interval") != fit_interval:
                    raise RuntimeError(
                        f"{path}: replica {replica_index} fit"
                        f" interval={replica_run.get('fit_step_interval')!r} differs"
                        f" from protocol interval={fit_interval}."
                    )
                ...
                if (
                    ...
                    raise ValueError(
                        f"{path}: invalid production initial cell for replica"
                        f" {replica_index}."
                    )
                if not np.allclose(
                    replica_cell,
                    ...
                    raise RuntimeError(
                        f"{path}: replica {replica_index} self-reported"
                        " production initial cell differs from the hashed"
            ...
            velocities = np.asarray(
                [
                    indexed_runs[index]["velocity_m_per_s"]
            ...
            for endpoint_index, expected_endpoint in enumerate(
                recomputed_interval
            ):
                assert_close(
                    ...
                    expected_endpoint,
                    context=(
                        f"confidence_interval_95_m_per_s[{endpoint_index}]"
                    ),
                    summary_path=path,
                )
        ...
        recomputed_zero_velocity = _resolve_zero_velocity(
            temperature_summaries
        )
        if (
            zero_velocity != recomputed_zero_velocity
            or bracket != recomputed_zero_velocity.get("bracket_temperature_K")
        ):
            raise RuntimeError(
                f"{path}: self-reported zero-velocity result does not match"
                " recomputation from per-replica velocities; reported"
                f" zero_velocity={zero_velocity!r}, reported"
                f" bracket={bracket!r},"
                f" recomputed={recomputed_zero_velocity!r}."
            )
        if bracket is None:
            reason = zero_velocity.get("reason")
            if not isinstance(reason, str) or not reason:
                raise ValueError(
                    f"{path}: unresolved melting scan must provide"
                    " zero_velocity.reason."
                )
            failures.append(
                f"{path}: zero-velocity bracket unresolved: {reason}"
            )
        else:
            ...
            if (
                ...
                or len(bracket) != 2
                or not all(
                    isinstance(value, (int, float)) for value in bracket
                )
                or not np.isfinite(bracket).all()
                or not float(bracket[0]) < float(bracket[1])
                or zero_velocity.get("status")
                != "resolved_for_this_finite_protocol"
                or not isinstance(interpolated, (int, float))
                or not np.isfinite(interpolated)
                or not float(bracket[0])
                <= float(interpolated)
                <= float(bracket[1])
            ):
                raise ValueError(
                    f"{path}: malformed resolved zero-velocity result:"
                    f" bracket={bracket!r}, interpolated={interpolated!r},"
                    f" status={zero_velocity.get('status')!r}."
                )
            interpolated_temperature = float(interpolated)
            interpolated_temperatures.append(interpolated_temperature)
            melting_error = abs(
                interpolated_temperature - reference.temperature_K
            )
            if melting_error > reference.maximum_absolute_error_K:
                failures.append(
                    f"{path}: predicted melting temperature"
                    f" {interpolated_temperature:.3f} K differs from reference"
                    f" {reference.temperature_K:.3f} K by"
                    f" {melting_error:.3f} K, above"
                    f" {reference.maximum_absolute_error_K:.3f} K"
                )
        scans.append(
            {
                "path": str(path),
    ...
    if len(paths) < reference.minimum_protocol_count:
        failures.append(
            ...
            f" {reference.minimum_protocol_count} independent"
            " finite-size/orientation protocols are required"
        )
    if len(protocol_signatures) < reference.minimum_protocol_count:
        failures.append(
            "coexistence summaries do not provide enough distinct"
            " cell-size/orientation protocols for finite-protocol validation"
        )
    if len(interpolated_temperatures) >= 2:
        spread = max(interpolated_temperatures) - min(
            interpolated_temperatures
        )
        if spread > reference.maximum_protocol_spread_K:
            failures.append(
                "coexistence protocol melting-temperature spread"
                f" {spread:.3f} K exceeds"
    ...
    *,
    ...
    if len(set(model_names)) != len(model_names):
        raise ValueError(
            ...
            "Use explicit names for distinct checkpoints/heads."
        )
    if any(
        item.system.chemical_symbol != config.chemical_symbol
    ...
    for label, atoms in application_frames.items():
        if not np.all(
            atoms.numbers == atomic_numbers[config.chemical_symbol]
    ...
    for generator_config in generator_configs:
        ...
        if (
            ...
            raise RuntimeError(
                "Benchmark calculator identity"
                f" {calculator_provenance.identity!r} does not match"
                f" configured model={potential.model_name!r},"
                f" sha256={potential.sha256!r}, head={potential.head!r}."
                " Qualification cannot be attributed to a different"
                " calculator."
            )
        eos_results = {
        ...
        fcc_lattice_constant_A = eos_results["fcc"][
        ...
        if reference_frames is None:
            ...
            for state_index, state in enumerate(
                ...
                state_frames = [
                    ...
                    if atoms.info["state"] == state
                ]
                target_temperature = (
                    config.qualification_scope.state_temperature_ranges_K[
                        state
                    ][1]
                )
                selected_frame = min(
        ...
        nve_results = {
            ...
            for state_index, (
                state,
                (initial_atoms, temperature_K),
            ) in enumerate(nve_initial_states.items())
        }
        ...
        dft_results = (
            None
            if reference_frames is None
            else dft_error_metrics(reference_frames, calculator)
        )

        failure_reasons: list[str] = []
        if ground_state != "fcc":
            failure_reasons.append(
                f"static phase ordering predicts {ground_state} rather than"
                " FCC as the Al ground state"
            )
        for metric, threshold in (
            ...
            if observed > threshold:
                failure_reasons.append(
                    f"Verlet cache parity {metric}={observed:.6g} exceeds"
                    f" {threshold:.6g}"
                )
        for state, state_nve_results in nve_results.items():
            maximum_absolute_drift = max(
                abs(item["drift_meV_per_atom_ps"])
                for item in state_nve_results
            )
            maximum_excursion = max(
                item["maximum_excursion_meV_per_atom"]
                for item in state_nve_results
            )
            maximum_detrended_rms = max(
                item["detrended_rms_meV_per_atom"]
                for item in state_nve_results
            )
        ...
        if dft_results is None:
            failure_reasons.append(
                "no provenance-complete Al DFT reference covering solid,"
                " liquid, interface, strained, nucleus, HCP, and BCC states"
                " was supplied"
            )
        else:
            for state, state_metrics in dft_results["by_state"].items():
                for metric, threshold in (
                    ...
                    (
                        "force_rmse_eV_per_A",
                        config.maximum_dft_force_rmse_eV_per_A,
                    ),
        ...
        results[model_name] = {
            "identity": {
                ...
                "implementation_class": (
                    calculator_provenance.implementation_class
                ),
    ...
    for candidate_name in model_names[1:]:
        candidate_fcc_energy = results[candidate_name]["equation_of_state"][
            "fcc"
        ]["equilibrium_energy_per_atom_eV"]
        pair_key = f"{candidate_name}_minus_{baseline_name}"
        pair_result: dict[str, Any] = {}
        for label in application_frames:
            baseline_forces = application_force_predictions[baseline_name][
                label
            ]
            candidate_forces = application_force_predictions[candidate_name][
                label
            ]
            baseline_excess = (
            ...
            pair_result[label] = {
                ...
                "force_rmse_eV_per_A": float(
                    np.sqrt(
                        np.mean(np.square(candidate_forces - baseline_forces))
                    )
                ),
            }
    ...
    report = {
        ...
        "selection": {
            "qualified_models": [
                ...
                if result["scientifically_qualified"]
            ],
            "qualified_replacement_candidates": qualified_candidates,
            "automatic_replacement_allowed": False,
            "manual_comparison_required": True,
            "policy": (
                "Qualification establishes eligibility, not superiority. A"
                " newer foundation model never replaces the baseline"
                " automatically; compare per-state DFT errors, thermodynamics,"
                " stability, speed, and uncertainty for the intended claim."
            ),
        },
    }
    ...
    for model_name, model_result in results.items():
        ...
        qualification_report = {
            ...
            "head": identity["head"],
            "scientifically_qualified": model_result[
            ...
            "evidence": {
                "benchmark_config_canonical_sha256": (
                    hashlib.sha256(
                        json.dumps(
                            config.to_dict(),
                            sort_keys=True,
                ...
                "benchmark_config_file_sha256": _sha256_file(
                    config.config_path
                ),
    ...
    for qualification_path, qualification_report in qualification_payloads:
        qualification_report["benchmark_report_sha256"] = (
