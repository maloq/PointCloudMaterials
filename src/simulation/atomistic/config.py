...
def potential_calculator_settings(
    potential: PotentialConfig,
) -> dict[str, object]:
...
def _required(
    mapping: dict[str, Any], key: str, context: str, path: Path
) -> Any:
...
def _reject_unknown_keys(
    ...
    if unknown:
        raise KeyError(
            f"{path}: unsupported keys in {context}: {unknown}. Remove"
            " obsolete/ad hoc controls instead of relying on ignored"
            " configuration."
        )
...
def _finite_range(
    ...
    *,
    ...
    if any(
        ...
        raise TypeError(
            f"{path}: {context} endpoints must be explicit numbers, got"
            f" {value!r}."
        )
    lower = float(value[0])
    upper = float(value[1])
    ...
    if lower > upper:
        raise ValueError(
            f"{path}: {context} minimum must not exceed its maximum, got"
            f" {value!r}."
        )
    return lower, upper
...
def _resolved_evidence_path(
    value: Any, *, context: str, report_path: Path
) -> Path:
    if not isinstance(value, str) or not value:
        raise TypeError(
            f"{report_path}: {context} must be a non-empty path string, got"
            f" {value!r}."
        )
    ...
    *,
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"{report_path}: {context} does not exist: {path}."
        )
...
def _validate_qualification_chain(
    ...
    *,
    ...
    if report_model_name != expected_model_name:
        raise RuntimeError(
            f"{report_path}: qualification report"
            f" model_name={report_model_name!r}, expected"
    ...
    failures = _required(
        report, "qualification_failures", "report", report_path
    )
    if not isinstance(failures, list) or any(
        not isinstance(failure, str) or not failure for failure in failures
    ):
        raise TypeError(
            f"{report_path}: qualification_failures must be a list of"
            f" non-empty strings, got {failures!r}."
        )
    if scientifically_qualified and failures:
        raise RuntimeError(
            f"{report_path}: scientifically_qualified=true conflicts with"
            f" non-empty qualification_failures={failures!r}."
        )
    if not scientifically_qualified and not failures:
        raise RuntimeError(
            f"{report_path}: scientifically_qualified=false requires explicit"
            " failure reasons."
        )
    ...
    if not isinstance(qualification_reports, dict):
        raise TypeError(
            f"{benchmark_path}: qualification_reports must be a mapping."
        )
    ...
    if linked_child_path != report_path.resolve():
        raise RuntimeError(
            f"{benchmark_path}: qualification report link for"
            f" {expected_model_name!r} points to {linked_child_path}, not the"
            f" configured child {report_path.resolve()}."
        )
    ...
    if not isinstance(models, dict) or not isinstance(
        ...
        raise RuntimeError(
            f"{benchmark_path}: no model result exists for"
            f" {expected_model_name!r}."
        )
    ...
    if not isinstance(identity, dict):
        raise TypeError(
            f"{benchmark_path}: models.{expected_model_name}.identity must be"
            " a mapping."
        )
    if (
        ...
        raise RuntimeError(
            f"{benchmark_path}: parent model identity={identity!r} does not"
            f" match configured model={expected_model_name!r},"
            f" sha256={expected_model_sha256!r}, head={expected_head!r}."
        )
    if (
        model_result.get("scientifically_qualified")
        != scientifically_qualified
        or model_result.get("qualification_failures") != failures
    ):
        raise RuntimeError(
            f"{report_path}: child qualification status/failures do not match"
            f" parent model result in {benchmark_path}."
        )
    ...
    if not isinstance(qualified_models, list) or (
        ...
        raise RuntimeError(
            f"{benchmark_path}:"
            f" selection.qualified_models={qualified_models!r} is inconsistent"
            f" with model qualification status={scientifically_qualified}."
        )
    if (
        selection.get("automatic_replacement_allowed") is not False
        or selection.get("manual_comparison_required") is not True
    ):
        raise RuntimeError(
            f"{benchmark_path}: benchmark selection policy must forbid"
            " automatic replacement and require manual comparison."
        )
    ...
    if not isinstance(benchmark_config, dict):
        raise TypeError(
            f"{benchmark_path}: benchmark_config must be a mapping."
        )
    ...
    if not isinstance(benchmark_scope, dict):
        raise TypeError(
            f"{benchmark_path}: benchmark_config.qualification_scope must be a"
            " mapping."
        )
    ...
    if not isinstance(benchmark_claims, dict):
        raise TypeError(
            f"{benchmark_path}:"
            " benchmark_config.qualification_scope.authorized_claims must be"
            " a mapping."
        )
    if set(benchmark_claims) != set(POTENTIAL_CLAIM_NAMES) or any(
        type(benchmark_claims.get(claim)) is not bool
        for claim in POTENTIAL_CLAIM_NAMES
    ):
        raise TypeError(
            f"{benchmark_path}:"
            " benchmark_config.qualification_scope.authorized_claims must"
            " contain exactly boolean values for"
            f" {list(POTENTIAL_CLAIM_NAMES)}, got {benchmark_claims!r}."
        )
    effective_authorized_claims = {
        claim: bool(
            scientifically_qualified and benchmark_claims.get(claim) is True
        )
        for claim in POTENTIAL_CLAIM_NAMES
    }
    ...
    if scope_mismatches:
        raise RuntimeError(
            f"{report_path}: qualification scope expands or differs from its"
            f" bound benchmark configuration: mismatches={scope_mismatches}."
        )
    ...
    if evidence.get("benchmark_config_canonical_sha256") != canonical_sha256:
        raise RuntimeError(
            f"{report_path}: benchmark config canonical digest does not match"
            " parent report."
        )
    ...
    parent_melting_evidence = (
        benchmark.get("melting_scans", {}).get(expected_model_name)
        if isinstance(benchmark.get("melting_scans"), dict)
        else None
    )
    if (
        evidence.get("dft_reference") != parent_dft_evidence
        or evidence.get("melting_scans") != parent_melting_evidence
    ):
        raise RuntimeError(
            f"{report_path}: child DFT/melting evidence does not exactly match"
            f" parent benchmark {benchmark_path}."
        )
    if report.get("melting_scan") != parent_melting_evidence:
        raise RuntimeError(
            f"{report_path}: top-level melting_scan does not match parent"
            " evidence."
        )
    parent_child_pairs = (
    ...
    for child_key, parent_key in parent_child_pairs:
        if report.get(child_key) != model_result.get(parent_key):
            raise RuntimeError(
                f"{report_path}: child {child_key} does not match parent model"
                " result."
            )
    ...
    if parent_dft_evidence is not None:
        if not isinstance(parent_dft_evidence, dict):
            raise TypeError(
                f"{benchmark_path}: dft_reference_evidence must be a mapping."
            )
    ...
    if parent_melting_evidence is not None:
        if not isinstance(parent_melting_evidence, dict) or not isinstance(
            ...
            raise TypeError(
                f"{benchmark_path}: melting evidence/scans are malformed."
            )
        for scan in parent_melting_evidence["scans"]:
            if not isinstance(scan, dict):
                raise TypeError(
                    f"{benchmark_path}: every melting scan must be a mapping."
                )
    ...
    if scientifically_qualified and (
        ...
        raise RuntimeError(
            f"{report_path}: scientifically_qualified=true requires non-null"
            " DFT errors and direct-coexistence melting evidence in the bound"
            " parent benchmark."
        )
...
def _parse_potential_qualification(
    ...
    *,
    ...
    if report_sha256 != expected_model_sha256 or report_head != expected_head:
        raise RuntimeError(
            f"{report_path}: validation report identifies"
            f" model_sha256={report_sha256!r}, head={report_head!r}, but the"
            f" active potential uses model_sha256={expected_model_sha256!r},"
    ...
    if not isinstance(chemical_symbol, str) or not chemical_symbol:
        raise TypeError(
            f"{report_path}: report.scope.chemical_symbol must be a non-empty"
            f" string, got {chemical_symbol!r}."
        )
    ...
    state_ranges_raw = _mapping(
        scope, "state_temperature_ranges_K", report_path
    )
    missing_states = sorted(
        set(ALLOWED_QUALIFIED_STATES) - set(state_ranges_raw)
    )
    unknown_states = sorted(
        set(state_ranges_raw) - set(ALLOWED_QUALIFIED_STATES)
    )
    if missing_states or unknown_states:
        raise KeyError(
            f"{report_path}: report.scope.state_temperature_ranges_K has"
            f" missing required states={missing_states} and unknown"
            f" states={unknown_states}; supported"
            f" states={list(ALLOWED_QUALIFIED_STATES)}."
        )
    state_temperature_ranges_K = {
        state: _finite_range(
    ...
    if missing_claims or unknown_claims:
        raise KeyError(
            f"{report_path}: report.scope.authorized_claims must contain"
            f" exactly {list(POTENTIAL_CLAIM_NAMES)};"
    ...
    if any(
        ...
        raise TypeError(
            f"{report_path}: every report.scope.authorized_claims value must"
            f" be an exact boolean, got {authorized_claims_raw!r}."
        )
    authorized_claims = {
    ...
    compile_mode = calculator_settings.get(
        "compile_mode", DEFAULT_COMPILE_MODE
    )
    compile_fullgraph = calculator_settings.get(
        "compile_fullgraph", DEFAULT_COMPILE_FULLGRAPH
    )
    pad_num_atoms = calculator_settings.get(
        "pad_num_atoms", DEFAULT_PAD_NUM_ATOMS
    )
    pad_num_edges = calculator_settings.get(
        "pad_num_edges", DEFAULT_PAD_NUM_EDGES
    )
    ...
    if not isinstance(implementation_class, str) or not implementation_class:
        raise TypeError(
            f"{report_path}:"
            " report.scope.calculator_settings.implementation_class must be a"
            f" non-empty string, got {implementation_class!r}."
        )
    if not isinstance(device, str) or not device:
        raise TypeError(
            f"{report_path}: report.scope.calculator_settings.device must be a"
            f" non-empty string, got {device!r}."
        )
    if not isinstance(default_dtype, str) or not default_dtype:
        raise TypeError(
            f"{report_path}: report.scope.calculator_settings.default_dtype"
            f" must be a non-empty string, got {default_dtype!r}."
        )
    if not isinstance(enable_cueq, bool):
        raise TypeError(
            f"{report_path}: report.scope.calculator_settings.enable_cueq must"
            f" be a boolean, got {enable_cueq!r}."
        )
    if not isinstance(enable_oeq, bool):
        raise TypeError(
            f"{report_path}: report.scope.calculator_settings.enable_oeq must"
            f" be a boolean, got {enable_oeq!r}."
        )
    if (
        compile_mode is not None
        and compile_mode not in SUPPORTED_COMPILE_MODES
    ):
        raise ValueError(
            f"{report_path}: report.scope.calculator_settings.compile_mode"
            f" must be null or one of {sorted(SUPPORTED_COMPILE_MODES)}, got"
            f" {compile_mode!r}."
        )
    if not isinstance(compile_fullgraph, bool):
        raise TypeError(
            f"{report_path}:"
            " report.scope.calculator_settings.compile_fullgraph must be a"
    ...
    if not isinstance(md_property_mode, str) or (
        ...
        raise ValueError(
            f"{report_path}: report.scope.calculator_settings.md_property_mode"
            f" must be one of {sorted(SUPPORTED_MD_PROPERTY_MODES)}, got"
            f" {md_property_mode!r}."
        )
    ...
    if observed_settings != expected_settings:
        raise RuntimeError(
            f"{report_path}: validation report calculator settings do not"
            " match the configured production implementation;"
            f" observed={observed_settings}, expected={expected_settings}."
        )
...
def validate_potential_qualification(
    ...
    *,
    ...
    if required_claim not in POTENTIAL_CLAIM_NAMES:
        raise ValueError(
            f"{context}: required_claim={required_claim!r} is unsupported;"
            f" expected one of {list(POTENTIAL_CLAIM_NAMES)}."
        )
    ...
    if qualification is None or report_path is None:
        raise RuntimeError(
            f"{context}: quantitative potential use requires a parsed"
    ...
    if observed_report_sha256 != config.potential.validation_report_sha256:
        raise RuntimeError(
            f"{context}: qualification report changed after configuration"
            f" load: path={report_path},"
            f" loaded_sha256={config.potential.validation_report_sha256},"
            f" observed_sha256={observed_report_sha256}. Reload and revalidate"
            " the configuration."
        )
    ...
    if qualification.report_type != QUALIFICATION_REPORT_TYPE:
        raise RuntimeError(
            f"{context}: qualification report {report_path} has"
            f" report_type={qualification.report_type!r}, expected"
            f" {QUALIFICATION_REPORT_TYPE!r}."
        )
    if qualification.authorized_claims[required_claim] is not True:
        raise RuntimeError(
            f"{context}: qualification report {report_path} does not authorize"
            f" required claim={required_claim!r};"
            f" authorized_claims={qualification.authorized_claims}."
        )
    if (
        ...
        raise RuntimeError(
            f"{context}: qualification report {report_path} is bound to"
            f" model_sha256={qualification.model_sha256!r},"
            f" head={qualification.head!r}, but the active potential uses"
            f" model_sha256={config.potential.sha256!r},"
    ...
    if qualified_settings != requested_settings:
        raise RuntimeError(
            f"{context}: requested calculator"
            f" settings={requested_settings} differ from qualified"
            f" settings={qualified_settings} in {report_path}."
        )
    if qualification.chemical_symbol != chemical_symbol:
        raise RuntimeError(
            f"{context}: qualification report {report_path} covers"
            f" chemical_symbol={qualification.chemical_symbol!r}, not"
            f" requested {chemical_symbol!r}."
        )
    pressure_min, pressure_max = qualification.pressure_range_GPa
    if not pressure_min <= pressure_GPa <= pressure_max:
        raise RuntimeError(
            f"{context}: requested pressure={pressure_GPa} GPa is outside"
            f" qualification range [{pressure_min}, {pressure_max}] GPa in"
            f" {report_path}."
        )
    if timestep_fs > qualification.maximum_timestep_fs:
        raise RuntimeError(
            f"{context}: requested timestep={timestep_fs} fs exceeds qualified"
            f" maximum={qualification.maximum_timestep_fs} fs in"
            f" {report_path}."
        )
    for state, temperatures_K in state_temperatures_K.items():
        if state not in qualification.state_temperature_ranges_K:
            raise RuntimeError(
                f"{context}: state={state!r} has no temperature qualification"
                f" in {report_path}."
            )
        ...
        if out_of_scope:
            raise RuntimeError(
                f"{context}: state={state!r} requests"
                f" temperatures_K={out_of_scope} outside qualified range"
                f" [{temperature_min}, {temperature_max}] K in {report_path}."
            )


def _reject_density_controls(
    value: Any, *, path: Path, location: str = "root"
) -> None:
    forbidden = {
        "density_target",
        "rho_target",
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            ...
            if str(key) in forbidden:
                raise ValueError(
                    f"{path}: {child_location} is not accepted. Number density"
                    " is an NPT simulation result, not a generator input."
                )
            _reject_density_controls(item, path=path, location=child_location)
    elif isinstance(value, list):
        for index, item in enumerate(value):
            _reject_density_controls(
...
def load_config(path: str | Path) -> GeneratorConfig:
    ...
    if not model_path.is_relative_to(datasets_root):
        raise ValueError(
            f"{config_path}: potential.model_path must be repository-owned"
            f" data below {datasets_root}, got {model_path}."
        )
    repetitions_raw = _required(
        system_raw, "repetitions", "system", config_path
    )
    if not isinstance(repetitions_raw, list) or len(repetitions_raw) != 3:
        raise ValueError(
            f"{config_path}: system.repetitions must be a three-item list, got"
            f" {repetitions_raw!r}."
        )
    if any(
        not isinstance(value, int) or isinstance(value, bool)
        for value in repetitions_raw
    ):
        raise TypeError(
    ...
    if min(repetitions) < 2:
        raise ValueError(
            f"{config_path}: every system.repetitions entry must be >= 2, got"
            f" {repetitions}."
        )
    ...
    if not isinstance(random_seeds_raw, list) or not random_seeds_raw:
        raise TypeError(
            f"{config_path}: random_seeds must be a non-empty list of"
            f" integers, got {random_seeds_raw!r}."
        )
    if any(
    ...
    target_temperature = _positive_float(
        _required(
            dynamics_raw, "target_temperature_K", "dynamics", config_path
        ),
    ...
    if melt_temperature <= target_temperature:
        raise ValueError(
            f"{config_path}:"
            f" dynamics.melt_temperature_K={melt_temperature} must exceed"
            f" target_temperature_K={target_temperature}."
        )

    slab_fraction = float(
        _required(system_raw, "liquid_slab_fraction", "system", config_path)
    )
    if not 0.2 <= slab_fraction <= 0.8:
        raise ValueError(
            f"{config_path}: system.liquid_slab_fraction must be within [0.2,"
            f" 0.8], got {slab_fraction}."
        )
    ...
    if density_cache_value is not None or density_error_value is not None:
        raise ValueError(
            f"{config_path}: legacy reference_density_cache validation is"
            " disabled. Its first/last-frame density inference has no"
            " explicit temperature, pressure, phase, units, protocol, or"
            " potential provenance and cannot qualify the configured MLIP."
            " Set both reference_density_cache and"
            " maximum_relative_density_error to null."
        )
    density_cache = None
    density_error = None
    ptm_rmsd_cutoff = _positive_float(
        _required(
            validation_raw, "ptm_rmsd_cutoff", "validation", config_path
        ),
    ...
    if ptm_rmsd_cutoff > 1.0:
        raise ValueError(
            f"{config_path}: validation.ptm_rmsd_cutoff is a dimensionless"
    ...
    for context, value in (
        ...
        if not 0.0 <= value <= 1.0:
            raise ValueError(
                f"{config_path}: {context} must be in [0, 1], got {value}."
            )
    if (
        minimum_interface_crystalline_fraction
        >= maximum_interface_crystalline_fraction
        ...
        raise ValueError(
            f"{config_path}: validation.minimum_interface_crystalline_fraction"
            " must be less than"
            " validation.maximum_interface_crystalline_fraction."
        )

    device = str(
        _required(potential_raw, "device", "potential", config_path)
    ).strip()
    if not device:
        raise ValueError(
            f"{config_path}: potential.device must be a non-empty string."
        )
    default_dtype = str(potential_raw.get("default_dtype", "float32"))
    if default_dtype not in {"float32", "float64"}:
        raise ValueError(
            f"{config_path}: potential.default_dtype must be 'float32' or"
            f" 'float64', got {default_dtype!r}."
        )
    ...
    if autocast_dtype_raw is not None and autocast_dtype_raw != "bfloat16":
        raise ValueError(
            f"{config_path}: potential.autocast_dtype must be null or"
            f" 'bfloat16', got {autocast_dtype_raw!r}."
        )
    autocast_dtype = autocast_dtype_raw
    if autocast_dtype is not None:
        if default_dtype != "float32":
            raise ValueError(
                f"{config_path}: BF16 autocast requires the checkpoint and"
                " graph inputs to use default_dtype='float32', got"
                f" {default_dtype!r}."
            )
        if not device.startswith("cuda"):
            raise ValueError(
                f"{config_path}: BF16 autocast requires"
                " potential.device='cuda' or an explicit CUDA index, got"
                f" {device!r}."
            )
    enable_cueq = _required(
        potential_raw, "enable_cueq", "potential", config_path
    )
    if not isinstance(enable_cueq, bool):
        raise TypeError(
    ...
    if (enable_cueq or enable_oeq) and not device.startswith("cuda"):
        raise ValueError(
            f"{config_path}: accelerated MACE kernels require"
            " potential.device='cuda' or an explicit CUDA index, got"
            f" device={device!r}, enable_cueq={enable_cueq},"
    ...
    if compile_mode is None:
        if compile_fullgraph or pad_num_atoms != 0 or pad_num_edges != 0:
            raise ValueError(
                f"{config_path}: compile_fullgraph and fixed padding require a"
                " non-null potential.compile_mode; got"
        ...
        if pad_num_atoms == 0 or pad_num_edges == 0:
            raise ValueError(
                f"{config_path}: compiled MACE requires explicit positive"
                " fixed-shape potential.pad_num_atoms and"
                f" potential.pad_num_edges; got {pad_num_atoms} and"
                f" {pad_num_edges}."
            )
    ...
    if nvt_md_property_mode is not None and (
        ...
        raise ValueError(
            f"{config_path}: potential.nvt_md_property_mode must be null or"
            f" 'forces', got {nvt_md_property_mode!r}."
        )
    ...
    if len(potential_sha256) != 64 or any(
        ...
        raise ValueError(
            f"{config_path}: potential.sha256 must be 64 lowercase hexadecimal"
            f" characters, got {potential_sha256!r}."
        )
    model_name = str(
        _required(potential_raw, "model_name", "potential", config_path)
    ).strip()
    family = str(
        _required(potential_raw, "family", "potential", config_path)
    ).strip()
    source_url = str(
    ...
    license_identifier = str(
        _required(
            potential_raw, "license_identifier", "potential", config_path
        )
    ).strip()
    for context, value in (
        ...
        if not value:
            raise ValueError(
                f"{config_path}: {context} must be a non-empty string."
            )
    potential_head_raw = _required(
        potential_raw, "head", "potential", config_path
    )
    if potential_head_raw is not None and not isinstance(
        potential_head_raw, str
    ):
        raise TypeError(
    ...
    potential_head = (
        potential_head_raw.strip()
        if isinstance(potential_head_raw, str)
        else None
    )
    if isinstance(potential_head_raw, str) and not potential_head:
        raise ValueError(
            f"{config_path}: potential.head must be a non-empty string or"
            " null."
        )
    usage_mode = str(
    ...
    if usage_mode == "quantitative" and nvt_md_property_mode is not None:
        raise RuntimeError(
            f"{config_path}:"
            f" potential.nvt_md_property_mode={nvt_md_property_mode!r} is an"
            " exploratory floating-point execution path. Set it to null until"
            " an exact force-only-NVT qualification report contract is"
            " implemented."
        )
    ...
    if validation_report is not None:
        ...
        if not isinstance(report, dict):
            raise TypeError(
                f"{validation_report}: validation report root must be a"
                f" mapping, got {type(report).__name__}."
            )
        if report.get("schema_version") != 1:
            raise RuntimeError(
                f"{validation_report}: expected validation report"
                f" schema_version=1, got {report.get('schema_version')!r}."
            )
        if report.get("report_type") != QUALIFICATION_REPORT_TYPE:
            raise RuntimeError(
                f"{validation_report}: expected"
                f" report_type={QUALIFICATION_REPORT_TYPE!r}, got"
                f" {report.get('report_type')!r}."
            )
        ...
        if not isinstance(scientifically_qualified_raw, bool):
            raise TypeError(
                f"{validation_report}: report.scientifically_qualified must be"
                f" a boolean, got {scientifically_qualified_raw!r}."
            )
    ...
    if usage_mode == "quantitative" and not scientifically_qualified:
        raise RuntimeError(
            f"{config_path}: potential.usage_mode='quantitative' requires a"
            " validation_report for the exact model SHA/head with"
            " scientifically_qualified=true. The configured potential has not"
    ...
    config = GeneratorConfig(
        ...
        dynamics=DynamicsConfig(
            pressure_GPa=float(
                _required(
                    dynamics_raw, "pressure_GPa", "dynamics", config_path
                )
            ),
            ...
            timestep_fs=_positive_float(
                _required(
                    dynamics_raw, "timestep_fs", "dynamics", config_path
                ),
            ...
            thermostat_time_fs=_positive_float(
                _required(
                    dynamics_raw, "thermostat_time_fs", "dynamics", config_path
                ),
            ...
            barostat_time_fs=_positive_float(
                _required(
                    dynamics_raw, "barostat_time_fs", "dynamics", config_path
                ),
            ...
            quench_steps=_nonnegative_int(
                _required(
                    dynamics_raw, "quench_steps", "dynamics", config_path
                ),
            ...
            quench_stages=_positive_int(
                _required(
                    dynamics_raw, "quench_stages", "dynamics", config_path
                ),
            ...
            sample_interval=_positive_int(
                _required(
                    dynamics_raw, "sample_interval", "dynamics", config_path
                ),
        ...
        system=SystemConfig(
            chemical_symbol=str(
                _required(system_raw, "chemical_symbol", "system", config_path)
            ),
            crystal_structure=str(
                _required(
                    system_raw, "crystal_structure", "system", config_path
                )
            ).lower(),
            ...
            interface_half_width_A=_positive_float(
                _required(
                    system_raw, "interface_half_width_A", "system", config_path
                ),
        ...
        output=OutputConfig(
            ...
            create_visualizations=bool(
                output_raw.get("create_visualizations", True)
            ),
        ),
