...
def build_calculator(config: GeneratorConfig) -> object:
    if not config.potential.model_path.is_file():
        raise FileNotFoundError(
            "Configured potential model is missing:"
            f" {config.potential.model_path}. Place the explicitly selected"
            " model below datasets/; generation never downloads a checkpoint"
            " or substitutes another potential."
        )
    observed_sha256 = sha256_file(config.potential.model_path)
    if observed_sha256 != config.potential.sha256:
        raise RuntimeError(
            f"Potential checksum mismatch for {config.potential.model_path}:"
            f" configured sha256={config.potential.sha256}, observed"
            f" sha256={observed_sha256}. Do not run a scientific benchmark"
            " with an unidentified checkpoint."
        )
    if (
        config.potential.device.startswith("cuda")
        and not torch.cuda.is_available()
    ):
        raise RuntimeError(
            f"potential.device={config.potential.device!r} requires CUDA, but"
            " torch reports that CUDA is unavailable. Select an available"
            " device explicitly."
        )
    try:
        ...
        raise ImportError(
            "The force-driven generator requires mace-torch in the pointnet"
            " environment."
        ) from exc
    if config.potential.compile_mode is not None:
        ...
        if config.potential.pad_num_atoms != produced_atom_count:
            raise ValueError(
                ...
                f" potential.pad_num_atoms={config.potential.pad_num_atoms}."
                " Use the exact repository-produced atom count;"
                " cross-workload padding belongs in a separate config."
            )
    ...
    if configured_head is None:
        if len(available_heads) != 1:
            raise RuntimeError(
                f"Configured MACE model {config.potential.model_path} exposes"
                f" heads={list(available_heads)}, but potential.head is null."
                " Select one head explicitly; multi-head energies and forces"
                " are different Hamiltonians."
            )
        expected_head = available_heads[0]
    else:
        if configured_head not in available_heads:
            raise RuntimeError(
                f"Configured potential.head={configured_head!r} does not exist"
                f" in MACE model {config.potential.model_path};"
                f" available_heads={list(available_heads)}. MACE may warn and"
                " fall back to a different head, which this generator"
                " forbids."
            )
    ...
    if selected_head != expected_head:
        raise RuntimeError(
            f"MACE selected head={selected_head!r} for"
            f" {config.potential.model_path}, but the validated expected head"
            f" is {expected_head!r}; available_heads={list(available_heads)}."
        )
    ...
    if calculator.kernel_backend != expected_backend:
        raise RuntimeError(
            "MACE calculator initialized"
            f" kernel_backend={calculator.kernel_backend!r}, but config"
            f" requires {expected_backend!r}. Backend substitution is"
            " forbidden."
        )
    if bool(calculator.use_compile) != (
        config.potential.compile_mode is not None
    ):
        raise RuntimeError(
            f"MACE calculator use_compile={calculator.use_compile!r} does not"
            " match configured"
            f" compile_mode={config.potential.compile_mode!r}."
        )
    if calculator.autocast_dtype != config.potential.autocast_dtype:
        raise RuntimeError(
            "MACE calculator initialized"
            f" autocast_dtype={calculator.autocast_dtype!r}, but config"
            f" requires {config.potential.autocast_dtype!r}."
        )
    return calculator
    ...
    *,
    ...
    if calculator is None:
        if injected_calculator_identity is not None:
            raise ValueError(
                "injected_calculator_identity was provided without an injected"
                " calculator."
            )
        ...
        if injected_calculator_identity is None:
            raise ValueError(
                "An injected calculator requires injected_calculator_identity."
                " This prevents test or alternate potentials from being"
                " persisted as the configured MACE Hamiltonian."
            )
        if config.potential.usage_mode == "quantitative":
            raise RuntimeError(
                "A quantitative qualification report is bound to the"
                " configured MACE model, head, and calculator settings. An"
                " injected calculator cannot inherit that qualification; use"
                " an explicit exploratory configuration for controlled"
    ...
    return selected_calculator, build_execution_provenance(
    ...
    *,
    ...
    if config.output.root_dir.exists() and not config.output.overwrite:
        raise FileExistsError(
            ...
            " output.overwrite=true only when replacement is intended. This"
            " check is performed before calculator construction or MD so an"
            " existing dataset cannot waste a production run."
        )
    ...
    progress(
        ...
        f" {len(config.random_seeds)} replicas,"
    ...
    for replica_index, random_seed in enumerate(config.random_seeds):
        ...
        replica_diagnostics, observed_reference = validate_systems(
            systems, config
        )
        ...
        if reference_densities is None:
            ...
            raise RuntimeError(
                "Repository density references changed while validating"
                f" replicas: first={reference_densities},"
                f" {replica_name}={observed_reference}."
            )
    ...
    progress(
        f"Wrote {len(environment_dirs)} environments to"
