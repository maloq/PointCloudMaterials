    ...
    *,
    ...
    if missing:
        raise FileNotFoundError(
            "Transition campaign cannot bind its prepared source because"
    ...
    loaded = (
        _load_prepared_interface(transition) if prepared is None else prepared
    )
    after = {
        relative.as_posix(): _sha256_file(path)
        for relative, path in zip(relative_paths, paths)
    ...
    if after != before:
        raise RuntimeError(
            "Transition source files changed while the prepared frame was"
            f" being loaded; before={before}, after={after}. Retry only after"
...
def load_content_bound_prepared_interface(
    ...
    if observed != config.source_evidence:
        raise RuntimeError(
            "The prepared transition source differs from the content-bound"
            f" campaign identity. expected={config.source_evidence},"
            f" observed={observed}."
        )
    return prepared
...
def _validate_runtime_generator(
    ...
    *,
    ...
    if scientific_mismatches:
        raise RuntimeError(
            f"{path}: runtime_generator_config changes source-bound scientific"
            f" or model fields: {scientific_mismatches}. Only"
            f" {sorted(RUNTIME_POTENTIAL_FIELDS)} may differ. Use a"
            " byte-identical model/head/dtype and the exact transition"
            " dynamics, system, and validation protocol."
        )


def load_transition_campaign_config(
    path: str | Path,
    ...
    if set(raw) != expected_keys:
        raise KeyError(
            f"{config_path}: root keys must be exactly"
    ...
    runtime_generator = load_config(
        _repo_path(raw["runtime_generator_config"])
    )
    ...
    chunk_steps = _positive_int(
        ...
        context="execution.chunk_steps",
        path=config_path,
    )
    if chunk_steps % transition.sample_interval:
        raise ValueError(
            f"{config_path}: execution.chunk_steps={chunk_steps} must be"
            " divisible by transition"
            f" sample_interval={transition.sample_interval}. Checkpoint"
            " boundaries must coincide with stored trajectory frames."
        )
    if transition.output.overwrite:
        raise ValueError(
            f"{config_path}: the queued campaign requires transition"
            " output.overwrite=false. Committed tasks are resumed and"
            " content-validated, never replaced implicitly."
        )
