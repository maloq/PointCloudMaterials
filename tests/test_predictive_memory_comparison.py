...
def completed_fits(root, modalities):
    for modality in modalities:
        for suffix, score in [('H0', 1.), ('H12', .9), ('H48', .8), ('H48-repeat', .85)]:
            ...
            (technical/'status.json').write_text(json.dumps(dict(state='complete', step=3000)))
...
def test_complete_paired_comparison_for_selected_modalities(tmp_path, modalities):
    completed_fits(tmp_path, modalities)
