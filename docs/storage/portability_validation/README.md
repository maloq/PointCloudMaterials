...
Operational validation: can the project launch outside this cluster without embedding mount
points in scientific settings, while using IDS for cache and SCRATCH for new MD?

Implementation and reproduction commands: [portable setup](../../portability.md).
...
This is operational documentation; operational receipts, logs and disposable fixtures are in
...
The original full snapshot failed its concurrent-write verification. The
[archive note](../../archived_research.md) documents the separately verified
retirement copies and the current research retention boundary.
