...
Execution revision: disjoint variable-size history graphs can be packed on one
atom axis, with per-example periodic cells and per-example pooling denominators.
Same-atom temporal attention never mixes examples. This changes floating-point
reduction order, not the physical protocol. `runtime.encoding` selects packed or
sequential execution; `runtime.residency` selects host, device or train_device.
`runtime.batch_size` limits graphs per encoder call, independently of the
source-sampled optimizer batch. Concatenated chunks retain one complete task loss
and its original eligible-example hazard reduction. It is not gradient accumulation.
Verified histories are validated once on CPU before synchronization-free forwards.
FP32 is retained. Normalized labels can reside on device but never enter the encoder.
