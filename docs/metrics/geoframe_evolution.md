...
Class 2 is a mixed-crystalline-neighborhood **proxy**: it can contain solid–liquid
interfaces, isolated accepted motifs and grain boundaries. Local PTM fractions
alone do not define a unique phase surface or separate all grain-boundary types.
The specific Al planar-fault assay must be interpreted separately. This
clarification changes no previously exported numerical calculation.
