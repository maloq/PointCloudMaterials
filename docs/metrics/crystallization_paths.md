...
The follow-up is a distinct protocol, documented in [crystallization_paths_refinement.md](crystallization_paths_refinement.md). Shared runtime/data code now supports its early stopping, EMA, variable history and cached event-index lookup; the original protocol's forecast definitions remain unchanged. Historical exports retain their original implementation hashes and execute from frozen campaign code.
