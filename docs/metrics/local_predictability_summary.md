...
The later H200 native update is preserved separately in
`technical/h200_native_reported_results.json`: training is reported complete,
with three rounded validation future-MSE values and test comparisons pending.
Its exact validation subset and reduction are unspecified. These values are not
combined with local test scores or assigned confidence intervals. Historical
exports retain their original capture and metric contract.
