...
Scalar-only context heads also accept the 128 scalar channels directly, omitting
unused tensor reshaping. This changes no metric or scalar-head computation; it
supports the GPU-resident structural-path companion study. Historical executable
snapshots retain their original input layout and metric hashes.
