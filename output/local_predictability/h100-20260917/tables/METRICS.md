...
The comparison family scores stored descriptor probabilities again on the exact
native anchor subset. It recalibrates the window-FPR threshold using that subset
of calibration sources and preserves source/center/anchor alignment. Per-source
candidate-minus-reference log-loss/Brier differences use paired resampling within
temperature; negative values favor the candidate. Intervals are exploratory,
unadjusted for multiple comparisons and conditional on the single trained seed.
The compared models and six horizons are fixed before these test results.


Table export: 2026-09-17T13:27:11.775487+00:00. The machine-readable values retain full precision; blank values mean undefined or unrecorded, never zero. Nested metric names preserve the producer's grouping. The implementation hashes are in `../technical/metric-contract.json`.
