...
Geometry MACE uses physical radius8Å, cutoff5Å, original GraphBank and trained
calibration buffers. Neighborhood JEPA uses the original scaled radius8 local
observation. Paired-relaxed models use their original nearest80 candidate cap
before radius cropping. Shared MACE/GATr use the pinned producer's local radius8
and material scale. GeoFrame uses its original nearest80/material radius. All
...
The new perturbation test displaces up to256 fixed candidate patches, holds the
...
All exports here have128 features per stage, allowing reuse of the already
computed128-zero-feature current-physics hazard baseline. Compare exact hazard
row/source/event identities before taking differences. At12ps calculate per-root
Brier(model)-Brier(current physics); for9ps calculate per-root residual MSE minus
zero baseline. `paired_mean_ci` resamples the15 paired roots 2000 times, seed20260923,
...
Table export: 2026-09-23T16:55:52.208878+00:00. The machine-readable values retain full precision; blank values mean undefined or unrecorded, never zero. Nested metric names preserve the producer's grouping. The implementation hashes are in `../technical/metric-contract.json`.
