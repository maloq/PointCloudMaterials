...
## Fourfold dynamic data and two-GPU MACE (19 September 2026)

The expanded release adds 763,560 dynamic anchors to the previous 254,520.
Five epoch equivalents at global B=2,048 mean 2,486 sampled updates. Static
rows are excluded; the 480 native-Al selection observations remain unchanged.
New source/frame anchor pairs exclude the parent tasks, balance training lineages,
and redistribute exhausted trajectory quotas only within the same lineage.
Source counts are not independent-sample counts. Fixed material scales, physical,
instantaneous-TDA, bond-order and backtracking definitions remain unchanged.

`parallel_mace.py` splits packed encoder microbatches across devices, restores
original row order, and differentiates the full global grouped objective once.
Replay gradients are summed before the original global clip and optimizer step;
there is no per-device VICReg averaging or change to denominators. Checkpoints
contain one encoder. Validation and baseline calculations remain on GPU 0.
The reference's 622 versus expanded fit's 2,486 updates are not compute matched.

## Execution-only two-GPU optimization (19 September 2026)

Process prefetch uses the same immutable row index and step-seeded sampler as
serial preparation. Workers never initialize CUDA; completed batches are returned
in update order and pinned in the training process. Prefetched future updates are
not consumed by checkpoint state and may be discarded safely. Full physical/TDA
labels, domains, temporal triplet ordering and time offsets must match serial
preparation exactly.

The deployed optimization preserves the previous two-GPU encoder executor byte
for byte, including its global objective, gradient sum, clipping and optimizer
semantics. Larger chunks and event-based scheduling were investigated but are not
part of this continuation. On one fixed trained checkpoint and batch, the original
executor itself showed approximately 22% relative encoder-gradient variation between
repeated zero-LR updates while losses differed by about 1e-6. This prevents treating
near-identical losses as evidence of tight gradient reproducibility. The audit and
its limits are retained separately; no model/precision change is introduced here.

An execution continuation requires a receipt naming the exact old/new training
identities and checkpoint SHA256. Data, architecture, library versions, loss and
...
Table export: 2026-09-19T01:04:32.441967+00:00. The machine-readable values retain full precision; blank values mean undefined or unrecorded, never zero. Nested metric names preserve the producer's grouping. The implementation hashes are in `../technical/metric-contract.json`.
